...
TL;DR: 本文提出了ParaBench基准测试来评估思维感知生成中的错误传播问题，并开发了MMaDA-Parallel并行多模态扩散框架，通过并行强化学习提升跨模态一致性，在ParaBench上相比最先进模型Bagel实现了6.9%的输出对齐改进。


<details>
  <summary>Details</summary>
Motivation: 现有顺序自回归方法在复杂任务中由于错误传播可能导致性能下降，特别是生成的推理与最终图像之间缺乏对齐的问题。
...
TL;DR: SliderEdit是一个用于指令式图像编辑的框架，通过引入可解释的滑块控制，实现对多指令编辑的连续精细控制。
...
Method: 提出SliderEdit框架，将多部分编辑指令解耦为全局训练的滑块，使用单一低秩适应矩阵集泛化到多样化编辑、属性和组合指令。
...
TL;DR: 本研究将原本用于图像分析的群体密度估计算法扩展到视频场景，通过引入去噪概率模型和扩散过程生成高质量密度图，结合事件驱动采样技术减少计算负担，在保持关键群体动态的同时提升实时监控效率。
...
Method: 集成去噪概率模型和扩散过程生成高质量密度图，使用窄高斯核生成多个密度图输出，结合回归分支进行精确特征提取，通过相似度评分整合机制产生稳健结果，并引入基于Farneback光流算法的事件驱动采样技术选择性捕获关键帧。

Result: 通过定性和定量评估（包括叠加图和平均绝对误差），模型在密集和稀疏场景中都能有效捕捉群体动态，采样方法显著减少帧数同时保持关键群体事件。

Conclusion: 该方法成功解决了视频分析中的时间挑战，为实时人群监控提供了一个可扩展且高效的解决方案，在保持准确性的同时显著降低了计算和存储需求。
...
<details>
  <summary>Details</summary>
Motivation: 解决室内GPS缺失环境下的定位问题，克服现有视觉方法如PALMS在智能手机LiDAR短距离和室内布局模糊性方面的限制。

Method: 使用基础单目深度估计模型从RGB图像重建尺度对齐的3D点云，然后通过卷积与平面图进行几何布局匹配，输出位置和方向的后验分布。

Result: 在Structured3D和自定义校园数据集上，PALMS+在静态定位精度上优于PALMS和F3Loc，无需训练；在33条真实轨迹上的序列定位中，定位误差更低。
...
TL;DR: 该论文提出了一种增强Social LSTM的新深度学习模型，通过引入动态占用空间损失函数来减少行人轨迹预测中的碰撞率，同时在不同人群密度下保持位移误差的准确性。


<details>
  <summary>Details</summary>
Motivation: 在动态拥挤环境中，现有行人轨迹预测方法通常将行人视为点实体，忽略了每个行人实际占用的物理空间，导致不现实的碰撞预测。

Method: 提出了一种新的动态占用空间损失函数，结合平均位移误差和基于场景密度及个体空间占用的碰撞惩罚，指导Social LSTM学习避免现实碰撞。

Result: 在五个数据集上的实验表明，该模型将碰撞率降低高达31%，平均位移误差和最终位移误差分别平均降低5%和6%，在大多数测试集上优于多个最先进模型。

Conclusion: 所提出的动态占用空间损失函数能有效提高行人轨迹预测的准确性并显著减少碰撞率，在不同人群密度条件下均表现优异。
...
TL;DR: 本文提出将汽车摄像头污垢检测视为语义分割问题，比较了流行分割方法与图块级分类方法的性能优势，并修正了Woodscape数据集中的数据泄漏和标注不精确问题。
...
Method: 将污垢检测视为语义分割问题，比较流行分割方法与图块级分类方法，创建新的数据子集解决原始数据集的数据泄漏和标注不精确问题。

Result: 分割方法在性能上优于图块级分类方法，新创建的数据子集虽然更小，但能为分割方法提供足够信息，在更短时间内达到可比结果。

Conclusion: 语义分割方法在汽车摄像头污垢检测中表现优越，修正后的数据集能够有效支持模型训练并提高效率。
...
TL;DR: 评估8个医学和通用领域基础模型在胸部X光分析中的表现，发现医学领域预训练具有显著优势，但特征有效性高度依赖任务类型，对于复杂病理分割任务仍需大量微调。
...
Result: 医学基础模型在线性探测中表现优于通用模型，但特征有效性高度任务依赖；对于复杂病理分割，所有模型都需要大量微调；发现模型使用混淆捷径进行分类；图像仅模型和标签监督模型表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 开发可靠的虹膜识别和呈现攻击检测方法需要多样化的数据集，能够捕捉虹膜特征的真实变化和各种异常情况。由于虹膜图像纹理丰富且空间频率范围广，合成相同身份但控制特定属性的虹膜图像具有挑战性。

Method: 通过在生成模型的潜在空间中遍历，寻找代表相同身份样本但具有所需操纵属性的潜在代码。潜在空间遍历由特定几何、纹理或质量相关虹膜图像特征的梯度引导，同时保持图像身份不变。可以使用预训练GAN模型生成的随机图像或真实世界虹膜图像，通过GAN反演将图像投影到潜在空间。

Result: 该方法能够合成相同身份但具有不同属性的虹膜图像，如改变清晰度、瞳孔大小、虹膜大小或瞳孔-虹膜比例等特征。

Conclusion: 提出的方法能够有效增强虹膜图像数据集，为虹膜识别和呈现攻击检测提供更丰富的训练数据，且易于扩展到任何可制定可微分损失项的属性操纵。
...
TL;DR: STORM是一个无需人工标注的实时6D姿态估计系统，通过结合视觉语言理解和自监督特征匹配的三阶段流程，解决了传统方法依赖首帧标注和面对遮挡/快速运动时性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 现有6D姿态估计方法依赖首帧人工标注分割掩码，这既费时又导致在遮挡或快速运动时性能下降。需要开发无需标注的鲁棒实时系统。

Method: 采用三阶段流程：上下文对象描述指导定位、自交叉注意力机制识别候选区域、分割模型生成精确掩码进行姿态估计。还包含自动重注册机制，通过特征相似性监测检测跟踪失败并从严重遮挡或快速运动中恢复。
...
TL;DR: PANDA是一个针对无示例持续学习的补丁和分布感知增强框架，通过CLIP编码器识别代表性区域并将其移植到频繁类别样本中，同时利用自适应平衡策略缓解任务间不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据流存在双重不平衡问题：数据集级分布与任务内极端或反向偏斜，导致任务内和任务间差异，阻碍有效学习和泛化。

Method: 提出PANDA框架，使用CLIP编码器识别低频率类别的代表性区域并移植到频繁类别样本中，同时采用自适应平衡策略利用先前任务分布来平滑任务间不平衡。

Result: 广泛的实验和消融研究表明PANDA能够与现有的基于预训练模型的持续学习方法协同工作，提高准确性并减少灾难性遗忘。

Conclusion: PANDA框架有效解决了无示例持续学习中的双重不平衡问题，提升了现有方法的性能并减轻了灾难性遗忘。
...
TL;DR: STS是一种轻量级的测试时适应框架，通过在文本嵌入的谱子空间中学习少量参数来引导潜在表示，无需反向传播或修改冻结编码器，显著提升了推理速度和内存效率。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在测试时域偏移下性能会下降，而传统的测试时适应方法需要反向传播或修改模型组件，计算开销大。
...
<details>
  <summary>Details</summary>
Motivation: 3DGS在新型视图合成中取得了最先进的光照真实效果，但在人类场景动画和交互方面仍未被充分探索。现有动画管道使用网格或点云作为底层3D表示，存在局限性。

Method: 使用3DGS作为3D表示，将渲染与运动合成解耦。提出高斯对齐运动模块，利用基于不透明度的线索和投影高斯结构来指导人体放置和姿态对齐，无需显式场景几何。进一步提出人类场景高斯细化优化，确保真实接触和导航。

Result: 在Scannet++和SuperSplat库的场景上评估，以及在稀疏和密集多视图人类捕获重建的虚拟角色上测试。展示了新颖应用，如对编辑的单目RGB视频进行几何一致的自由视点渲染，添加新的动画人类。

Conclusion: 该框架展示了3DGS在单目视频人类动画中的独特优势，实现了高质量的人类场景交互动画，无需配对的人类场景数据。
...
Conclusion: CertMask提供了一种高效且可证明鲁棒的防御机制，显著降低了对抗性补丁攻击的威胁，同时保持了模型的实用性。
...
<details>
  <summary>Details</summary>
Motivation: 点云在AR和机器人等领域广泛应用，但现有方法无法有效分离反照率和阴影，主要面临点云非网格结构和全局光照方向捕获不准确的挑战。

Method: 提出IPCD-Net，通过点级特征聚合处理非网格数据；引入基于投影的亮度分布(PLD)和分层特征细化，通过多视图投影捕获全局光照线索。

Result: 实验结果表明IPCD-Net能减少反照率中的投射阴影，提高阴影中的颜色准确性，并在纹理编辑、重光照和点云配准等应用中表现良好。
...
TL;DR: 本文提出了一种推理阶段的T-DRS策略，通过三步衰减恢复策略解决LVLMs中ROPE导致的长距离依赖建模问题，无需训练即可提升VQA任务性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在使用ROPE时面临长距离依赖建模挑战，ROPE虽然能精确建模token位置，但随着token距离增加会导致注意力衰减，损害模型记忆全局上下文的能力。

Method: 提出推理阶段的三步衰减恢复策略T-DRS：1) SD-DRS通过内容感知残差放大语义重要但距离较远的信号；2) DC-DRS基于位置距离平滑调节权重来净化注意力；3) reRD-DRS巩固剩余的信息性远程依赖以保持全局一致性。
...
### [17] [VISTA: A Vision and Intent-Aware Social Attention Framework for Multi-Agent Trajectory Prediction](https://arxiv.org/abs/2511.10203)
*Stephane Da Silva Martins,Emanuel Aldea,Sylvie Le Hégarat-Mascle*

Main category: cs.CV

TL;DR: VISTA是一个基于递归目标条件变换器的多智能体轨迹预测模型，通过结合长时程意图、过去运动信息和社会交互建模，生成更真实的多智能体未来轨迹。
...
Method: VISTA采用递归目标条件变换器，包含跨注意力融合模块整合长时程意图与过去运动、社会令牌注意力机制进行灵活交互建模，以及成对注意力图提供可解释的社会影响模式。
...
TL;DR: 提出SAM-DAQ方法，通过深度引导自适应查询将SAM2适配于RGB-D视频显著目标检测，解决了手动提示依赖、内存消耗高和计算负担大的问题。
...
### [19] [MSGNav: Unleashing the Power of Multi-modal 3D Scene Graph for Zero-Shot Embodied Navigation](https://arxiv.org/abs/2511.10376)
*Xun Huang,Shijia Zhao,Yunxiang Wang,Xin Lu,Wanfa Zhang,Rongsheng Qu,Weixin Li,Yunhong Wang,Chenglu Wen*

Main category: cs.CV

TL;DR: 提出M3DSG多模态3D场景图和MSGNav零样本导航系统，解决现有方法视觉信息丢失、构建成本高和词汇受限的问题，在GOAT-Bench和HM3D-OVON数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有零样本导航方法构建显式3D场景图时压缩视觉观测为文本关系导致的构建成本高、视觉证据丢失不可逆和词汇受限的问题。
...
Conclusion: M3DSG和MSGNav有效解决了零样本导航中的视觉信息保留和开放词汇泛化问题，并专门解决了最后一英里问题。
...
### [20] [SemanticVLA: Semantic-Aligned Sparsification and Enhancement for Efficient Robotic Manipulation](https://arxiv.org/abs/2511.10518)
...
<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在机器人操作中存在两个关键限制：1）感知冗余，处理不相关视觉输入效率低下；2）指令-视觉对齐浅层，阻碍动作的语义基础。这些限制阻碍了实际部署。

Method: 提出三个核心组件：1）语义引导的双视觉剪枝器（SD-Pruner），包括指令驱动剪枝器和空间聚合剪枝器；2）语义互补的分层融合器（SH-Fuser），融合密集补丁和稀疏标记；3）语义条件动作耦合器（SA-Coupler），替代传统的观察到自由度方法。
...
TL;DR: 本文提出HCC-3D方法，通过分层补偿压缩技术有效减少3D-VLMs中3D token的计算开销，在保持关键信息完整性的同时实现约98%的压缩率，并在效率和性能上均取得显著提升。
...
Result: 实验表明HCC-3D相比之前的3D-VLMs实现了约98%的极端压缩率，同时达到了新的最先进性能，在效率和性能上都有显著改进。

Conclusion: HCC-3D通过分层补偿压缩机制有效解决了3D-VLMs的计算瓶颈问题，在保持信息完整性的同时大幅提升效率，为3D多模态理解提供了高效解决方案。
...
TL;DR: 本文提出了一种用于医学图像自动描述的Swin-BART编码器-解码器系统，通过轻量级区域注意力模块增强诊断关键区域，在ROCO数据集上实现了最先进的语义保真度。


<details>
  <summary>Details</summary>
Motivation: 自动化医学图像描述可以将复杂的放射学图像转换为诊断叙述，支持报告工作流程，提高诊断效率和准确性。

Method: 采用Swin-BART编码器-解码器架构，加入轻量级区域注意力模块在交叉注意力前放大诊断关键区域，使用束搜索解码策略。

Result: 模型在ROGE（0.603 vs 基线0.356/0.255）和BERTScore（0.807 vs 基线0.645/0.623）指标上显著优于基线，BLEU、CIDEr和METEOR指标具有竞争力，并提供可解释的热力图可视化。

Conclusion: 所提出的设计能够生成准确、临床化的描述和透明的区域归因，支持安全的研究使用，需要人工参与验证。
...
TL;DR: DocWeaver多智能体管道自动生成MosaicDoc双语基准，用于评估视觉语言模型在视觉丰富文档理解任务中的表现，包含72K图像和600K问答对。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要是英语中心、布局简单、任务有限，无法充分评估视觉语言模型在复杂布局和密集文本的视觉丰富文档理解任务中的性能。

Method: 使用大型语言模型的多智能体管道DocWeaver自动生成双语基准MosaicDoc，源自从报纸和杂志提取的多样化复杂布局文档。

Result: 创建了包含72K图像和600K问答对的大规模双语基准，具有多样复杂布局、196个出版商的丰富风格，以及OCR、VQA、阅读顺序和定位等多任务标注。

Conclusion: 在MosaicDoc基准上的广泛评估揭示了现有模型在处理真实世界文档复杂性方面的局限性，并为未来研究指明了方向。
...
<details>
  <summary>Details</summary>
Motivation: 顺序微调方法在类增量学习中面临分布漂移问题，导致先前学习类别与新模型之间的分布不匹配，影响分类器性能。

Method: 提出SLDC方法，包括线性变体（通过正则化最小二乘学习线性算子）和弱非线性变体（使用可学习的弱非线性映射），并结合知识蒸馏来减少表示漂移。

Result: 在标准CIL基准测试中，SLDC显著提升了SeqFT性能，结合知识蒸馏后，SeqFT在所有评估数据集上达到与联合训练相当的性能。

Conclusion: SLDC通过补偿分布漂移和表示漂移，有效解决了顺序微调中的分布不匹配问题，为类增量学习提供了有效的解决方案。
...
TL;DR: 本文提出了一种针对行人重识别（ReID）任务的去偏双重不变防御框架，通过数据平衡和双对抗自元防御来解决模型偏差和复合泛化需求两大挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的对抗防御策略主要针对分类任务，而针对度量学习任务如行人重识别的防御研究相对较少。现有防御方法未能解决对抗鲁棒ReID的独特挑战，包括模型偏差和复合泛化需求。

Method: 提出去偏双重不变防御框架，包含两个阶段：数据平衡阶段使用基于扩散模型的数据重采样策略缓解模型偏差；双对抗自元防御阶段引入新型度量对抗训练方法，结合最远负样本扩展软化和对抗增强的自元机制。
...
TL;DR: TSPE-GS通过均匀采样透射率来建模像素级的多模态不透明度和深度分布，解决了3D高斯泼溅在半透明表面重建中的问题，能够分别重建外部和内部表面。
...
Method: 提出TSPE-GS方法，通过均匀采样透射率来建模像素级的多模态不透明度和深度分布，取代先前的单峰假设，解决跨表面深度模糊问题。通过逐步融合截断符号距离函数，在统一框架内分别重建外部和内部表面。
...
TL;DR: 本文提出了一种自适应融合框架，将CLIP图像特征的余弦相似度与特征幅度相结合，用于无参考图像质量评估，无需特定任务训练即可在多个基准数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的NR-IQA方法仅使用图像嵌入与文本提示的余弦相似度，忽略了CLIP图像特征幅度与感知质量之间的强相关性，这是一个关键但未被充分探索的线索。

Method: 提出自适应融合框架：提取CLIP图像特征的绝对值并应用Box-Cox变换进行统计归一化，生成语义归一化的辅助线索；设计置信度引导的融合方案，根据相对强度自适应加权余弦相似度和幅度线索。

Result: 在多个基准IQA数据集上的广泛实验表明，该方法始终优于标准基于CLIP的IQA和最先进的基线方法。

Conclusion: 通过将余弦相似度与幅度感知质量线索互补结合，提出的自适应融合框架显著提升了无参考图像质量评估性能，无需任何特定任务训练。
...
TL;DR: 提出了一种利用视觉语言模型自动生成伪标签来训练高效实时目标检测器的新流程，通过每对象协同教学策略减少VLM生成标签中的噪声，在自动驾驶场景中显著减少对人工标注的依赖。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在零样本目标检测方面表现优异，但检测延迟和幻觉预测问题使其无法直接部署。自动驾驶领域人工标注成本极高，需要一种自动化的训练方法。

Method: 采用每对象协同教学训练策略，两个YOLO模型协作学习，基于彼此的对象级损失值过滤不可靠的边界框，而不是过滤整个图像。

Result: 在KITTI数据集上，方法比基线YOLOv5m模型显著提升mAP@0.5（31.12%到46.61%），保持实时检测延迟。补充10%真实标签后达到57.97% mAP@0.5，在ACDC和BDD100k数据集上也观察到类似性能提升。
...
TL;DR: 本文提出了EquS和EquS+方法，通过双采样轨迹施加等变信息，并结合时间步感知调度来提升基于扩散模型的图像恢复性能。


<details>
  <summary>Details</summary>
Motivation: 现有问题无关的扩散模型图像恢复方法在充分利用扩散先验方面存在挑战，导致性能不佳。

Method: 提出EquS方法，通过双采样轨迹施加等变信息；进一步提出时间步感知调度(TAS)和EquS+，优先确定性步骤以增强确定性和采样效率。

Result: 在基准测试上的大量实验表明，该方法与先前的问题无关DMIR方法兼容，并能显著提升性能而不增加计算成本。

Conclusion: 该方法有效解决了现有问题无关DMIR方法的局限性，显著提升了图像恢复性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有对比学习微调方法会扭曲嵌入的几何结构，这限制了视觉语言模型在OOD和零样本设置中的泛化能力。本文旨在在ID数据上稳健微调的同时不损害泛化性能。

Method: 提出差异向量均衡化(DiVE)方法，通过平均向量损失(AVL)和成对向量损失(PVL)来约束差异向量，分别实现全局和局部几何结构保持。

Result: 实验表明DiVE能有效保持几何结构，在ID、OOD和零样本指标上均取得强劲结果。

Conclusion: DiVE通过保持嵌入几何结构，成功解决了视觉语言模型微调中泛化能力下降的问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的场景文本编辑方法存在三个主要问题：缺乏对低资源语言的支持、合成数据与真实数据之间的领域差距、以及缺乏合适的文本样式保持评估指标。

Method: 提出STELLAR模型，使用语言自适应字形编码器实现可靠的多语言编辑，采用多阶段训练策略（先在合成数据上预训练，然后在真实图像上微调），并构建了新的数据集STIPLAR。

Result: 实验结果表明STELLAR在视觉一致性和识别准确率方面优于最先进模型，在TAS指标上比基线平均提升2.2%。

Conclusion: STELLAR通过创新的模型架构、训练策略和评估指标，有效解决了场景文本编辑中的关键挑战，特别是在低资源语言和真实世界数据场景下表现出色。
...
TL;DR: DBGroup是一个两阶段弱监督3D实例分割框架，利用场景级注释作为更高效和可扩展的替代方案，通过双分支点分组和自训练方法实现竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督3D实例分割方法依赖点级或边界框注释，仍存在标注劳动密集、复杂度高和依赖专家标注者的问题，需要更高效的弱监督方法。

Method: 提出两阶段框架：第一阶段使用双分支点分组模块生成伪标签，结合粒度感知实例合并和语义选择传播策略；第二阶段进行多轮自训练，并引入实例掩码过滤策略处理伪标签不一致性。

Result: 实验表明DBGroup在稀疏点级监督3D实例分割方法中达到竞争性性能，同时超越了最先进的场景级监督3D语义分割方法。

Conclusion: DBGroup证明了场景级注释作为弱监督3D实例分割的有效性，提供了一种更高效和可扩展的解决方案。
...
TL;DR: LampQ是一种针对Vision Transformer的层间混合精度量化方法，通过类型感知的Fisher度量、整数线性规划和迭代更新，解决了现有方法在粒度、度量尺度不匹配和量化感知位分配方面的限制。


<details>
  <summary>Details</summary>
Motivation: 现有ViT量化方法采用统一精度，忽略了不同组件对量化的敏感度差异。基于度量的混合精度量化方法存在三个主要问题：粒度粗、跨组件类型度量尺度不匹配、量化不感知的位分配。

Method: 提出LampQ方法：1）层间量化实现细粒度控制和高效加速；2）引入类型感知的Fisher度量来测量敏感度；3）通过整数线性规划最优分配位宽；4）迭代更新位宽分配。

Result: 大量实验表明，LampQ在图像分类、目标检测和零样本量化等多种任务预训练的ViT量化中，提供了最先进的性能。

Conclusion: LampQ通过层间混合精度量化方法，有效克服了现有ViT量化方法的局限性，在各种视觉任务中实现了优异的量化性能。
...
TL;DR: 本文提出了细粒度3D具身推理任务，要求智能体基于任务指令预测3D场景中每个可操作元素的空间位置、运动类型和运动轴。作者开发了AffordBot框架，将多模态大语言模型与链式思维推理相结合，在SceneFun3D数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常在对象级别操作或分离处理细粒度的可供性推理，缺乏连贯的指令驱动接地和推理。需要开发能够理解在物理环境中与可操作元素交互的完整框架。

Method: 提出AffordBot框架，集成多模态大语言模型和定制化的链式思维推理范式。通过渲染场景的环绕视图图像并将3D元素候选投影到这些视图中，构建与场景几何对齐的丰富视觉表示。推理流程包括主动感知阶段和逐步推理阶段。

Result: 在SceneFun3D数据集上评估，AffordBot实现了最先进的性能，展示了强大的泛化能力和物理接地推理能力，仅使用3D点云输入和多模态大语言模型。

Conclusion: 该工作为细粒度3D具身推理任务提供了有效的解决方案，证明了多模态大语言模型与链式思维推理结合在物理环境理解中的潜力。
...
TL;DR: Anomagic是一种零样本异常生成方法，无需异常示例即可生成语义一致的异常。通过跨模态提示编码整合视觉和文本线索，利用上下文信息引导基于修复的生成流程，并通过对比细化策略增强异常与掩码的对齐，提升下游异常检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有的异常生成方法通常需要异常示例，限制了其应用范围。作者旨在开发一种无需异常示例的零样本异常生成方法，能够生成语义一致的异常，并提升下游异常检测任务的性能。

Method: 提出Anomagic方法：1）通过跨模态提示编码统一视觉和文本线索；2）利用基于修复的生成流程；3）采用对比细化策略确保异常与掩码的精确对齐。同时构建AnomVerse数据集，包含12,987个异常-掩码-描述三元组。

Result: 实验表明，Anomagic在AnomVerse上训练后，能够生成比现有方法更真实、更多样化的异常，显著提升了下游异常检测任务的性能。该方法还能通过用户定义提示为任何正常类别图像生成异常。

Conclusion: Anomagic建立了一个通用的异常生成基础模型，无需异常示例即可生成语义一致的异常，显著提升了异常检测性能，具有广泛的适用性。
...
TL;DR: DGFusion是一种基于双引导范式的多模态3D目标检测方法，通过难度感知实例配对匹配器和双引导模块，有效解决远距离、小目标和遮挡目标等困难实例的检测问题，在nuScenes数据集上取得了优于基线方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态3D目标检测方法通常采用单一引导范式，无法充分考虑不同模态间困难实例信息密度的差异，导致远距离、小目标和遮挡目标的检测性能不佳，直接影响自动驾驶系统的安全性。

Method: 提出DGFusion方法，基于双引导范式，继承点引导图像范式的优势并整合图像引导点范式。核心组件包括难度感知实例配对匹配器(DIPM)进行基于难度的实例级特征匹配，以及双引导模块利用两种配对类型的优势实现有效的多模态特征融合。

Result: 在nuScenes数据集上，DGFusion相比基线方法分别提升了+1.0% mAP、+0.8% NDS和+1.3%平均召回率。在自车距离、目标尺寸、可见性和小规模训练场景下，对困难实例检测均表现出稳定的鲁棒性提升。

Conclusion: DGFusion通过双引导范式有效解决了困难实例检测的挑战，在多个指标和场景下均表现出优越性能，为自动驾驶感知系统的安全性提供了重要保障。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在表示任意拓扑结构(如开放表面和复杂内部结构)时面临挑战，基于SDF的方法需要昂贵的水密预处理且难以处理非流形几何，点云表示存在采样伪影和表面不连续问题。

Method: 使用UDF作为更鲁棒和计算高效的表示，提出局部到全局架构，将UDF划分为均匀子体积(UBlocks)，结合3D卷积捕获局部细节和稀疏变换器确保全局一致性，采用Pad-Average策略保证子体积边界平滑过渡。

Result: 实现了最先进的3D重建精度和生成质量，在表面平滑度和几何灵活性方面表现优异，支持高达2048^3的超高分辨率。

Conclusion: 该框架成功克服了现有3D表示方法的限制，为复杂拓扑结构的高保真3D内容生成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 可见光和红外模态由于不同传感器捕获，在复杂场景中存在信息不平衡问题，导致跨模态融合不充分，检测性能下降。现有方法大多在空间域使用Transformer，忽略了频率域Transformer挖掘互补信息的优势。

Method: 提出频率域融合Transformer(FreDFT)，包含多模态频率域注意力(MFDA)挖掘模态间互补信息，频率域前馈层(FDFFL)通过混合尺度频率特征融合增强多模态特征，跨模态全局建模模块(CGMM)消除模态信息不平衡，局部特征增强模块(LFEM)强化多模态局部特征表示。
...
Method: 收集包含128k样本的全面数据集，评估四个美学维度；提出HCM-GRPO方法，在GRPO框架中引入硬案例挖掘策略和动态比例准确度奖励。

Result: 实验表明，即使是GPT4o等最先进的闭源模型在图像美学推理上表现类似随机猜测，而HCM-GRPO方法能够用更小的模型超越开源和闭源大模型的得分。
...
TL;DR: 该论文提出了AV-ConfuseBench基准测试，发现多模态大语言模型在音频-视觉混淆场景中存在视觉主导推理问题，并提出了基于强化学习的协作多MLLM框架RL-CoMM来改善这一问题。
...
Method: 提出了RL-CoMM框架，包含两个阶段：1）引入大型音频语言模型作为参考模型生成纯音频推理，设计逐步推理奖励函数；2）引入答案中心置信度优化来减少异构推理差异的不确定性。

Result: 在音频-视觉问答和音频-视觉幻觉任务上的实验表明，RL-CoMM在有限训练数据下比基线模型准确率提高了10-30%。

Conclusion: RL-CoMM框架有效解决了MLLMs在音频-视觉混淆场景中的视觉主导推理问题，显著提升了模型的音频-视觉推理能力。
...
TL;DR: 本文提出了GaussMedAct框架，通过多元高斯编码和混合空间编码来改进医疗动作评估，在CPREval-6k数据集上达到92.1%的Top-1准确率，比ST-GCN基线提升5.9%且仅需10%计算量。


<details>
  <summary>Details</summary>
Motivation: 医疗视觉中的细粒度动作评估面临数据集不完整、精度要求严格以及快速动作时空动态建模不足等独特挑战。

Method: 提出GaussMedAct框架，包括多元高斯表示将关节运动投影到时标多维空间，分解为自适应3D高斯token；混合空间编码采用笛卡尔和向量双流策略利用骨骼信息。

Result: 在CPREval-6k数据集上达到92.1% Top-1准确率，比ST-GCN基线提升5.9%，仅需10% FLOPs，跨数据集实验证实了方法的鲁棒性。

Conclusion: GaussMedAct框架通过自适应时空表示学习有效提升了医疗动作分析的性能，在准确率和计算效率方面均表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 传统高光谱图像分类中，模型往往错误地将高预测分数等同于正确性，缺乏不确定性意识，导致在稀疏标注或类别不平衡情况下产生确认偏差和过拟合。

Method: CABIN采用三阶段方法：1）感知阶段通过认知不确定性估计识别模糊区域；2）行动阶段使用不确定性引导的双重采样策略，选择不确定样本进行探索，固定置信样本作为稳定伪标签；3）校正阶段引入细粒度动态分配策略，将伪标签数据分为可靠、模糊和噪声子集，并应用定制损失函数。
...
TL;DR: VLF-MSC是一种统一的多模态语义通信系统，通过传输单一的视觉-语言特征表示，在接收端同时支持图像和文本生成，提高了频谱效率。


<details>
  <summary>Details</summary>
Motivation: 现有的语义通信技术通常单独处理每种模态，需要特定的模态流或重传，导致频谱效率低下。VLF-MSC旨在通过统一的视觉-语言表示解决这一问题。

Method: 使用预训练的视觉语言模型将源图像编码为视觉-语言语义特征（VLF），通过无线信道传输。在接收端，基于解码器的语言模型和基于扩散的图像生成器都基于VLF生成描述性文本和语义对齐的图像。
...
TL;DR: GlobalDiff是一个基于扩散模型的框架，首次直接在全局关节旋转空间操作，通过多级约束方案解决累积误差问题，在语音驱动运动生成任务中相比现有SOTA方法提升46.0%性能。


<details>
  <summary>Details</summary>
Motivation: 现有语音驱动运动生成方法通常在局部关节旋转空间操作，由于骨架结构的层次性定义导致累积误差，表现为末端效应器的不稳定和不合理运动。

Method: 提出GlobalDiff框架，在全局关节旋转空间直接操作，解耦各关节预测的上游依赖关系；引入多级约束方案：关节结构约束使用虚拟锚点捕获细粒度方向，骨架结构约束保持骨骼角度一致性，时间结构约束使用多尺度变分编码器对齐真实时间模式。

Result: 在标准语音驱动运动基准测试上的广泛评估显示，GlobalDiff生成平滑准确的运动，在多个说话者身份下相比当前SOTA方法性能提升46.0%。

Conclusion: GlobalDiff通过全局旋转空间操作和多级约束方案，有效解决了层次误差累积问题，显著提升了语音驱动运动生成的质量和稳定性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉token剪枝方法主要关注"选择什么"，忽略了"看哪里"的空间分配问题，导致效率低下、位置偏差和冗余token保留。受人类视觉系统启发，需要先确定关注区域再精细选择。
...
TL;DR: SUGAR是一种将大型语言模型与人体骨骼数据结合进行动作识别和描述的新范式，通过视觉-运动知识学习骨骼表示，无需修改LLM预训练权重即可实现动作分类和描述。
...
Method: 使用现成的大规模视频模型作为知识库生成视觉和运动信息，通过先验知识监督骨骼学习得到离散表示，利用未修改的LLM理解这些表示并生成动作目标和描述，提出Temporal Query Projection模块处理长序列骨骼信号。

Result: 在多个骨骼动作分类基准测试中验证了SUGAR的有效性，零样本场景实验显示SUGAR比基于线性方法更具通用性。

Conclusion: SUGAR成功地将LLMs与骨骼数据结合，实现了有效的动作识别和描述，在零样本场景下表现出更好的泛化能力。
...
TL;DR: 提出RobIA框架，用于立体深度估计中的持续测试时适应，通过动态路由和鲁棒伪监督解决域偏移问题


<details>
  <summary>Details</summary>
Motivation: 解决真实环境中立体深度估计面临的动态域偏移、稀疏监督和密集标签获取成本高的问题，现有TTA方法受限于静态目标域假设和输入不变适应策略

Method: 集成Attend-and-Excite Mixture-of-Experts模块动态路由输入到冻结专家，以及Robust AdaptBN Teacher模型提供密集伪监督补充稀疏手工标签
...
TL;DR: 本文提出了一种名为CACMI的显式时空语义建模框架，通过跨模态帧聚合和上下文感知特征增强，解决了密集视频描述中时间连贯性和语义完整性的问题，在ActivityNet Captions和YouCook2数据集上达到了最先进性能。
...
Conclusion: CACMI框架通过显式建模视频的潜在时间特性和文本语料库的语言语义，有效提升了密集视频描述的性能。
...
TL;DR: 这篇论文调查了当前文本到图像模型在处理逻辑组合方面的根本缺陷，特别是在否定、计数和空间关系三个核心原语上的表现崩溃。研究发现模型在单个原语上准确，但在组合时性能急剧下降，暴露了严重的干扰问题。


<details>
  <summary>Details</summary>
Motivation: 研究动机是揭示当今领先文本到图像模型架构中的基本缺陷：无法处理逻辑组合。这种缺陷导致模型在组合多个逻辑原语时出现性能崩溃。

Method: 研究方法包括分析三个核心逻辑原语（否定、计数和空间关系）的表现，调查训练数据中明确否定的缺失情况，评估连续注意力架构对离散逻辑的不适用性，以及分析评估指标偏向视觉合理性而非约束满足的问题。

Result: 研究结果显示：1）训练数据中几乎完全没有明确的否定；2）连续注意力架构本质上不适合处理离散逻辑；3）评估指标奖励视觉合理性而非约束满足。当前解决方案和简单扩展无法弥合这一差距。

Conclusion: 结论指出，实现真正的组合性需要在表示和推理方面进行根本性进步，而不是对现有架构进行渐进式调整。
...
TL;DR: 提出split-layer方法，通过将MLP每层分成多个并行分支并用Hadamard乘积整合输出，构建高维多项式空间，显著提升隐式神经表示(INR)的表征能力，同时避免计算开销的二次增长。


<details>
  <summary>Details</summary>
Motivation: 传统MLP架构中低维特征空间限制了隐式神经表示的表征能力，而拓宽MLP会导致计算和内存成本的二次增长，需要一种更高效的方法来提升表征能力。

Method: 提出split-layer重构MLP构建方式，将每层分成多个并行分支，通过Hadamard乘积整合分支输出，构建高维多项式空间。

Result: 实验表明split-layer显著提升INR性能，在2D图像拟合、2D CT重建、3D形状表示和5D新视角合成等多个任务中超越现有方法。

Conclusion: split-layer通过高效扩展特征空间维度，显著增强了INR的表征能力，为解决传统MLP架构的局限性提供了有效方案。
...
TL;DR: 提出一种双机制协同优化框架，通过结构公平解耦和全局分布对齐来提升深度伪造检测模型的公平性，同时保持检测精度。
...
TL;DR: 本文提出了一种基于生成视角的生成增强对齐方法（GEA），用于解决文本到图像行人检索中的跨模态对齐问题，通过扩散生成图像作为中间语义表示来弥合文本与视觉模式之间的差距。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像行人检索方法存在文本查询无法准确全面反映图像内容的问题，导致跨模态对齐不佳和对有限数据集的过拟合。文本和图像之间的固有模态差距进一步加剧了这些问题。
...
Conclusion: GEA方法通过生成视角有效解决了文本到图像行人检索中的跨模态对齐挑战，实验结果证明了该方法的优越性能。
...
TL;DR: 评估NASA和IBM的Prithvi-EO-2.0地理空间基础模型在小沙岛海岸线描绘任务中的表现，使用少量训练图像即可获得高性能。


<details>
  <summary>Details</summary>
Motivation: 探索地理空间基础模型在数据匮乏地区海岸监测中的潜力，特别是对小沙岛海岸线描绘任务。
...
Conclusion: Prithvi模型在海岸线描绘任务中表现出色，证明了此类模型在数据匮乏地区支持海岸监测的巨大潜力。
...
TL;DR: 提出单阶段训练框架，通过结合单模态和跨模态特征的下一帧预测来增强泛化能力，并引入窗口级注意力机制检测预测帧与实际帧之间的差异，以准确分类全篡改视频并定位部分伪造样本中的深度伪造片段。


<details>
  <summary>Details</summary>
Motivation: 现有多模态深度伪造检测方法主要关注音频-视觉不一致性，可能忽略单模态伪影，且需要真实样本预训练。本文旨在解决这些限制，提升模型对保持音频-视觉对齐的篡改的检测能力。

Method: 单阶段训练框架，结合单模态和跨模态特征的下一帧预测，引入窗口级注意力机制捕捉预测帧与实际帧之间的差异，检测每帧周围的局部伪影。

Result: 在多个基准数据集上评估，模型表现出强大的泛化能力和精确的时间定位性能。

Conclusion: 所提出的单阶段训练框架通过下一帧预测和窗口级注意力机制，有效提升了深度伪造检测的泛化能力和定位精度。
...
TL;DR: 本文提出TubeRMC框架，通过管状条件重建与互约束来解决弱监督时空视频定位中的目标识别和跟踪不一致问题。
...
Method: 提出TubeRMC框架，使用预训练视觉定位模型生成文本条件候选管，通过管状条件重建和时空约束进行细化，设计了时间、空间和时空三种重建策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有AQA方法主要基于整个视频提取特征预测分数，可解释性和可靠性有限，且缺乏细粒度评分注释。

Method: 提出JudgeMind方法：1）将动作视频分段评分；2）阶段感知特征增强融合模块；3）基于知识的等级感知解码器，融入扣分项作为先验知识。

Result: 实验结果表明该方法达到了最先进的性能。

Conclusion: JudgeMind方法通过模拟裁判评分思维，显著提升了AQA的性能和可靠性。
...
Method: 提出三阶段对齐框架：1) 指令微调建立基本情绪推理能力；2) 引入以情绪和AU标签为奖励信号的强化训练；3) 设计数据合成管道迭代扩展训练数据集。

Result: 在八个标准基准测试中，Facial-R1实现了面部情绪分析的最先进性能，具有强大的泛化能力和稳健的可解释性。

Conclusion: Facial-R1框架有效解决了面部情绪分析中的关键挑战，通过最小监督实现了推理与识别的对齐，并引入了包含17,737个训练样本的FEA-20K基准数据集。
...
TL;DR: H3Former是一个新颖的token-to-region框架，利用高阶语义关系聚合局部细粒度表示，通过语义感知聚合模块和双曲层次对比损失来解决细粒度视觉分类中的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的细粒度视觉分类方法通常依赖特征选择机制或区域提议策略来定位判别性区域，但这些方法往往无法全面捕捉判别性线索，同时引入大量类别无关冗余。
...
Conclusion: H3Former通过高阶语义关系和结构化区域级建模，有效提升了细粒度视觉分类的性能，解决了现有方法在捕捉判别性线索和减少冗余方面的局限性。
...
TL;DR: RUDDER是一个低开销框架，通过提取自注意力层的残差更新作为视觉证据向量，并使用贝叶斯自适应门进行token级注入，有效减少大型视觉语言模型的对象幻觉问题，同时保持计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有缓解对象幻觉的推理时干预方法存在计算开销大的问题，通常需要额外的前向传播，限制了在实际延迟敏感场景中的实用性。

Method: 提出RUDDER框架，包含两个关键创新：1) 从单次标准前向传播中提取自注意力层残差更新的上下文激活残差方向向量；2) 贝叶斯启发的自适应门进行token级注入，根据模型偏离视觉上下文程度调整校正信号强度。

Result: 在POPE和CHAIR等关键幻觉基准测试上的广泛实验表明，RUDDER实现了与最先进方法相当的性能，同时引入可忽略的计算延迟。
...
TL;DR: LLaViT是一种新颖的视觉语言模型架构，通过让LLM同时作为视觉编码器来解决LLaVA架构中视觉特征整合不足的问题，在多个基准测试中显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 解决LLaVA架构中文本和视觉模态不匹配导致的视觉特征整合效果不佳的问题，从让LLM同时作为视觉编码器的新视角出发。
...
TL;DR: 提出了一种结合景深监督和多视图一致性监督的新框架，用于改进3D高斯泼溅技术，解决极端深度变化场景中的深度估计问题。


<details>
  <summary>Details</summary>
Motivation: 在深度变化极端的场景中，现有方法无法同时处理远场区域深度估计不准确和近场区域结构退化的问题。

Method: 包含两个核心组件：1）景深监督使用尺度恢复的单目深度估计器生成深度先验，通过散焦卷积合成物理准确的散焦图像，并通过景深损失增强几何一致性；2）多视图一致性监督使用基于LoFTR的半稠密特征匹配来最小化跨视图几何误差，并通过可靠匹配点的最小二乘优化强制深度一致性。
...
TL;DR: 本文提出了一种结合von Mises-Fisher分布和傅里叶变换的CLIP引导框架，用于增强单域泛化目标检测，通过建模方向特征和频率域扰动来提高跨域鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有单域泛化目标检测方法（如基于CLIP的语义增强）往往忽略了特征分布结构和频率域特性，而这些对于模型鲁棒性至关重要。

Method: 使用von Mises-Fisher分布建模对象表示的方向特征以捕获域不变语义结构；引入基于傅里叶的增强策略，扰动幅度和相位分量来模拟频率域中的域偏移。

Result: 在多样化天气驾驶基准测试上的广泛实验表明，该方法优于现有最先进方法。

Conclusion: 所提框架不仅保留了CLIP的语义对齐优势，还丰富了跨域特征多样性和结构一致性，显著提升了单域泛化目标检测性能。
...
TL;DR: DermAI是一个基于智能手机的轻量级应用，能够在常规咨询中实时捕获、标注和分类皮肤病变，通过设备端质量检查和本地模型适应解决AI皮肤病学中的偏见数据集、图像质量变化和验证有限等问题。


<details>
  <summary>Details</summary>
Motivation: AI皮肤病学的采用受到偏见数据集、图像质量变化和有限验证的限制，需要开发能够适应多样化临床环境且易于使用的工具。

Method: 开发DermAI智能手机应用，包含设备端质量检查、本地模型适应功能，并构建包含广泛肤色、种族和来源设备的临床数据集。

Result: 在初步实验中，基于公共数据集训练的模型无法泛化到DermAI样本，而使用本地数据进行微调显著提高了性能。
...
TL;DR: SHRUG-FM是一个可靠性感知预测框架，通过整合输入空间和嵌入空间的OOD检测以及任务特定预测不确定性，提高地球观测地理空间基础模型在预训练数据不足环境中的可靠性。


<details>
  <summary>Details</summary>
Motivation: 地理空间基础模型在预训练数据不足的环境中表现不可靠，需要开发可靠性感知预测方法来确保在气候敏感应用中的安全部署。

Method: 整合三种互补信号：输入空间的OOD检测、嵌入空间的OOD检测和任务特定预测不确定性，应用于烧伤疤痕分割任务。

Result: OOD分数与特定环境条件下的较低性能相关，基于不确定性的标志有助于排除许多性能较差的预测，失败集中在低海拔区域和大型河流区域等特定地理区域。
...
TL;DR: 本文分析了现有对抗DreamBooth隐私保护方法的局限性，发现这些方法存在明显的人工痕迹且易被简单过滤器移除，提出了AntiDB_Purify评估框架来系统评估防御方法在真实净化威胁下的有效性。


<details>
  <summary>Details</summary>
Motivation: 个性化AI应用如DreamBooth虽然能根据用户图像生成定制内容，但也带来严重的隐私风险，特别是面部身份泄露。现有防御机制如Anti-DreamBooth试图通过注入对抗性扰动来防止成功个性化，但这些方法存在两个关键但被忽视的局限性。

Method: 提出了AntiDB_Purify评估框架，系统评估现有防御方法在真实净化威胁下的有效性，包括传统图像过滤器和对抗性净化方法。

Result: 结果显示，当前所有防御方法在净化威胁下都无法保持其保护效果，表明现有防御只提供了虚假的安全感。

Conclusion: 这些发现强调了当前防御方法的不足，迫切需要开发更不可察觉和鲁棒的保护机制来保护个性化生成中的用户身份安全。
...
TL;DR: 提出SAMIRO方法，通过空间注意力互信息正则化和预训练模型作为Oracle，提升车道检测性能，保持领域无关的空间信息，并在多个基准测试中验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 解决现实环境中背景杂乱、光照变化和遮挡等挑战对车道检测的影响，减少数据收集和标注的成本，利用上下文和全局信息提升检测效果。
...
Conclusion: SAMIRO是一种有效的车道检测增强方法，能显著提升现有方法的性能，具有广泛适用性。
...
Conclusion: MonkeyOCR v1.5通过统一框架有效解决了复杂文档解析的挑战，在布局理解和内容识别方面都取得了显著改进。
...
TL;DR: 提出了一种结合YOLOMS目标检测和大型语言模型的集成框架，用于风力涡轮机故障的智能分析和诊断，提高故障检测准确性和维护报告的可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有风力涡轮机故障检测方法主要限于视觉识别，缺乏语义解释性，无法支持维护决策制定。
...
Method: 1) GT++：使用标注的解剖标志从3D超声体积中估计标准面部平面的稳健算法；2) 3DFETUS：在3D胎儿超声体积中自动化和标准化面部平面定位的深度学习模型。

Result: 提出的方法在3D超声体积上实现了每平面平均平移误差4.13毫米和平均旋转误差7.93度，优于其他最先进方法。临床评估进一步证实了GT++和3DFETUS的有效性，显示出平面估计精度的统计学显著改进。

Conclusion: GT++和3DFETUS方法能够有效解决胎儿超声面部平面定位的挑战，提供更准确和标准化的平面估计，在临床应用中具有重要价值。
...
TL;DR: 该研究构建了一个用于自动检测啮齿类动物癫痫发作的视频数据集RodEpil，包含10,101个阴性样本和2,952个阳性样本，采用TimeSformer视频分类器实现了97%的F1分数。
...
Method: 构建包含10秒俯视图和侧视图视频片段的标注数据集，采用TimeSformer视频分类器进行五折交叉验证，严格按受试者划分以防止数据泄露。

Result: TimeSformer架构能够区分癫痫发作和正常活动，平均F1分数达到97%。

Conclusion: RodEpil数据集和基准代码已公开发布，为基于视频的癫痫监测研究提供了可靠资源。
...
TL;DR: 提出了一种基于语义分割的组织学信息分块方法HIT，通过提取前列腺癌组织中的腺体作为生物学意义明确的输入块，改进了多实例学习模型的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统数字病理学流程使用网格分块方法忽略了组织结构，引入了无关信息且限制了可解释性。需要一种能够聚焦于生物学意义结构的方法。

Method: 使用语义分割从全切片图像中提取腺体作为输入块，应用于多实例学习和表型分析。在ProMPT队列的137个样本上训练，在ICGC-C和TCGA-PRAD队列的760个WSI中提取了38万个腺体。

Result: 腺体水平Dice得分为0.83±0.17，将MIL模型检测上皮-间质转化和MYC相关基因拷贝数变异的AUC提高了10%，识别出15个腺体簇，其中多个与癌症复发、致癌突变和高Gleason评分相关。
...
<details>
  <summary>Details</summary>
Motivation: 降低研究人员和实践者使用SRGAN进行实验的门槛，为多光谱卫星数据提供易于配置和扩展的超分辨率工具。

Method: 采用模块化设计，通过配置文件暴露生成器、判别器、损失函数和训练计划，支持多种架构、尺度因子和波段设置。

Result: 提供了即用型配置、合理的默认设置以及日志记录、验证和大场景推理的内置钩子。

Conclusion: 通过将基于GAN的超分辨率转变为配置驱动的工作流程，OpenSR-SRGAN促进了可重复性比较和跨多样化地球观测数据集的超分辨率管道部署。
...
TL;DR: 本研究开发了一种全自动方法，通过CT成像生物标志物（特别是胰腺表面分叶度PSL）来筛查2型糖尿病。研究发现糖尿病患者PSL显著更高，多变量模型预测T2DM的AUC达到0.90。
...
Method: 使用四种深度学习模型在584名患者内部数据集上分割胰腺，自动检测PSL，并建立多变量模型预测T2DM。
...
<details>
  <summary>Details</summary>
Motivation: Vision Transformers的计算需求随令牌数量呈二次方增长，紧凑的注意力表示可以指导在注意力计算前早期检测和减少不太重要的令牌。

Method: SPOT利用令牌嵌入、交互和跨层注意力动态来推断令牌重要性，采用计算轻量级的预测器，可插入各种ViT架构，学习跨层的输入特定令牌优先级。

Result: 实证评估显示，与标准ViT相比，SPOT实现了高达40%的效率提升，同时保持甚至提高了准确性。

Conclusion: SPOT提供了一种上下文感知和可解释的相关性检测过程，支持适应不同资源约束的各种性能水平，在不牺牲性能的情况下提高计算效率。
...
TL;DR: 提出Separate-then-Map策略，通过专门的信息映射机制桥接分别定义和优化的模型，解决动态人类与真实环境交互重建中的空间不一致和视觉伪影问题。
...
Method: 采用Separate-then-Map策略，为每个高斯属性使用共享变换函数统一分别建模的组件，避免详尽成对交互同时确保人类与环境的空间和视觉一致性。

Result: 在单目视频数据集上的广泛实验表明，StM在视觉质量和渲染精度上显著优于现有最先进方法，特别是在具有挑战性的人-场景交互边界。

Conclusion: 提出的Separate-then-Map策略通过信息映射机制有效解决了分别建模组件间的空间不一致问题，在动态人类与环境交互重建中取得了优越性能。
...
TL;DR: 提出了一个评估文本到图像模型多样性的框架，通过评估个体概念及其变化因素来系统分析模型多样性。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型在生成质量上有所进步，但输出往往缺乏多样性，生成同质化结果，需要建立稳健的多样性评估方法。

Method: 开发了人类评估模板、精选提示集覆盖多样概念及其变化因素、通过二项检验比较模型的人类标注结果，并比较不同图像嵌入方法用于多样性测量。

Result: 该框架能够对文本到图像模型进行多样性排名，识别模型在特定类别中的困难领域。
...
TL;DR: 本文提出了CoTyle方法，通过数值风格代码实现代码到风格的图像生成，无需冗长文本提示或参考图像，仅凭一个数值代码就能生成新颖且风格一致的图像。


<details>
  <summary>Details</summary>
Motivation: 现有生成方法依赖文本提示、参考图像或参数微调，但存在风格一致性差、创造力有限和风格表示复杂的问题。本文旨在通过数值风格代码解决这些问题。

Method: 首先从图像集合训练离散风格码本提取风格嵌入，作为文本到图像扩散模型的条件。然后训练自回归风格生成器建模风格嵌入分布，合成新颖风格嵌入。推理时数值代码通过风格生成器映射为风格嵌入，指导扩散模型生成对应风格的图像。

Result: 大量实验验证CoTyle能有效将数值代码转化为风格控制器，证明一个风格确实可以用一个代码表示。

Conclusion: CoTyle方法提供了前所未有的简单性和多样性，从最小输入解锁了大量可复现的风格空间，实现了代码到风格的创新图像生成。
...
<details>
  <summary>Details</summary>
Motivation: 现有的3D基础模型大多仅使用RGB输入，忽略了容易获得的几何线索（相机内参、位姿、深度图等），限制了模型性能。

Method: 提出GeoAdapter使用零初始化卷积渐进式注入几何信息，不破坏基础模型表示空间；采用随机多模态融合机制，训练时随机采样模态子集，测试时可接受任意数量模态输入。

Result: 在单目/多视角深度估计、多视角立体视觉和相机位姿估计任务中优于现有方法，即使仅使用RGB输入也能达到SOTA；集成到VLA模型中在机器人任务上实现持续增益。
...
Method: 设计了基于对象的图像分析Mamba模型（OBIA-Mamba）以减少冗余计算；构建了全局-局部双分支CNN-Mamba架构来联合建模局部空间细节和全局上下文信息；采用了多任务优化框架，使用双重损失函数平衡局部精度和全局一致性。

Result: 在加拿大阿尔伯塔省的Sentinel-2影像上进行测试，与几种先进分类方法相比，所提出的方法实现了更高的分类精度和更精细的细节保留。

Conclusion: 该方法通过创新的OBIA-Mamba模型、全局-局部双分支架构和多任务优化，有效解决了Sentinel-2影像分类中的关键挑战，在精度和细节方面优于现有最先进方法。
...
TL;DR: 开发用于天花板钻孔等高空作业的机器人系统，包含移动底座、双阶段升降机、双臂躯干和定制钻孔末端执行器，采用高斯泼溅在线3D重建和神经配置空间障碍方法实现动态环境中的安全遥操作。


<details>
  <summary>Details</summary>
Motivation: 解决高空作业（如天花板钻孔）在动态环境中安全遥操作的挑战，特别是在视野受限的情况下处理移动障碍物。

Method: 使用高斯泼溅进行在线3D重建，引入运动参数建模移动物体，开发神经配置空间障碍方法进行规划和控制。

Result: 初步可行性研究表明硬件具备钻孔、螺栓固定和锚固能力，软件在动态环境中实现安全遥操作。

Conclusion: 该系统展示了在动态环境中进行高空作业的可行性，为安全遥操作提供了有效的硬件和软件解决方案。
...
### [82] [Baby Sophia: A Developmental Approach to Self-Exploration through Self-Touch and Hand Regard](https://arxiv.org/abs/2511.09727)
...
TL;DR: 基于婴儿发育启发，提出了一种用于机器人自主自我探索的强化学习框架，通过内在奖励机制模仿婴儿的好奇心驱动探索，学习自我触摸和手部注视行为。


<details>
  <summary>Details</summary>
Motivation: 受婴儿发育过程启发，旨在开发能够通过纯粹好奇心驱动信号实现自主多模态学习的机器人系统，模仿婴儿从随机运动到有目的行为的自然发展过程。

Method: 使用BabyBench仿真环境，通过内在奖励机制和课程学习：对于自我触摸，将高维触觉输入转换为紧凑表示；对于手部注视，通过运动探索学习手部视觉特征，并采用从单手到双手的课程学习策略。

Result: 结果表明，纯粹基于好奇心的信号无需外部监督即可驱动协调的多模态学习，成功模仿了婴儿从随机运动到有目的行为的发展过程。

Conclusion: 好奇心驱动的内在奖励机制能够有效促进机器人的自主学习和多模态协调能力发展，为开发更自然的机器人学习系统提供了新思路。
...
### [83] [A Robust Task-Level Control Architecture for Learned Dynamical Systems](https://arxiv.org/abs/2511.09790)
*Eshika Pathak,Ahmed Aboudonia,Sandeep Banik,Naira Hovakimyan*

Main category: cs.RO

TL;DR: 提出L1-DS框架，通过结合名义稳定控制器和L1自适应控制器来解决DS-LfD中的任务执行不匹配问题，并引入窗口DTW目标选择器处理时间错位。


<details>
  <summary>Details</summary>
Motivation: 解决DS-LfD中因未建模动态、持续干扰和系统延迟导致的任务执行不匹配问题，确保机器人实际任务空间状态与期望运动轨迹的一致性。

Method: 将任何基于DS的LfD模型与名义稳定控制器和L1自适应控制器结合，并引入窗口DTW目标选择器处理时间错位。

Result: 在LASA和IROS手写数据集上验证了框架的有效性。

Conclusion: L1-DS框架能够有效处理任务执行不匹配问题，提高基于DS的LfD方法的鲁棒性和跟踪性能。
...
### [84] [Provably Safe Stein Variational Clarity-Aware Informative Planning](https://arxiv.org/abs/2511.09836)
*Kaleb Ben Naveed,Utkrisht Sahai,Anouck Girard,Dimitra Panagou*

Main category: cs.RO

TL;DR: 提出了一种结合信息感知规划和安全验证的机器人轨迹规划框架，通过Stein变分推理优化信息采集轨迹，同时使用门禁机制确保安全性。


<details>
  <summary>Details</summary>
Motivation: 现有规划器大多将信息建模为静态或均匀衰减，忽略了空间变化衰减率的环境，且通常将安全性作为软约束处理，难以在信息采集任务中同时保证信息性和安全性。

Method: 使用清晰度模型表示环境不确定性，将清晰度动态嵌入轨迹优化，通过Stein变分推理进行贝叶斯推理学习，并采用门禁框架进行低层安全过滤。

Result: 在具有不同衰减率和障碍物的环境中进行的硬件实验和仿真表明，该方法能够持续保证安全性并减少信息赤字。

Conclusion: 该框架成功解决了在空间和时间变化环境中同时优化信息采集和确保安全性的挑战，为自主机器人在复杂环境中的信息收集任务提供了有效解决方案。
...
### [85] [PuffyBot: An Untethered Shape Morphing Robot for Multi-environment Locomotion](https://arxiv.org/abs/2511.09885)
*Shashwat Singh,Zilin Si,Zeynep Temel*

Main category: cs.RO

TL;DR: PuffyBot是一个受两栖动物启发的形状变形机器人，能够通过改变身体形态在陆地和水下环境中导航，使用剪刀式升降机构和连杆系统实现形态转换。


<details>
  <summary>Details</summary>
Motivation: 受两栖动物在陆地和水生环境中适应形态和运动的生物学特征启发，开发能够在多种环境中导航的机器人。

Method: 采用剪刀式升降机构作为主要结构，由线性驱动器驱动实现形状变形；集成曲柄连杆机构调节伺服驱动的肢体90度旋转，实现爬行和游泳模式的无缝切换；使用热塑性聚氨酯(TPU)织物确保防水性能。

Result: 机器人能够实现体积从255.00 cm³到423.75 cm³的变化，抵消3.237 N的下向力；可在陆地爬行、水下爬行、水面游泳，并具有双模态浮力调节功能；无缆操作时间达2小时。
...
### [86] [A Study on Enhancing the Generalization Ability of Visuomotor Policies via Data Augmentation](https://arxiv.org/abs/2511.09932)
*Hanwen Wang*

Main category: cs.RO

TL;DR: 该论文研究了如何通过自动化数据生成来增强视觉运动策略的泛化能力，创建了一个包含多种随机化因素的广泛数据集，并验证了这些因素对策略泛化能力的影响。


<details>
  <summary>Details</summary>
Motivation: 现有方法生成的数据缺乏多样性，限制了模仿学习策略的泛化能力，特别是在处理场景中物体随机放置的情况下。

Method: 通过自动化数据生成，创建了一个包含五种机械臂和两种抓手的广泛随机化数据集，涵盖了相机姿态、光照条件、桌面纹理和桌子高度等随机化因素。

Result: 研究发现所有随机化因素都会影响策略的泛化能力，任何形式的随机化都能增强策略泛化，特别是多样化的轨迹能有效弥合视觉差距。在低成本机械臂上验证了场景随机化对零样本仿真到现实迁移的增强效果。

Conclusion: 通过广泛的场景随机化可以显著提升视觉运动策略的泛化能力，特别是在零样本仿真到现实迁移任务中表现出色。
...
### [87] [Audio-VLA: Adding Contact Audio Perception to Vision-Language-Action Model for Robotic Manipulation](https://arxiv.org/abs/2511.09958)
*Xiangyi Wei,Haotian Zhang,Xinyi Cao,Siyu Xie,Weifeng Ge,Yang Li,Changbo Wang*

Main category: cs.RO

TL;DR: 本文提出Audio-VLA多模态操作策略，利用接触音频感知接触事件和动态过程反馈，克服纯视觉VLA模型的限制，并引入任务完成率(TCR)指标系统评估动态操作过程。


<details>
  <summary>Details</summary>
Motivation: 纯视觉VLA模型在感知交互和操作动态过程方面存在根本性限制，需要多模态感知来克服这些约束。
...
Conclusion: Audio-VLA通过整合音频模态显著提升了机器人操作性能，TCR指标为动态操作过程提供了更全面的评估标准。
...
### [88] [Phantom Menace: Exploring and Enhancing the Robustness of VLA Models against Physical Sensor Attacks](https://arxiv.org/abs/2511.10008)
*Xuancun Lu,Jiaxiang Chen,Shilin Xiao,Zizhi Jin,Zhangrui Chen,Hanwen Yu,Bohan Qian,Ruochen Zhou,Xiaoyu Ji,Wenyuan Xu*

Main category: cs.RO

TL;DR: 本文首次系统研究了针对视觉-语言-动作（VLA）模型的物理传感器攻击，提出了"Real-Sim-Real"框架来自动模拟基于物理的传感器攻击向量，并开发了基于对抗训练的防御方法。


<details>
  <summary>Details</summary>
Motivation: VLA模型严重依赖传感器输入，但其在物理世界传感器攻击下的安全性研究严重不足，需要填补这一研究空白。

Method: 提出"Real-Sim-Real"框架，自动模拟针对摄像头和麦克风的物理传感器攻击，包括6种摄像头攻击和2种麦克风攻击，并在真实机器人系统上进行验证。

Result: 通过大规模评估发现VLA模型存在显著漏洞，易受攻击性模式揭示了任务类型和模型设计的关键依赖性。基于对抗训练的防御方法能增强VLA模型对传感器攻击的鲁棒性。

Conclusion: 研究结果揭示了在安全关键环境中部署VLA模型时，迫切需要标准化鲁棒性基准和缓解策略来确保系统安全。
...
### [89] [DecARt Leg: Design and Evaluation of a Novel Humanoid Robot Leg with Decoupled Actuation for Agile Locomotion](https://arxiv.org/abs/2511.10021)
*Egor Davydenko,Andrei Volchenkov,Vladimir Gerasimov,Roman Gorbachev*

Main category: cs.RO

TL;DR: 提出了一种新型电动驱动机器人腿DecARt Leg，具有准伸缩运动学结构、旋转电机解耦驱动、近人形外观和新型多杆踝关节扭矩传输系统，通过FAST指标评估其敏捷运动能力。


<details>
  <summary>Details</summary>
Motivation: 旨在设计能够执行敏捷运动的电动驱动机器人腿，解决传统设计中运动性能和驱动效率的平衡问题。

Method: 采用准伸缩运动学结构配合旋转电机实现解耦驱动，设计近人形外观和新型多杆系统传输踝关节扭矩，提出FAST指标进行定量评估。

Result: 通过数值分析、广泛仿真和初步硬件实验验证了DecARt Leg设计的敏捷运动性能。

Conclusion: DecARt Leg设计在敏捷运动方面表现出色，为电动驱动机器人腿的设计提供了新的思路和评估方法。
...
### [90] [Physics-informed Machine Learning for Static Friction Modeling in Robotic Manipulators Based on Kolmogorov-Arnold Networks](https://arxiv.org/abs/2511.10079)
*Yizheng Wang,Timon Rabczuk,Yinghua Liu*

Main category: cs.RO

TL;DR: 本文提出了一种基于Kolmogorov Arnold Network (KAN)的物理启发机器学习方法，用于机器人关节的静摩擦建模，该方法结合样条激活函数和符号回归机制，在保持高预测精度的同时实现模型简化与物理表达提取。
...
Method: 采用基于KAN的物理启发机器学习方法，集成样条激活函数和符号回归机制，通过剪枝和属性评分实现模型简化和物理表达提取。

Result: 在合成数据和六自由度工业机械臂真实摩擦数据上的实验表明，该方法在各种任务中实现了大于0.95的决定系数，并成功提取了简洁且物理意义明确的摩擦表达式。
...
<details>
  <summary>Details</summary>
Motivation: 解决离线到在线强化学习中存在的两个基本挑战：多模态行为覆盖有限和在线适应期间的分布偏移问题。

Method: 提出UEPO统一生成框架，包括：(1)多种子动态感知扩散策略，无需训练多个模型即可有效捕捉多样化模态；(2)动态发散正则化机制，强制实施物理上有意义的策略多样性；(3)基于扩散的数据增强模块，增强动态模型泛化能力。

Result: 在D4RL基准测试中，UEPO在运动任务上比Uni-O4绝对提升5.9%，在灵巧操作任务上提升12.4%，展现出强大的泛化能力和可扩展性。

Conclusion: UEPO框架通过借鉴大语言模型预训练和微调策略，有效解决了离线到在线强化学习中的关键挑战，在多个任务上取得了显著性能提升。
...
TL;DR: 本文提出了一种基于轨迹分解和检索的模仿学习方法MT3，在少量演示（<10个）情况下比传统单阶段行为克隆方法提高了一个数量级的数据效率，实现了从单个演示学习日常操作任务，并在24小时内教会机器人1000个不同任务。


<details>
  <summary>Details</summary>
Motivation: 人类能够高效地从演示中学习任务，但当前的机器人模仿学习方法通常需要数百或数千个演示。本文旨在通过轨迹分解和检索先验来提高学习效率。

Method: 将操作轨迹分解为顺序对齐和交互阶段，并采用基于检索的泛化方法。开发了MT3方法，结合分解和检索技术。

Result: 在少量演示情况下，分解方法比单阶段学习提高了一个数量级的数据效率。MT3能够从单个演示学习日常操作任务，并泛化到新的物体实例，在24小时内教会机器人1000个不同任务。

Conclusion: 轨迹分解和检索先验显著提高了模仿学习的数据效率，使机器人能够从极少量演示中学习大量任务，为高效机器人学习提供了新途径。
...
TL;DR: 提出了RoboBenchMart基准测试，针对零售商店环境中的机器人操作任务，比传统的桌面场景更具挑战性和现实性。


<details>
  <summary>Details</summary>
Motivation: 现有机器人操作基准主要关注简化的桌面场景，缺乏对现实零售环境中复杂操作任务的评估能力。

Method: 开发了RoboBenchMart套件，包括程序化商店布局生成器、轨迹生成管道、评估工具和微调基线模型。

Result: 当前最先进的通用模型在常见的零售任务上表现不佳，证明了该基准的必要性。

Conclusion: RoboBenchMart为零售自动化领域的研究提供了重要的基准测试平台，有助于推动机器人操作技术的发展。
...
TL;DR: nuPlan-R是一个新的反应式闭环规划基准，用基于学习的反应式多智能体模拟替代了传统的基于规则的IDM智能体，提供了更真实、多样和类似人类的交通行为，改进了自动驾驶规划评估。
...
Method: 将基于学习的反应式多智能体模拟集成到nuPlan框架中，用噪声解耦的基于扩散的反应式智能体替代基于规则的IDM智能体，并引入交互感知的智能体选择机制以保证真实性和计算效率。

Result: 实验表明，反应式智能体模型产生了更真实、多样和类似人类的交通行为，基准环境更好地反映了真实世界的交互驾驶。重新实现了一系列规划方法，更清晰地展示了学习型规划器在处理复杂动态场景中的优势。
...
TL;DR: 本文提出了一种用于可靠机器人螺栓连接任务的控制框架，通过精确的扭矩控制、主动柔顺性和多模态人机界面，提高了操作员的情境意识和故障检测能力，在管道法兰连接任务中得到验证。
...
Method: 采用控制架构确保精确的驱动扭矩控制和主动柔顺性，设计多模态人机界面提供实时系统信息可视化，支持自动和手动控制的无缝切换，高层监督器协调执行并管理控制模式转换。

Result: 系统在管道法兰连接任务中验证，结果显示提高了故障检测能力、增强了操作员情境意识，并实现了精确柔顺的螺栓连接操作。

Conclusion: 该框架有效提升了机器人螺栓连接任务的可靠性和安全性，但发现仅依赖单个摄像头难以实现完全的情境意识。
...
TL;DR: 提出基于MuJoCo的折纸机制仿真框架，使用图形界面定义约束和驱动，通过案例研究验证优化效果


<details>
  <summary>Details</summary>
Motivation: 折纸机制在机器人系统中应用广泛，但准确模拟其折叠行为和环境交互仍具挑战性

Method: 将折纸片表示为相互连接的可变形元素图，利用MuJoCo的可变形体功能，通过GUI定义折痕和驱动等约束

Result: 在折纸弹射器案例中，通过CMA-ES优化设计参数，实验验证优化结构实现了更好的投掷性能
...
### [98] [Probability-Biased Attention over Directed Bipartite Graphs for Long-Tail ICD Coding](https://arxiv.org/abs/2511.09559)
*Tianlei Chen,Yuxiao Chen,Yang Li,Feifei Wang*

Main category: cs.LG

TL;DR: 本文提出了一种基于有向二分图编码器的自动化ICD编码方法，通过建模常见代码和罕见代码之间的细粒度共现关系，利用条件概率引导信息从常见代码流向罕见代码，从而改善长尾分布下的分类性能。


<details>
  <summary>Details</summary>
Motivation: 自动化ICD编码面临大规模标签空间（10,000-20,000个代码）和长尾分布挑战，许多罕见代码缺乏足够的训练数据。需要解决如何利用常见代码的信息来增强罕见代码表示的问题。

Method: 构建有向二分图编码器，包含常见代码和罕见代码两个不相交节点集。边仅从常见代码指向罕见代码，基于条件概率定义连接权重。通过共现编码将概率偏置注入注意力模块，利用LLM生成代码描述来丰富初始嵌入。

Result: 在三个自动化ICD编码基准数据集上的实验表明，该方法实现了最先进的性能，在长尾分类关键指标Macro-F1上取得了显著改进。

Conclusion: 通过建模代码间的细粒度共现关系，利用常见代码的统计共现信息来增强罕见代码表示，能够有效解决ICD编码中的长尾分布问题，提升分类性能。
...
### [99] [Let the Experts Speak: Improving Survival Prediction & Calibration via Mixture-of-Experts Heads](https://arxiv.org/abs/2511.09567)
*Todd Morrill,Aahlad Puli,Murad Megjhani,Soojin Park,Richard Zemel*

Main category: cs.LG

TL;DR: 本文提出了几种基于离散时间深度混合专家模型的生存分析架构，其中一种能够同时实现聚类、校准和预测准确性。研究发现，更灵活、能够为每个患者定制预测的专家模型优于依赖固定组原型的专家模型。


<details>
  <summary>Details</summary>
Motivation: 深度混合专家模型在生存分析中受到关注，但实践中分组往往以校准误差和预测准确性为代价。本文旨在探索是否能在发现患者群体结构的同时提高校准和预测准确性。

Method: 引入了几种离散时间深度混合专家架构，比较了不同表达能力的专家模型，特别关注专家模型如何为个体患者定制预测。

Result: 研究发现，更灵活、能够为每个患者定制预测的专家模型优于依赖固定组原型的专家模型。其中一种架构成功实现了聚类、校准和预测准确性的平衡。

Conclusion: 专家模型的表达能力是区分不同混合专家架构的关键因素，为个体患者定制预测的灵活专家模型能够同时实现良好的聚类效果、校准性能和预测准确性。
...
### [100] [Group Averaging for Physics Applications: Accuracy Improvements at Zero Training Cost](https://arxiv.org/abs/2511.09573)
...
<details>
  <summary>Details</summary>
Motivation: 许多自然科学中的机器学习任务具有特定的对称性等变性，但等变方法常因训练困难、期望模型自行学习对称性或实现复杂而未被采用。群平均技术可以解决这些问题。

Method: 在测试时对训练好的模型进行群平均操作，即对模型在对称群上的输出进行平均，成本与群大小成正比，且不要求模型结构或训练过程具有等变性。
...
### [101] [Scaling Environments for LLM Agents in the Era of Learning from Interaction: A Survey](https://arxiv.org/abs/2511.09586)
*Yuchen Huang,Sijia Li,Minghao Liu,Wei Liu,Shijue Huang,Zhiyuan Fan,Hou Pong Chan,Yi R. Fung*

Main category: cs.LG

TL;DR: 本文提出Generation-Execution-Feedback (GEF)循环框架，强调通过环境交互而非静态数据集来训练LLM智能体，并系统综述了环境扩展方法。


<details>
  <summary>Details</summary>
Motivation: 当前基于人类知识构建的静态数据集成本高、缺乏动态性和真实性，无法有效培养智能体的自适应行为和长期决策能力。

Method: 提出GEF循环框架，将智能体训练过程分为任务生成、任务执行和反馈三个阶段，并从环境中心视角系统综述环境扩展方法。

Result: 建立了环境扩展方法的分类框架，分析了基准测试、实现策略和应用场景，整合了该领域的碎片化进展。

Conclusion: 环境作为经验数据生产者对智能体智能发展至关重要，需要向更复杂、真实和交互的方向扩展，并提出了未来研究方向。
...
### [102] [Making Every Head Count: Sparse Attention Without the Speed-Performance Trade-off](https://arxiv.org/abs/2511.09596)
*Mingkuan Zhao,Wentao Hu,Jiayin Wang,Xin Lai,Tianchen Huang,Yuheng Min,Rui Yan,Xiaoyan Zhu*

Main category: cs.LG

TL;DR: SPAttention通过引入原则性结构稀疏性，将多头注意力计算从H个独立的O(N²)计算转变为单个协作的O(N²)计算，将复杂度降低H倍，在保持性能的同时显著提升训练吞吐量。


<details>
  <summary>Details</summary>
Motivation: 解决LLM中多头注意力机制的计算冗余问题，标准实现存在H个独立头在相同序列空间计算注意力的计算冗余，而现有稀疏方法往往以信息完整性为代价换取计算效率。

Method: 提出SPAttention，采用原则性结构稀疏性范式，将总注意力工作负载划分为平衡的非重叠距离带，为每个头分配唯一片段，强制头间功能专业化。

Result: 在OLMoE-1B-7B和0.25B-1.75B模型系列上的实验验证显示，训练吞吐量提升约2倍，性能与标准密集注意力相当，在某些关键指标上甚至超越，且在所有评估指标上持续优于Longformer、Reformer和BigBird等代表性稀疏注意力方法。

Conclusion: SPAttention通过结构化的归纳偏置实现了计算效率与性能的良好平衡，为LLM设计提供了新的高效注意力机制解决方案。
...
### [103] [Parametric Expensive Multi-Objective Optimization via Generative Solution Modeling](https://arxiv.org/abs/2511.09598)
*Tingyang Wei,Jiao Liu,Abhishek Gupta,Chin Chun Ooi,Puay Siew Tan,Yew-Soon Ong*

Main category: cs.LG

TL;DR: 本文提出了首个参数化多目标贝叶斯优化器，通过交替使用采集驱动搜索和条件生成模型，学习能够直接预测任意任务偏好查询优化解的反向模型，避免了昂贵的重复评估。


<details>
  <summary>Details</summary>
Motivation: 现实应用中需要在不同操作条件下解决昂贵的多目标优化问题族，这产生了参数化昂贵多目标优化问题(P-EMOPs)。传统方法需要为每个任务单独进行昂贵评估，而连续任务参数空间包含无限个不同问题，因此需要学习能够直接预测优化解的反向模型。

Method: 提出交替框架：(1)利用任务间协同效应的采集驱动搜索，(2)通过条件生成模型进行生成解采样。使用任务感知高斯过程理论证明更快的收敛速度。

Result: 理论分析表明通过任务间协同效应实现了更快收敛。在合成和真实世界基准测试中的实证研究验证了交替框架的有效性。

Conclusion: 该方法能够高效优化相关任务，最终实现无需额外昂贵评估即可直接预测未见参数化EMOPs的解。
...
### [104] [Optimistic Reinforcement Learning with Quantile Objectives](https://arxiv.org/abs/2511.09652)
*Mohammad Alipour-Vaezi,Huaiyang Zhong,Kwok-Leung Tsui,Sajad Khodadadian*

Main category: cs.LG

TL;DR: 本文提出了UCB-QRL算法，一种用于有限时域马尔可夫决策过程中τ-分位数目标的乐观学习算法，通过估计转移概率并在置信区间内优化分位数价值函数，获得了高概率遗憾界。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习基础未考虑目标函数的风险敏感性，这在医疗和金融等领域至关重要。通过优化累积奖励分布的特定分位数来纳入风险敏感性是一种流行方法。

Method: 开发了UCB-QRL迭代算法，在每个迭代中先估计底层转移概率，然后在估计值的置信区间内优化分位数价值函数。

Result: UCB-QRL在具有S状态、A动作、T回合和H时域的回合设置中，获得了高概率遗憾界O((2/κ)^{H+1}H√(SATHlog(2SATH/δ)))，其中κ>0是问题相关常数。

Conclusion: 该算法成功解决了风险敏感的强化学习问题，为分位数目标提供了理论保证的解决方案。
...
### [105] [Generalization Can Emerge in Tabular Foundation Models From a Single Table](https://arxiv.org/abs/2511.09665)
*Junwei Ma,Nour Shaheen,Alex Labach,Amine Mhedhbi,Frank Hutter,Anthony L. Caterini,Valentin Thomas*

Main category: cs.LG

TL;DR: 研究发现，仅需少量数据即可实现表格数据的泛化，无需大规模合成或真实数据集。通过在单个真实表格上进行自监督预训练，就能在异构基准测试中获得强大迁移效果。


<details>
  <summary>Details</summary>
Motivation: 挑战当前观点，即表格深度建模需要在大规模合成或真实数据集上进行预训练才能实现广泛泛化。探索是否少量数据就足够实现泛化。

Method: 使用简单的自监督预训练方法，仅在单个真实表格上进行预训练，然后系统地在多个异构数据集上评估迁移性能。分析数据特征对构建表格基础模型的重要性。

Result: 发现仅需相对少量数据即可实现泛化，单个表格的自监督预训练能在异构基准测试中产生令人惊讶的强大迁移效果。数据集能构建的任务数量和质量对下游性能至关重要。

Conclusion: 表格基础模型的泛化能力主要取决于从数据集中构建的任务数量和质量，而非预训练数据的规模。这为更高效的表格建模方法提供了新思路。
...
### [106] [Boosted GFlowNets: Improving Exploration via Sequential Learning](https://arxiv.org/abs/2511.09677)
*Pedro Dall'Antonia,Tiago da Silva,Daniel Augusto de Souza,César Lincoln C. Mattos,Diego Mesquita*

Main category: cs.LG

TL;DR: 提出了Boosted GFlowNets方法，通过顺序训练集成模型来改善GFlowNets在探索奖励空间时的不平衡问题，提高了样本多样性和探索能力。


<details>
  <summary>Details</summary>
Motivation: 标准GFlowNets在实践中往往难以均匀探索奖励空间，容易偏向容易到达的区域，导致难以到达的高奖励区域梯度消失或信息不足，从而影响对高奖励区域的覆盖。

Method: 采用顺序训练集成GFlowNets的方法，每个后续模型优化残差奖励，补偿前一个模型已捕获的质量，从而重新激活未充分探索区域的学习信号。

Result: 在合成多模态基准和肽设计任务中，Boosted GFlowNets显著改善了探索能力和样本多样性，同时保持了标准轨迹平衡训练的稳定性和简单性。

Conclusion: Boosted GFlowNets通过残差原理有效解决了GFlowNets的探索不平衡问题，在保持训练稳定性的同时显著提高了性能。
...
### [107] [ConstrainedSQL: Training LLMs for Text2SQL via Constrained Reinforcement Learning](https://arxiv.org/abs/2511.09693)
...
TL;DR: 本文提出了一种带约束的强化学习框架用于Text2SQL任务，通过引入自然且可解释的奖励和约束信号，并在训练过程中动态平衡它们之间的权衡，解决了传统RL方法中奖励函数设计敏感和奖励黑客问题。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习方法在提升Text2SQL LLMs推理能力时，性能高度依赖于奖励函数设计，不当的奖励会导致奖励黑客问题，即模型利用奖励结构漏洞获得高分而非真正解决问题。

Method: 采用带约束的强化学习框架，引入自然可解释的奖励和约束信号，在训练过程中动态平衡这些信号之间的权衡。

Result: 在知名Text2SQL数据集上的数值实验证实了该方法相比最先进的RL训练LLMs的改进效果。

Conclusion: 提出的约束强化学习框架为Text2SQL任务提供了理论保证，并通过实验验证了其优于现有RL方法的性能。
...
### [108] [Efficient Hyperdimensional Computing with Modular Composite Representations](https://arxiv.org/abs/2511.09708)
*Marco Angioli,Christopher J. Kymn,Antonello Rosato,Amy Loutfi,Mauro Olivieri,Denis Kleyko*

Main category: cs.LG

TL;DR: MCR是一种使用模运算的高维整数向量计算模型，在容量、精度和硬件效率方面达到独特平衡，相比二进制向量性能更优，内存占用更少，硬件实现可带来3个数量级加速和显著能耗降低。


<details>
  <summary>Details</summary>
Motivation: 尽管MCR作为二进制散射码模型的泛化具有更高表示能力，但缺乏系统分析，其额外复杂性被认为超过表达性改进，需要重新评估其实际价值。

Method: 对MCR进行首次广泛评估，包括容量测量、123个数据集上的精度测试，以及设计首个专用硬件加速器进行硬件实现分析。

Result: MCR在容量上优于二进制和整数向量，接近复数表示但内存占用更少；精度测试显示可匹配二进制散射码性能但内存减少4倍；硬件实现带来最高3个数量级加速和2.68倍能耗降低。

Conclusion: MCR虽然需要更复杂操作，但其模运算和更高精度支持更低维度，配合专用硬件可实现更快、更节能的高精度替代方案。
...
TL;DR: FlowCast是首个将条件流匹配（CFM）应用于降水临近预报的模型，相比扩散模型，CFM学习直接从噪声到数据的映射，能以更少的函数评估实现快速、高保真样本生成，在预测精度和计算效率方面均优于扩散模型。


<details>
  <summary>Details</summary>
Motivation: 雷达降水临近预报对洪水风险管理和决策至关重要，但现有深度学习方法面临大气动力学不确定性和高维数据建模效率两大挑战。扩散模型虽然能产生清晰可靠的预报，但其迭代采样过程计算成本过高，不适合时间关键型应用。

Method: 提出FlowCast模型，首次将条件流匹配（CFM）应用于降水临近预报。CFM学习直接从噪声到数据的映射，而不是像扩散模型那样通过迭代去噪过程，从而大幅减少函数评估次数。

Result: 实验表明FlowCast在预测精度上达到新的最先进水平。与相同架构上的扩散目标相比，CFM目标不仅更准确，而且显著更高效，能在显著更少的采样步骤下保持高性能。

Conclusion: 这项工作将CFM定位为高维时空预报中强大且实用的替代方案，为时间关键型应用提供了更高效的解决方案。
...
TL;DR: 本文提出了SPARC方法，通过单阶段适应实现鲁棒控制，解决了强化学习在未见环境中的泛化问题，特别是在缺乏显式上下文信息的OOD设置下。


<details>
  <summary>Details</summary>
Motivation: 解决机器人和控制领域中智能体在未见环境中的泛化挑战，特别是在自动驾驶、四足机器人等需要适应不同地形和天气条件的场景中，且测试时无法获取显式上下文信息。

Method: 提出SPARC方法，简化了现有的两阶段训练方法，采用单阶段适应策略，避免了分别训练上下文编码器和历史适应模块的复杂性。

Result: 在Gran Turismo 7赛车模拟器和风扰动的MuJoCo环境中测试，SPARC实现了可靠且鲁棒的OOD泛化性能。

Conclusion: SPARC方法简化了训练流程，同时保持了在OOD设置下的良好泛化能力，为上下文强化学习提供了更实用的解决方案。
...
### [111] [TawPipe: Topology-Aware Weight Pipeline Parallelism for Accelerating Long-Context Large Models Training](https://arxiv.org/abs/2511.09741)
*Houming Wu,Ling Chen*

Main category: cs.LG

TL;DR: TawPipe是一种拓扑感知的权重管道并行方法，通过利用分布式集群中的分层带宽来提高通信效率，解决了大语言模型训练中的内存限制和通信开销问题。


<details>
  <summary>Details</summary>
Motivation: 训练大语言模型面临设备内存有限和设备间通信成本高的根本限制。虽然管道并行通过跨设备分区模型缓解了内存压力，但会产生随序列长度线性增长的激活通信开销，限制了长上下文训练的效率。现有的权重传递方法虽然通过传输模型权重而非激活来缓解这一问题，但存在冗余的对等传输和未充分利用的节点内带宽问题。

Method: TawPipe采用拓扑感知的权重管道并行方法：(i) 基于拓扑对设备进行分组，优化节点内集体通信和节点间对等通信；(ii) 为每个设备分配固定的模型权重和梯度分片，避免冗余传输；(iii) 将通信与计算重叠以隐藏延迟。与完全分片数据并行中使用的全局集体操作不同，TawPipe将大部分通信限制在节点边界内。

Result: 在最多24个GPU上使用LLaMA风格模型进行的广泛实验表明，与最先进的基线方法相比，TawPipe实现了卓越的吞吐量和可扩展性。

Conclusion: TawPipe通过利用分布式集群的拓扑结构，显著减少了跨节点流量，在保持模型训练效率的同时有效解决了大语言模型训练中的通信瓶颈问题。
...
TL;DR: 本文提出了一种基于宏观情境检索的金融预测框架，通过检索历史上类似的宏观经济情境来增强预测的稳健性，解决了传统模型在分布外数据上的性能下降问题。
...
Result: 在17年S&P 500数据上训练，并在AAPL和XOM上进行OOD评估，该框架显著缩小了CV到OOD的性能差距，实现了唯一正向的样本外交易结果（AAPL: PF=1.18, Sharpe=0.95; XOM: PF=1.16, Sharpe=0.61）。

Conclusion: 通过操作化"金融历史可能不会重复，但常常押韵"的原则，这项工作证明宏观感知检索能在分布变化下产生稳健、可解释的预测，检索到的邻居形成可解释的证据链，对应可识别的宏观情境。
...
### [113] [Is nasty noise actually harder than malicious noise?](https://arxiv.org/abs/2511.09763)
...
Result: 在分布无关学习中，两种噪声模型具有强等价性；在固定分布学习中，存在任意大的分离；ICE算法在两种噪声模型下等价，但噪声率存在2倍的必要差异。

Conclusion: 学习设置对噪声容忍度有重大影响，ICE算法为固定分布设置提供了实用的解决方案，但噪声率差异是不可避免的。
...
### [114] [Harnessing Bounded-Support Evolution Strategies for Policy Refinement](https://arxiv.org/abs/2511.09923)
*Ethan Hirschowitz,Fabio Ramos*

Main category: cs.LG

TL;DR: 本文提出了一种基于三角分布进化策略（TD-ES）的机器人策略优化方法，通过有界三角噪声和中心秩有限差分估计器实现稳定、可并行化的无梯度更新，在PPO预训练后进行TD-ES微调，显著提升机器人操作任务的性能。


<details>
  <summary>Details</summary>
Motivation: 基于策略的强化学习在改进机器人策略时经常受到噪声大、信号弱的梯度问题困扰，需要更稳定的优化方法。

Method: 提出三角分布进化策略（TD-ES），使用有界、反三角分布的扰动进行局部探索，结合中心秩有限差分估计器，形成两阶段优化流程：PPO预训练后接TD-ES微调。

Result: 在一系列机器人操作任务中，TD-ES相比PPO将成功率相对提高了26.5%，并大幅降低了方差。

Conclusion: TD-ES提供了一种简单、计算量小的可靠微调路径，在保持早期样本效率的同时实现了稳健的后期性能提升。
...
### [115] [Hail to the Thief: Exploring Attacks and Defenses in Decentralised GRPO](https://arxiv.org/abs/2511.09780)
*Nikolay Blagoev,Oğuzhan Ersoy,Lydia Yiyu Chen*

Main category: cs.LG

TL;DR: 本文提出了去中心化GRPO中的首个对抗性攻击方法，展示了恶意方可以通过注入恶意令牌来毒化良性模型，攻击成功率可达100%，并提出了两种防御方法。


<details>
  <summary>Details</summary>
Motivation: GRPO在大型语言模型后训练中应用广泛，但由于其去中心化特性存在安全风险，需要研究对抗性攻击及其防御方法。

Method: 通过在去中心化GRPO系统中注入任意恶意令牌，实施上下文外和上下文内攻击，使用数学和编程任务进行实证验证。

Result: 对抗性攻击可以轻易毒化良性节点，污染其本地LLM后训练，在50次迭代内攻击成功率可达100%。

Conclusion: 提出了两种防御方法，根据用户是否训练相同模型而不同，防御效果可达100%的阻止率，使攻击无法进行。
...
### [116] [Koopman Invariants as Drivers of Emergent Time-Series Clustering in Joint-Embedding Predictive Architectures](https://arxiv.org/abs/2511.09783)
*Pablo Ruiz-Morales,Dries Vanoost,Davy Pissoort,Mathias Verbeke*

Main category: cs.LG

TL;DR: JEPAs在时间序列数据中能够自动聚类不同动态机制，本文提出理论解释：JEPA的预测目标使其隐式学习系统Koopman算子的不变子空间。证明理想化JEPA损失最小化时编码器学习到系统机制指示函数，即Koopman特征函数。


<details>
  <summary>Details</summary>
Motivation: 解释JEPAs在时间序列数据中自动聚类不同动态机制的现象，建立自监督学习与动力系统理论的联系。

Method: 提出理论框架，证明理想化JEPA损失最小化条件，在已知动态的合成数据上验证理论，分析线性预测器约束的作用。

Result: 验证了约束JEPA线性预测器为近恒等算子是关键归纳偏置，迫使编码器学习不变特征。揭示了预测器在表示解纠缠中的作用。

Conclusion: 阐明了JEPAs的关键行为，建立了现代自监督学习与动力系统理论的原理性联系，为设计更鲁棒和可解释的时间序列模型提供了指导。
...
### [117] [CaReTS: A Multi-Task Framework Unifying Classification and Regression for Time Series Forecasting](https://arxiv.org/abs/2511.09789)
...
Method: 采用双流架构：分类分支学习未来逐步趋势，回归分支估计与最新观测值的偏差；设计多任务损失函数进行不确定性感知加权；实例化四个变体整合CNN、LSTM和Transformer等主流时序建模编码器。

Result: 在真实世界数据集上的实验表明，CaReTS在预测准确性上优于最先进算法，同时实现更高的趋势分类性能。

Conclusion: CaReTS框架通过结合分类和回归任务，能够提供既准确又可解释的时间序列预测，成功将宏观趋势与微观偏差解耦。
...
### [118] [Beyond Monotonicity: Revisiting Factorization Principles in Multi-Agent Q-Learning](https://arxiv.org/abs/2511.09792)
*Tianmeng Hu,Yongzheng Cui,Rui Tang,Biao Luo,Ke Li*

Main category: cs.LG

TL;DR: 本文对多智能体强化学习中的非单调价值分解进行动力学系统分析，证明在近似贪婪探索下，违反IGM一致性的零损失平衡点是不稳定的鞍点，而IGM一致的解是稳定的吸引子。实验表明无约束的非单调分解能可靠恢复IGM最优解，并优于单调基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有价值分解方法要么施加单调性约束限制表达能力，要么采用较软的替代方法但增加算法复杂度。本文旨在通过动力学系统分析探索非单调价值分解的可行性。

Method: 将学习动态建模为连续时间梯度流，对非单调价值分解进行动力学系统分析，证明在近似贪婪探索下的稳定性特性。

Result: 实验在合成矩阵游戏和挑战性MARL基准测试中表明，无约束的非单调分解能可靠恢复IGM最优解，并持续优于单调基线方法。

Conclusion: 非单调价值分解是可行的，无需施加单调性约束也能获得IGM一致解，为未来基于价值的MARL算法设计提供了可操作的见解。
...
### [119] [Constrained Best Arm Identification with Tests for Feasibility](https://arxiv.org/abs/2511.09808)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的最佳臂识别问题通常需要满足额外的可行性约束，且性能和约束的测量往往是分开进行的（如药物发现中的效力和安全性测试），现有研究假设两者同时观测的局限性需要新的算法设计。

Method: 提出了一种高效的BAI算法，允许决策者选择臂和测试类型（性能或可行性约束），算法能够根据问题难度自然适应，通过较差性能或不可行性来消除臂。

Result: 给出了算法的样本复杂度上界，并证明了算法在δ→0时的渐近最优性，实验表明该算法在合成和真实数据集上优于其他最先进的BAI算法。

Conclusion: 本文提出的算法能够有效处理带可行性约束的最佳臂识别问题，在理论和实验上都表现出优越性能，为解决现实世界中的约束优化问题提供了有效工具。
...
TL;DR: 本文提出了一种新算法，能够绕过CSQ（相关统计查询）的硬度障碍，在满足因子化假设的分布下，以多项式时间学习两个半空间的交集，而CSQ方法即使弱学习也需要拟多项式时间。


<details>
  <summary>Details</summary>
Motivation: 学习半空间交集是计算学习理论中的核心问题。即使对于两个半空间，是否能在多项式时间内学习仍然是一个主要的开放性问题。现有最佳算法运行在拟多项式时间，且证明仅依赖CSQ的算法无法避免这种复杂度。

Method: 采用新的对偶框架分析边际分布诱导的矩张量结构，结合Jennrich算法的改进版本、随机投影矩张量的PCA，以及基于梯度下降的非凸优化框架。

Result: 在因子化分布下，CSQ方法需要拟多项式时间进行弱学习，而我们的算法通过利用更一般的统计查询（SQ），实现了多项式时间学习，建立了CSQ和SQ之间的强分离。

Conclusion: 本文证明了在因子化分布假设下，可以绕过CSQ硬度障碍，实现两个半空间交集的快速学习，为这一经典问题提供了新的解决方案。
...
TL;DR: 提出ACT数据标注管道，使用LLMs作为标注者和错误识别者，通过人类仅审查可疑案例来提高标注效率，在多个领域实现性能差距小于2%的同时节省90%人力成本。


<details>
  <summary>Details</summary>
Motivation: 解决监督学习中高质量标注数据获取成本高、耗时长的问题，现有LLM标注质量仍低于人类水平。

Method: ACT数据管道：LLMs同时担任标注者和评判者，识别潜在错误，人类仅需审查最可疑的案例，结合多模态LLMs适用于NLP、CV和多模态领域。

Result: 在大多数基准数据集上性能差距可减少到小于2%，同时节省高达90%的人力成本。

Conclusion: ACT管道能有效提升标注质量并大幅降低人力成本，通过理论分析和实验验证了其有效性，并提供了7条实用指导原则。
...
TL;DR: 提出一种轻量级动态对齐机制，通过从验证器隐藏状态计算转向向量并注入预训练草稿器，显著提升接受率，同时计算开销可忽略不计。


<details>
  <summary>Details</summary>
Motivation: 推测解码通过草稿-验证分离加速语言模型推理，但草稿器与验证器不对齐限制了令牌接受率。预训练草稿器虽然较慢但接受率更高，本研究旨在通过动态对齐机制进一步提升其接受率。

Method: 引入轻量级动态对齐机制：从验证器隐藏状态计算转向向量并注入预训练草稿器，相比现有离线对齐方法（如蒸馏）具有动态适应性。

Result: 在标准采样下接受令牌数提升高达35%，贪婪采样下提升22%，计算开销可忽略不计，且可适配现有架构和预训练模型。

Conclusion: 动态对齐机制有效解决了草稿器-验证器不对齐问题，显著提升推测解码效率，具有实际部署价值。
...
### [123] [Unlearning Imperative: Securing Trustworthy and Responsible LLMs through Engineered Forgetting](https://arxiv.org/abs/2511.09855)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在敏感领域的广泛应用，确保私人信息能够被永久遗忘的能力成为关键弱点，现有系统缺乏可靠机制来保证敏感信息在使用后能够被永久移除。

Method: 通过综述近期机器遗忘研究，评估遗忘是否发生的验证方法，分析遗忘模型对抗攻击的韧性，以及支持用户信任的机制。同时考察了差分隐私、同态加密、联邦学习和短暂记忆等技术方案，以及审计实践和监管框架等制度保障。

Result: 研究发现虽然取得了稳步进展，但稳健且可验证的遗忘问题仍未解决。需要避免昂贵重新训练的高效技术、更强的对抗恢复防御能力，以及加强问责的治理结构。

Conclusion: 通过整合技术和组织视角，本研究为构建既能按要求遗忘、又能维护隐私和公众信任的AI系统指明了路径，但实现这一目标仍需进一步努力。
...
### [124] [Uncertainty-Guided Checkpoint Selection for Reinforcement Finetuning of Large Language Models](https://arxiv.org/abs/2511.09864)
*Manh Nguyen,Dung Nguyen,Dai Do,Svetha Venkatesh,Hung Le*

Main category: cs.LG

TL;DR: 本文提出了一种基于不确定性的检查点选择方法(UGCS)，通过识别困难样本并使用这些样本的奖励来评估检查点，从而选择具有更好泛化能力的模型检查点。
...
Method: 使用基于不确定性的检查点选择方法，识别困难问答对，通过计算top不确定样本在短训练窗口内的平均奖励来评估检查点。

Result: 在三个数据集和三个大语言模型上的实验表明，该方法能一致地选择具有更强泛化能力的检查点，优于依赖训练或验证性能的传统策略。

Conclusion: 能够以低不确定性解决最困难任务的模型是最可靠的，基于不确定性的检查点选择方法有效提升了模型选择质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有公平聚类算法计算复杂度高（二次或超二次），难以应用于大规模数据集，需要开发更高效的解决方案。
...
Conclusion: AFCF为公平聚类提供了通用、即插即用的线性时间解决方案，显著提升了算法的实用性和可扩展性。
...
TL;DR: SE-Diff是一种新颖的生理模拟器和经验增强扩散模型，用于生成心电图(ECG)。它通过整合轻量级ODE模拟器和LLM驱动的经验检索策略，解决了现有方法忽视生理机制知识和临床经验知识的问题。


<details>
  <summary>Details</summary>
Motivation: 心血管疾病是全球主要死亡原因，ECG是最常用的无创心脏评估工具，但大型标注ECG数据集稀缺。生成ECG有助于理解心脏电活动机制、构建大规模无偏数据集并促进隐私保护的数据共享。现有扩散模型方法忽视了生理模拟器知识和基于经验的临床知识。

Method: SE-Diff将轻量级ODE-based ECG模拟器通过节拍解码器和模拟器一致性约束整合到扩散过程中，注入机制先验以促进生理合理的波形。同时设计LLM驱动的经验检索增强策略来注入临床知识，为ECG生成提供更多指导。

Result: 在真实世界ECG数据集上的广泛实验表明，SE-Diff在信号保真度和文本-ECG语义对齐方面均优于基线方法，证明了其在文本到ECG生成方面的优越性。基于模拟器和经验的知识也受益于下游ECG分类任务。
...
TL;DR: 本文证明了增量生成在基于流的去噪模型中是必要且充分的，用于在[0,1]^d上的保向同胚类中实现通用逼近。证明了单步自治流无法实现通用逼近，但通过组合多个自治流可以实现逼近。


<details>
  <summary>Details</summary>
Motivation: 为基于流的增量去噪模型提供严格的逼近理论基础，解决其经验优势缺乏理论支撑的问题。

Method: 使用拓扑动力学论证证明单步自治流的局限性，利用自治流的代数性质构建多步流组合，并通过线性提升维度实现通用逼近。

Result: 证明了单步自治流在保向同胚空间中是贫集且非通用的，但通过组合K_d个自治流可以实现O(n^{-1/d})的逼近率，在光滑条件下可实现维度无关的逼近。

Conclusion: 增量生成是基于流去噪模型实现通用逼近的必要条件，通过多步流组合可以克服单步流的局限性，为这类模型提供了坚实的理论基础。
...
### [128] [Towards Multiple Missing Values-resistant Unsupervised Graph Anomaly Detection](https://arxiv.org/abs/2511.09917)
*Jiazhen Chen,Xiuqin Liang,Sichao Fu,Zheng Ma,Weihua Ou*

Main category: cs.LG

TL;DR: M$^2$V-UGAD是一个针对不完整图的无监督图异常检测框架，能够同时处理节点属性和图结构的缺失，通过双路径编码器和潜在空间正则化来防止跨视图干扰并减轻插补偏差。


<details>
  <summary>Details</summary>
Motivation: 现有无监督图异常检测方法通常假设完整的节点属性和结构信息，但现实场景中由于隐私、收集错误或动态节点到达等因素，图数据往往不完整。标准插补方案可能修复异常节点使其看起来正常，导致插补偏差，同时节点属性和边同时缺失时会产生跨视图干扰。

Method: 提出双路径编码器独立重建缺失的节点属性和图结构，防止错误传播；在联合潜在空间中进行融合和正则化，使正常节点占据紧凑内部流形而异常节点位于外部壳层；通过采样潜在空间外部代码解码为真实节点特征和子图，提供硬负例以锐化决策边界。

Result: 在七个公共基准测试上的实验表明，M$^2$V-UGAD在不同缺失率下始终优于现有的无监督图异常检测方法。

Conclusion: M$^2$V-UGAD框架有效解决了不完整图上的异常检测问题，通过防止跨视图干扰和减轻插补偏差，显著提升了检测性能。
...
### [129] [MDMLP-EIA: Multi-domain Dynamic MLPs with Energy Invariant Attention for Time Series Forecasting](https://arxiv.org/abs/2511.09924)
...
Method: 提出三个关键创新：1) 自适应融合双域季节性MLP，将季节信号分为强弱分量并采用自适应零初始化通道融合；2) 能量不变注意力机制，自适应关注不同特征通道并保持总信号能量恒定；3) 通道独立MLP的动态容量调整机制，神经元数量随通道数平方根缩放。

Result: 在九个基准数据集上的广泛实验表明，MDMLP-EIA在预测精度和计算效率方面均达到最先进性能。

Conclusion: MDMLP-EIA通过多域动态MLP和能量不变注意力机制，有效解决了MLP方法在时间序列预测中的关键限制，实现了优异的预测性能和计算效率。
...
### [130] [EEGAgent: A Unified Framework for Automated EEG Analysis Using Large Language Models](https://arxiv.org/abs/2511.09947)
*Sha Zhao,Mingyi Peng,Haiteng Jiang,Tao Li,Shijian Li,Gang Pan*

Main category: cs.LG

TL;DR: EEGAgent是一个基于大语言模型的通用脑电图分析框架，能够调度多种工具自动完成EEG相关任务，支持灵活可解释的脑电分析。
...
Result: 在公共数据集上评估显示，EEGAgent能够支持灵活和可解释的EEG分析，展示了在真实临床应用中的潜力。
...
### [131] [Autonomous Concept Drift Threshold Determination](https://arxiv.org/abs/2511.09953)
...
Method: 提出动态阈值确定算法，通过证明动态策略能优于任何单一固定阈值，并在现有漂移检测框架中引入新的比较阶段来指导阈值调整。

Result: 在广泛的合成和真实数据集（包括图像和表格数据）上的大量实验验证，该方法显著提升了最先进漂移检测器的性能。

Conclusion: 动态阈值策略在理论上和实践上都优于固定阈值方法，能够有效平衡误报和延迟检测，提升漂移检测的整体效果。
...
### [132] [MultiTab: A Scalable Foundation for Multitask Learning on Tabular Data](https://arxiv.org/abs/2511.09970)
*Dimitrios Sinodinos,Jack Yi Wei,Narges Armanfard*

Main category: cs.LG

TL;DR: 提出了MultiTab-Net，首个专为大型表格数据设计的多任务transformer架构，通过新颖的多任务掩码注意力机制动态建模特征依赖并缓解任务竞争，在多个领域表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 表格数据是最丰富的数据类型，但现有多任务学习方法主要关注推荐系统，在更广泛的表格领域潜力未充分挖掘，且现有基于多层感知机的方法难以捕捉复杂特征交互。

Method: 设计了MultiTab-Net多任务transformer架构，采用多任务掩码注意力机制动态建模特征-特征依赖关系，同时减轻任务竞争。

Result: 在包括大规模推荐数据、人口普查类社会经济数据和物理数据集等多个领域中，MultiTab-Net始终比现有MTL架构和单任务transformer获得更高的多任务增益。

Conclusion: MultiTab-Net是首个专为大型表格数据设计的多任务transformer架构，能有效提升多任务泛化性能，并贡献了MultiTab-Bench基准数据集生成器用于系统评估。
...
### [133] [Rediscovering the Lunar Equation of the Centre with AI Feynman via Embedded Physical Biases](https://arxiv.org/abs/2511.09979)
*Saumya Shah,Zi-Yu Khoo,Abel Yang,Stéphane Bressan*

Main category: cs.LG

TL;DR: 使用AI Feynman符号回归算法自动重新发现天文学中的基本方程——中心方程，通过数据预处理和搜索空间限制引入物理系统的观测和归纳偏差，成功从月球星历数据中恢复该方程的一阶解析形式。


<details>
  <summary>Details</summary>
Motivation: 探索如何通过物理启发的符号回归算法自动重新发现天文学基本方程，并研究领域知识嵌入对符号回归的影响。
...
### [134] [A Novel Data-Dependent Learning Paradigm for Large Hypothesis Classes](https://arxiv.org/abs/2511.09996)
*Alireza F. Pour,Shai Ben-David*

Main category: cs.LG

TL;DR: 提出了一种新的学习范式，通过更充分地利用经验数据，在候选模型集过大时避免依赖先验假设，适用于多种常见学习假设场景。


<details>
  <summary>Details</summary>
Motivation: 解决候选模型集过大时经验估计无法一致收敛到真实损失的问题，减少对先验假设的依赖。

Method: 提出新的学习范式，强调经验数据的更强整合，避免基于先验假设的算法决策。

Result: 分析了该方法的泛化能力，在多种学习假设下验证了其有效性，包括邻近点相似性、标签同质区域聚类、标记规则的Lipschitz性和对比学习假设。

Conclusion: 该方法能够在不知道真实参数的情况下有效利用各种学习假设，提供了一种更数据驱动的学习框架。
...
### [135] [Temporal Latent Variable Structural Causal Model for Causal Discovery under External Interferences](https://arxiv.org/abs/2511.10031)
*Ruichu Cai,Xiaokai Huang,Wei Chen,Zijian Li,Zhifeng Hao*

Main category: cs.LG

TL;DR: 提出了一种新的时序潜变量结构因果模型，通过引入潜变量表示未观测的外部干扰因素，并开发了基于变分推理的参数学习方法，能够结合专家先验知识来指导模型估计。


<details>
  <summary>Details</summary>
Motivation: 从观测数据推断因果关系是一个重要任务，但当数据受到各种外部干扰时变得困难。这些干扰通常是外部因素对观测变量的额外影响，而外部因素往往未知。

Method: 提出时序潜变量结构因果模型，包含表示变量间因果关系的因果强度和邻接系数。基于变分推理开发参数学习方法，能够结合专家先验知识。

Result: 实验结果表明所提方法具有稳定性和准确性。

Conclusion: 通过引入潜变量表示未观测外部因素，并结合专家知识，提出的方法能够有效处理受外部干扰的因果推断问题。
...
### [136] [From Static Structures to Ensembles: Studying and Harnessing Protein Structure Tokenization](https://arxiv.org/abs/2511.10056)
*Zijing Liu,Bin Feng,He Cao,Yu Li*

Main category: cs.LG

TL;DR: 本文研究发现蛋白质结构标记化中的语义冗余可被利用，通过简单的"同义词交换"策略生成多样化构象集合，该方法在计算上轻量且能准确捕捉蛋白质灵活性。


<details>
  <summary>Details</summary>
Motivation: 尽管已有许多蛋白质结构标记化的工作，但对底层离散表示的特性理解不足。本文旨在深入理解结构标记的语义特性，并探索如何利用这些特性来建模蛋白质动力学。

Method: 首先分析结构词汇表的语义冗余特性，发现多个不同标记对应几乎相同的局部几何结构（"结构同义词"）。然后提出"同义词交换"策略，通过用结构同义词扰动预测结构来生成多样化构象集合。

Result: 该方法能准确重现蛋白质灵活性，与最先进模型表现相当。计算上非常轻量，可实现近乎即时的蛋白质动力学建模。
...
### [137] [eXIAA: eXplainable Injections for Adversarial Attack](https://arxiv.org/abs/2511.10088)
*Leonardo Pesce,Jiawen Wei,Gianmarco Mengaldo*

Main category: cs.LG

TL;DR: 本文提出了一种新的黑盒模型无关对抗攻击方法，针对图像领域的后验可解释AI方法，能够在保持预测类别不变的情况下显著改变解释结果，而无需访问模型内部权重。


<details>
  <summary>Details</summary>
Motivation: 当前后验可解释性方法在安全关键应用中的可靠性存在隐患，需要揭示其潜在漏洞。

Method: 基于模型预测和解释结果，在单步内生成对抗样本，针对多种解释方法（显著性图、积分梯度、DeepLIFT SHAP）进行攻击。

Result: 攻击能够在不改变预测概率的情况下显著改变解释结果，通过平均绝对差和结构相似性指数验证了攻击的有效性。
...
### [138] [T2IBias: Uncovering Societal Bias Encoded in the Latent Space of Text-to-Image Generative Models](https://arxiv.org/abs/2511.10089)
*Abu Sufian,Cosimo Distante,Marco Leo,Hanan Salam*

Main category: cs.LG

TL;DR: 本研究分析了5个主流开源文本到图像生成模型，发现它们都系统性地编码和放大了种族与性别刻板印象，如护理职业女性化、高管职业男性化和白人主导等现象。
...
Method: 对5个最流行的开源模型进行实证研究，使用10个中性职业相关提示生成每个职业100张图像，共5000张图像由代表不同种族和性别的多样化人类评估者评估。

Result: 所有五个模型都编码并放大了明显的社会偏见：护理和护理角色持续女性化，而企业CEO、政治家、医生和律师等高地位职业绝大多数由男性和主要是白人代表。还识别了模型特定模式，如QWEN-Image几乎只关注东亚输出，Kandinsky的白人主导，SDXL相对更广泛但仍偏见的分布。

Conclusion: 这些结果为AI项目经理和从业者提供了关键见解，使他们能够选择公平的AI模型和定制提示，生成符合负责任AI原则的图像。讨论了这些偏见风险，并提出了在构建负责任GenAI系统中偏见缓解的可操作策略。
...
### [139] [How does My Model Fail? Automatic Identification and Interpretation of Physical Plausibility Failure Modes with Matryoshka Transcoders](https://arxiv.org/abs/2511.10094)
*Yiming Tang,Abhijeet Sinha,Dianbo Liu*

Main category: cs.LG

TL;DR: 本文提出了Matryoshka Transcoders框架，用于自动发现和解释生成模型中的物理合理性特征，通过分层稀疏特征学习和多模态模型解释，识别物理相关的失败模式，并建立评估基准。


<details>
  <summary>Details</summary>
Motivation: 尽管现代生成模型能产生逼真输出，但仍存在物理合理性错误，且现有评估方法难以检测这些错误，缺乏自动识别和解释物理错误模式的框架。
...
Conclusion: 该框架为生成模型的物理合理性评估提供了基准，分析八个最先进生成模型揭示了它们如何违反物理约束，为模型改进铺平了道路。
...
### [140] [RI-Loss: A Learnable Residual-Informed Loss for Time Series Forecasting](https://arxiv.org/abs/2511.10130)
*Jieting Wang,Xiaolei Shang,Feijiang Li,Furong Peng*

Main category: cs.LG

TL;DR: 该论文提出了一种基于希尔伯特-施密特独立准则的残差感知损失函数，用于解决时间序列预测中MSE损失的两个根本弱点：无法捕捉时间关系和忽略数据固有噪声。
...
Method: 引入残差感知损失函数，通过强制残差序列与随机时间序列之间的依赖性来显式建模噪声结构，实现更鲁棒的噪声感知表示。

Result: 在八个真实世界基准测试和五个领先预测模型上的实验表明预测性能得到改善，验证了方法的有效性。

Conclusion: RI-Loss方法通过显式建模噪声结构，在理论和实证上都证明了其在时间序列预测中的有效性，为噪声感知表示提供了严格的理论保证。
...
### [141] [EPO: Diverse and Realistic Protein Ensemble Generation via Energy Preference Optimization](https://arxiv.org/abs/2511.10165)
...
<details>
  <summary>Details</summary>
Motivation: 分子动力学模拟在探索蛋白质构象集成时面临高计算成本和能量势垒陷阱问题，需要开发更高效的采样方法。

Method: EPO利用随机微分方程采样探索构象空间，基于列表偏好优化引入新颖的能量排序机制，并提出实用上界来近似连续时间生成模型中难以处理的采样轨迹概率。

Result: 在Tetrapeptides、ATLAS和Fast-Folding基准测试中，EPO在九个评估指标上建立了新的最先进水平，成功生成多样化且物理真实的构象集成。

Conclusion: 仅使用能量偏好信号就能有效引导生成模型产生热力学一致的构象集成，为长分子动力学模拟提供了替代方案，扩展了学习势能在结构生物学和药物发现中的应用范围。
...
### [142] [Towards Leveraging Sequential Structure in Animal Vocalizations](https://arxiv.org/abs/2511.10190)
*Eklavya Sarkar,Mathew Magimai. -Doss*

Main category: cs.LG

TL;DR: 本文研究了如何通过向量量化和Gumbel-softmax向量量化从自监督语音模型中提取离散声学标记序列，以有效捕捉和利用动物发声中的时序信息。


<details>
  <summary>Details</summary>
Motivation: 大多数计算生物声学研究在提取帧级特征时会跨时间轴进行平均，丢弃了发声内部子单元的顺序信息。本文旨在探索是否可以通过离散声学标记序列来有效利用动物发声中的时序结构。

Method: 使用HuBERT嵌入生成离散声学标记序列，通过向量量化和Gumbel-softmax向量量化方法，然后使用基于Levenshtein距离的k-最近邻算法进行序列分类实验。

Result: 对四个生物声学数据集的标记序列进行成对距离分析表明，这些序列能够区分呼叫类型和呼叫者。序列分类实验显示向量量化标记序列在呼叫类型和呼叫者分类方面表现良好。

Conclusion: 向量量化的标记序列作为替代特征表示具有潜力，能够有效利用动物发声中的顺序信息。
...
### [143] [Beyond MSE: Ordinal Cross-Entropy for Probabilistic Time Series Forecasting](https://arxiv.org/abs/2511.10200)
*Jieting Wang,Huimei Shi,Feijiang Li,Xiaolei Shang*

Main category: cs.LG

TL;DR: OCE-TS是一种新颖的时间序列预测方法，使用序数交叉熵损失替代传统的均方误差损失，通过概率输出量化不确定性并保持预测值的序数关系。
...
Method: 将观测值离散化为有序区间，通过参数分布推导概率作为监督信号，使用线性模型预测每个时间步的概率分布，计算预测和真实概率的累积分布之间的序数交叉熵损失。

Result: 在7个公开时间序列数据集上与5个基准模型比较，使用MSE和MAE评估，OCE-TS始终优于基准模型。
...
<details>
  <summary>Details</summary>
Motivation: 受生物注意力的多尺度动态和动力系统理论启发，旨在理解和扩展自注意力机制的基本原理，为人工智能发展提供神经科学基础。

Method: 引入分数神经注意力（FNA）框架，使用分数拉普拉斯算子控制的Lévy扩散来建模token交互，实现同时捕获短程和长程依赖关系。

Result: FNA在文本分类中即使使用单层单头也能达到竞争性性能，在图像处理和神经机器翻译中提升性能，并可通过扩散映射算法进行降维。

Conclusion: FNA建立了自注意力、随机动力学和几何学之间的原则性联系，为强大的神经科学启发AI提供了可解释的生物学基础。
...
### [145] [Out-of-Context Misinformation Detection via Variational Domain-Invariant Learning with Test-Time Training](https://arxiv.org/abs/2511.10213)
...
<details>
  <summary>Details</summary>
Motivation: 当前OOC虚假信息检测方法假设训练和测试数据来自相同分布，但在遇到新新闻领域时性能下降。需要解决模型在未知领域适应能力不足的问题。

Method: 提出VDT方法，包含领域不变变分对齐模块学习领域不变特征，领域一致性约束模块保持语义完整性，测试时采用测试时训练策略和置信度-方差过滤模块动态更新模型。

Result: 在NewsCLIPpings数据集上的广泛实验表明，该方法在大多数领域适应设置下优于最先进的基线方法。

Conclusion: VDT方法通过领域不变特征学习和测试时训练机制有效提升了OOC虚假信息检测的领域适应能力，在跨领域场景中表现优异。
...
### [146] [FedCure: Mitigating Participation Bias in Semi-Asynchronous Federated Learning with Non-IID Data](https://arxiv.org/abs/2511.10227)
...
<details>
  <summary>Details</summary>
Motivation: 半异步联邦学习虽然结合了同步训练效率和异步更新的灵活性，但存在参与偏差问题，特别是在非IID数据分布下更为严重。分层架构将参与从单个客户端转移到客户端组，进一步加剧了这一问题。现有研究大多关注传统云-端架构，忽视了非IID数据在云-边-客户端层次结构中对调度的关键影响。
...
Conclusion: FedCure通过联盟构建和参与感知调度有效缓解了非IID数据下的参与偏差问题，在准确性和效率方面都表现出显著优势，为分层联邦学习架构提供了有效的解决方案。
...
### [147] [Lost in Serialization: Invariance and Generalization of LLM Graph Reasoners](https://arxiv.org/abs/2511.10234)
*Daniel Herbst,Lea Karbeska,Divyanshu Kumar,Akanksha Ahuja,Fatemeh Gholamzadeh Nasrabadi,Fabrizio Frasca*

Main category: cs.LG

TL;DR: 本文分析了基于大语言模型的图推理器在图表示对称性方面缺乏内在不变性的问题，系统研究了微调对编码敏感性和未见任务泛化能力的影响，并提出了图序列化的分解方法。
...
Method: 提出了将图序列化分解为节点标记、边编码和语法的原则性分解方法，并在综合基准测试套件上评估大语言模型对这些因素变化的鲁棒性。还贡献了一组新的谱任务来进一步评估微调推理器的泛化能力。

Result: 结果显示，更大的（未经微调的）模型更加鲁棒。微调降低了节点重标记的敏感性，但可能增加对结构和格式变化的敏感性，同时并未一致地提高在未见任务上的性能。

Conclusion: 大语言模型在图推理任务中对图表示对称性缺乏鲁棒性，微调虽然能改善某些方面的敏感性，但可能在其他方面产生负面影响，且不能保证在未见任务上的泛化能力提升。
...
### [148] [Heuristic Transformer: Belief Augmented In-Context Reinforcement Learning](https://arxiv.org/abs/2511.10251)
...
<details>
  <summary>Details</summary>
Motivation: 现有transformer在上下文学习中表现出色，但在强化学习领域，如何更好地利用上下文信息进行决策仍有改进空间。本文旨在通过引入奖励信念分布来增强transformer的决策能力。

Method: 提出Heuristic Transformer (HT)方法，使用变分自编码器学习低维随机变量来表示奖励的后验分布，将该分布与上下文数据集和查询状态一起作为transformer策略的输入提示。

Result: 在Darkroom、Miniworld和MuJoCo环境中测试表明，HT在效果和泛化能力方面持续优于可比基线方法。
...
### [149] [Unitho: A Unified Multi-Task Framework for Computational Lithography](https://arxiv.org/abs/2511.10255)
...
<details>
  <summary>Details</summary>
Motivation: 计算光刻中可靠、可泛化的数据基础对于大规模模型至关重要，但关键任务（掩模生成、规则违规检测和布局优化）通常孤立处理，受到数据集稀缺和建模方法有限的阻碍。
...
### [150] [PITE: Multi-Prototype Alignment for Individual Treatment Effect Estimation](https://arxiv.org/abs/2511.10320)
*Fuyuan Cao,Jiaxuan Zhang,Xiaoli Li*

Main category: cs.LG

TL;DR: PITE是一种用于个体治疗效果估计的多原型对齐方法，通过捕捉局部群体结构并进行跨组对齐，解决了传统方法忽略个体异质性和局部结构的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在解决混杂偏差时通常采用全局分布平衡，但忽略了个体异质性和代表个体自然聚类的局部结构，这损害了ITE估计的准确性。
...
Conclusion: PITE通过细粒度的原型级对齐减少分布偏移，同时保留处理和对照组的局部结构，为ITE估计提供了有意义的约束。
...
### [151] [EDGC: Entropy-driven Dynamic Gradient Compression for Efficient LLM Training](https://arxiv.org/abs/2511.10333)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型训练面临计算资源和内存容量的挑战，现有静态梯度压缩方法忽略了训练过程中梯度的动态变化特性，导致性能下降，需要在保持性能的同时提升通信效率。

Method: EDGC框架包含三个关键组件：1）下采样方法高效估计梯度熵；2）建立压缩率与梯度熵的理论模型；3）基于窗口的调整机制动态适应管道阶段的压缩率。
...
### [152] [Robust Decentralized Multi-armed Bandits: From Corruption-Resilience to Byzantine-Resilience](https://arxiv.org/abs/2511.10344)
*Zicheng Hu,Yuchen Wang,Cheng Chen*

Main category: cs.LG

TL;DR: 本文研究了去中心化合作多智能体多臂老虎机问题中的对抗攻击鲁棒性，提出了DeMABAR算法，能在对抗性腐败和拜占庭攻击下保持良好性能。


<details>
  <summary>Details</summary>
Motivation: 现有的去中心化多智能体多臂老虎机方法对各类对抗攻击缺乏鲁棒性，需要设计能够抵御对抗性腐败和拜占庭攻击的算法。

Method: 提出DeMABAR算法，通过设计鲁棒的协作机制，在对抗性腐败和拜占庭攻击下仍能有效工作。

Result: 理论分析表明DeMABAR算法在对抗性腐败下仅产生与腐败预算成正比的附加遗憾，在拜占庭设置下几乎完全消除攻击影响。数值实验验证了方法的鲁棒性和有效性。

Conclusion: DeMABAR算法为去中心化多智能体多臂老虎机问题提供了对抗攻击的鲁棒解决方案，在多种攻击场景下均表现出色。
...
### [153] [Product distribution learning with imperfect advice](https://arxiv.org/abs/2511.10366)
*Arnab Bhattacharyya,Davin Choo,Philips George John,Themis Gouleakis*

Main category: cs.LG

TL;DR: 本文研究了在有先验知识（另一个产品分布的参数）的情况下，从布尔超立方体上的产品分布中学习未知分布的问题。当先验分布与目标分布的均值向量足够接近时，可以显著减少样本复杂度。


<details>
  <summary>Details</summary>
Motivation: 传统的分布学习需要Ω(d/ε²)个样本来学习布尔超立方体上的产品分布。本文探索在有先验知识（另一个产品分布的参数）的情况下，是否能减少样本复杂度。

Method: 提出了一种高效的算法，当给定一个先验产品分布Q的参数时，如果Q与目标分布P的均值向量在ℓ₁距离上足够接近（‖p-q‖₁ < εd⁰·⁵⁻Ω(η)），则可以用更少的样本学习P。

Result: 算法实现了样本复杂度为Õ(d¹⁻η/ε²)，这比传统方法所需的Ω(d/ε²)样本有显著改进，前提是先验分布与目标分布足够接近。

Conclusion: 在有合适的先验知识的情况下，可以显著减少学习产品分布所需的样本数量，这为分布学习提供了新的理论洞见。
...
### [154] [Enhancing Kernel Power K-means: Scalable and Robust Clustering with Random Fourier Features and Possibilistic Method](https://arxiv.org/abs/2511.10392)
*Yixi Chen,Weixuan Liang,Tianrui Liu,Jun-Jie Huang,Ao Li,Xueling Zhu,Xinwang Liu*

Main category: cs.LG

TL;DR: 本文提出了RFF-KPKM和IP-RFF-MKPKM两种方法，通过随机傅里叶特征解决核幂k均值在大规模数据上的计算瓶颈和噪声鲁棒性问题。


<details>
  <summary>Details</summary>
Motivation: 核幂k均值存在两个关键限制：(1)完整核矩阵的计算负担限制了其在大规模数据上的应用；(2)缺乏真实的质心-样本分配学习降低了其对噪声的鲁棒性。
...
Conclusion: 提出的方法成功解决了核幂k均值的计算可扩展性和噪声鲁棒性问题，并提供了理论保证和实验验证。
...
### [155] [AgentEvolver: Towards Efficient Self-Evolving Agent System](https://arxiv.org/abs/2511.10395)
...
TL;DR: AgentEvolver是一个自主进化的智能体系统，利用大语言模型的语义理解和推理能力，通过自我提问、自我导航和自我归因三个协同机制，实现高效、低成本的智能体学习。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的自主智能体开发方法成本高、效率低，需要手动构建任务数据集和强化学习流程，存在数据构建成本高、探索效率低和样本利用率差的问题。

Method: 提出AgentEvolver系统，包含三个协同机制：自我提问实现好奇心驱动的任务生成，减少对手工数据集的依赖；自我导航通过经验重用和混合策略指导提高探索效率；自我归因通过为轨迹状态和动作分配差异化奖励来增强样本效率。

Result: 初步实验表明，AgentEvolver相比传统的基于强化学习的基线方法，实现了更高效的探索、更好的样本利用率和更快的适应能力。

Conclusion: AgentEvolver通过整合三个自我进化机制，为智能体能力的可扩展、成本效益和持续改进提供了一个统一的框架。
...
### [156] [Unlocking Dynamic Inter-Client Spatial Dependencies: A Federated Spatio-Temporal Graph Learning Method for Traffic Flow Forecasting](https://arxiv.org/abs/2511.10434)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界交通数据分布在多个利益相关者之间，现有方法主要处理静态依赖关系，忽略了动态特性，导致性能不佳。需要在不违反数据本地化约束的情况下建模客户端间动态空间依赖。
...
Conclusion: FedSTGD能够有效建模和重建联邦学习环境下的动态客户端间空间依赖关系，在保护数据隐私的同时实现高性能，对超参数变化具有鲁棒性。
...
### [157] [Improving Perturbation-based Explanations by Understanding the Role of Uncertainty Calibration](https://arxiv.org/abs/2511.10439)
*Thomas Decker,Volker Tresp,Florian Buettner*

Main category: cs.LG

TL;DR: 本文研究了不确定性校准与基于扰动的解释之间的关系，发现模型在解释性特定扰动下会产生不可靠的概率估计，这直接影响了全局和局部解释质量。作者提出了ReCalX方法来重新校准模型以改进解释，同时保持原始预测。


<details>
  <summary>Details</summary>
Motivation: 基于扰动的解释在实践中被广泛用于提高机器学习模型的透明度，但其可靠性常因模型在特定扰动下的未知行为而受损。本文旨在探索不确定性校准与扰动解释之间的关系。

Method: 引入ReCalX方法，重新校准模型以改进解释质量，同时保持模型的原始预测不变。该方法专门针对解释性特定扰动进行优化。

Result: 跨多种模型和数据集的实证评估表明，ReCalX能最有效地减少扰动特定的校准错误，同时提高解释的鲁棒性和全局重要输入特征的识别能力。

Conclusion: 模型在解释性特定扰动下会产生系统性的不可靠概率估计，这直接损害了解释质量。ReCalX方法能有效解决这一问题，在保持预测性能的同时显著提升解释的可靠性。
...
### [158] [Intrinsic Dimensionality as a Model-Free Measure of Class Imbalance](https://arxiv.org/abs/2511.10475)
*Çağrı Eser,Zeynep Sonat Baltacı,Emre Akbaş,Sinan Kalkan*

Main category: cs.LG

TL;DR: 本文提出使用数据内在维度（ID）作为易于计算、无需模型训练的类别不平衡度量方法，替代传统的基于样本数量的不平衡度量，并在多种不平衡数据集上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统基于样本数量的不平衡度量方法忽略了冗余样本和类别学习难度的差异，而基于训练损失和不确定性的复杂度量方法需要训练机器学习模型。本文旨在寻找一种易于计算且无需模型训练的不平衡度量方法。

Method: 使用数据内在维度（ID）作为类别不平衡的度量，并将其无缝集成到各种不平衡缓解方法中。同时探索将ID与样本数量相结合的方法。

Result: 在五个不同不平衡比率的数据集上的实验结果表明，基于ID的方法始终优于文献中基于样本数量的重新加权和重采样技术。将ID与样本数量结合可以进一步提高性能。

Conclusion: 数据内在维度是一种有效的不平衡度量方法，可以替代传统的基于样本数量的方法，并且与样本数量结合使用时效果更佳。
...
### [159] [Panda: Test-Time Adaptation with Negative Data Augmentation](https://arxiv.org/abs/2511.10481)
*Ruxi Deng,Wenxuan Bao,Tianxin Wei,Jingrui He*

Main category: cs.LG

TL;DR: Panda是一种基于负数据增强的测试时自适应方法，通过生成破坏语义内容的负样本来抑制图像损坏相关特征，从而提升预训练视觉语言模型在损坏图像上的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的测试时自适应方法使用正数据增强存在两个主要问题：计算开销大（每张图像需要大量增强）和无法缓解预测偏差（模型在损坏下倾向于预测某些类别）。

Method: Panda采用负数据增强，将图像分割成小块并从共享块池中随机重组，生成保留损坏特征但破坏语义内容的负样本，然后从原始图像特征中减去负样本的平均特征来抑制损坏相关成分。

Result: 实验表明Panda相比正数据增强方法性能更优，且能显著提升多种测试时自适应方法的性能，同时计算开销极小。

Conclusion: Panda是一种高效且有效的测试时自适应方法，能够通过负数据增强抑制图像损坏的影响，提升模型在分布偏移下的鲁棒性。
...
### [160] [Holonorm](https://arxiv.org/abs/2511.10504)
*Daryl Noupa Yongueng,Hamidou Tembine*

Main category: cs.LG

TL;DR: 本文提出了一种名为Holonorm的新型归一化方法，用于替代Transformer中的层归一化和Tanh归一化。Holonorm通过残差连接和非线性特性解决了Tanh存在的正交性、线性和失真问题，同时保持了信号的正交性、方向和可逆性。


<details>
  <summary>Details</summary>
Motivation: 现有的Tanh归一化方法存在正交性、线性和失真问题，导致其可靠性不足。作者旨在开发一种更可靠的归一化方法来替代Tanh，同时解决深度Transformer模型中的激活爆炸和稳定性问题。

Method: 提出Holonorm归一化方法，该方法结合了残差连接和非线性特性。Holonorm将向量映射到开放单位球内，防止激活爆炸，并保持信号的正交性、方向和可逆性。虽然在一维情况下与softsign函数相似，但Holonorm更适合高维张量和向量。

Result: Holonorm被证明是一种有效的归一化方法，能够替代层归一化和Tanh归一化。它解决了Tanh的正交性、线性和失真问题，同时提高了深度Transformer模型的稳定性。

Conclusion: Holonorm是一种广义的softsign函数形式，适合作为归一化函数。其值在0到1之间，便于理解和评估模型性能，是Transformer训练中归一化的可靠替代方案。
...
### [161] [Maximizing Efficiency of Dataset Compression for Machine Learning Potentials With Information Theory](https://arxiv.org/abs/2511.10561)
*Benjamin Yu,Vincenzo Lordi,Daniel Schwalbe-Koda*

Main category: cs.LG

TL;DR: 提出了一种基于信息论的最小集合覆盖算法，用于高效压缩原子数据集，在保持多样性和异常值的同时显著减少数据量，提升机器学习原子间势能的性能。


<details>
  <summary>Details</summary>
Motivation: 机器学习原子间势能在准确性和计算成本之间取得平衡，但训练数据集的大小和多样性影响其性能。大数据集计算昂贵，小数据集可能丢失重要原子环境信息。

Method: 将原子数据集压缩建模为原子中心环境的最小集合覆盖问题，开发信息论框架量化压缩效率，提出最大化效率的算法。

Result: 在GAP-20、TM23和ColabFit的64个数据集上验证，MSC方法在高压缩率下保持异常值、数据集多样性和力的长尾分布，优于其他子采样方法。

Conclusion: MSC压缩的数据集训练出的MLIP在分布外数据上误差更小，算法已开源实现，可用于原子建模中的数据子采样、异常检测和低成本训练改进的MLIP。
...
### [162] [Impact of Layer Norm on Memorization and Generalization in Transformers](https://arxiv.org/abs/2511.10566)
*Rishi Singhal,Jung-Eun Kim*

Main category: cs.LG

TL;DR: 本文研究了LayerNorm在Pre-LayerNorm和Post-LayerNorm两种transformer架构中对学习和记忆的影响差异，发现LayerNorm在Pre-LayerNorm模型中稳定学习，而在Post-LayerNorm模型中影响记忆。


<details>
  <summary>Details</summary>
Motivation: 虽然Pre-LayerNorm已成为transformer的首选架构，但LayerNorm在不同架构中对学习和记忆的具体影响尚不清楚，需要系统研究。

Method: 通过分析Pre-LayerNorm和Post-LayerNorm模型中LayerNorm的作用，特别关注消除LayerNorm参数的影响，并在6个视觉和语言数据集的13个模型上进行验证。

Result: 在Pre-LayerNorm模型中移除LayerNorm参数会加剧记忆并破坏学习稳定性，而在Post-LayerNorm模型中移除LayerNorm参数能有效减轻记忆并恢复真实标签。早期层的LayerNorm最为关键。

Conclusion: LayerNorm在不同transformer架构中扮演不同角色：在Pre-LayerNorm中稳定学习，在Post-LayerNorm中影响记忆，这为理解transformer中LayerNorm的作用提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有基于静态规则或参与度最大化的决策系统忽视用户情感背景和伦理约束的问题，避免在心理健康、药物滥用等敏感领域推荐不恰当或危险的干预措施。

Method: 将个性化问题建模为约束马尔可夫决策过程，设计多目标奖励函数平衡短期行为参与和长期用户福祉，使用情感感知状态表示捕捉情绪准备度、情感和风险波动。

Result: 提出了一个可实例化的架构，能够与任何强化学习算法结合，通过安全约束或拉格朗日正则化实现情感对齐和伦理安全。

Conclusion: 该框架在机器学习策略优化中实现了同理心和责任的操作化，为行为健康、教育和数字治疗等领域的人本系统提供了理论基础和验证路径。
...
TL;DR: 提出了一种半统一的稀疏字典学习框架，将经典稀疏模型与现代深度架构相结合，通过集成严格Top-K LISTA及其凸变体到LC-KSVD2模型中，实现稀疏编码器和字典在监督或无监督机制下的协同演化。
...
TL;DR: 本文提出了一种针对ℓ₀攻击的线性边界传播方法，通过将ℓ₀球的凸包近似为边界框与不对称缩放ℓ₁多面体的交集，显著提升了现有ℓ₀验证器的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的局部鲁棒性验证器主要依赖线性边界传播来处理凸扰动空间，但ℓ₀球是非凸的，这限制了验证器在少像素攻击场景下的有效性。

Method: 首先证明ℓ₀球的凸包是其边界框与不对称缩放ℓ₁多面体的交集，然后提出一种精确计算凸包边界的线性边界传播方法。

Result: 该方法在最具挑战性的鲁棒性基准测试中将最先进的ℓ₀验证器性能提升了1.24倍到7.07倍，几何平均提升为3.16倍。

Conclusion: 通过将ℓ₀球的非凸扰动空间近似为凸包，并开发相应的线性边界传播方法，可以有效提升ℓ₀攻击验证的效率和精度。
...
### [166] [An Efficient and Almost Optimal Solver for the Joint Routing-Assignment Problem via Partial JRA and Large-α Optimization](https://arxiv.org/abs/2511.09563)
*Qilong Yuan*

Main category: cs.AI

TL;DR: 本文提出了一种新颖高效的Partial Path Reconstruction (PPR)求解器来解决联合路由分配(JRA)优化问题，通过识别关键项目-占位符对形成简化子问题，结合全局大α约束，在保持计算效率的同时实现了近乎最优的解决方案。


<details>
  <summary>Details</summary>
Motivation: 现有精确混合整数规划方法虽然能保证最优性，但在大规模实例上计算效率低下；而启发式方法虽然高效但存在约1%的偏差。需要开发一种既能保持高效率又能获得高精度解的新方法。

Method: 提出PPR求解器框架，首先识别关键项目-占位符对形成简化子问题，然后高效求解以优化全局解；结合全局大α约束增强最优性；采用迭代优化路径进行解的精炼。

Result: 在n=300、500和1000的基准数据集上测试，平均偏差达到0.00%，相比现有启发式方法偏差减少一半，同时保持高计算效率。
...
### [167] [Variable Neighborhood Search for the Electric Vehicle Routing Problem](https://arxiv.org/abs/2511.09570)
...
<details>
  <summary>Details</summary>
Motivation: 由于电动汽车在物流中的日益普及，电动汽车路径问题扩展了经典车辆路径问题。文献中考虑的各种约束使得比较不同问题变体的方法具有挑战性。

Method: 使用变邻域搜索元启发式算法来解决最小化的电动汽车路径问题变体——容量约束绿色车辆路径问题。

Result: 该方法在完整的竞赛数据集上取得了最佳结果，并且优于后来发布的更近期的算法。

Conclusion: 基于变邻域搜索的方法在解决容量约束绿色车辆路径问题方面表现出色，在竞赛中获胜并超越了后续算法。
...
TL;DR: 该论文探讨了语言模型与基于逻辑的知识表示在推理能力方面的融合，旨在分析语言模型的推理能力、注入基于知识的推理能力，并形式化语言模型的推理过程。


<details>
  <summary>Details</summary>
Motivation: 随着基于transformer的语言模型在自然语言处理领域的快速发展，研究者希望探索这些模型是否真正具备推理能力，以及如何将其与传统的基于逻辑的推理方法相结合，以提高在需要精确性和可靠性的应用中的实用性。

Method: 通过跨学科研究平台，分析语言模型的推理能力与知识表示方法的对比，使用神经符号方法向语言模型注入基于知识的推理能力，并形式化语言模型执行的推理类型。

Result: 该研究旨在揭示语言模型如何有效整合和利用知识与推理，从而在精度和可靠性要求高的领域改进其应用和效用。

Conclusion: 通过融合语言模型和基于逻辑的推理方法，可以提升AI系统的推理能力，使其在关键应用中更加可靠和精确。
...
<details>
  <summary>Details</summary>
Motivation: 探索两种非可接受论证框架语义之间的理论关系，澄清它们之间的包含关系。

Method: 通过理论证明和逻辑分析，建立cogent语义和弱可接受语义之间的数学关系。

Result: 证明了cogent扩展必然是弱可接受的，但存在弱可接受扩展不是cogent的情况。

Conclusion: cogent语义是弱可接受语义的真子集，这两种语义之间存在严格的不对称包含关系。
...
TL;DR: 本文研究了音频推理模型（ARMs）的安全性，发现标准推理训练可以防御基础音频越狱攻击，但无法抵御更高级的攻击。作者提出了Rebellion方法，通过训练ARMs对最坏情况下的表示漂移具有鲁棒性，显著提高了模型的安全性和性能平衡。


<details>
  <summary>Details</summary>
Motivation: 随着音频推理模型（ARMs）的流行，目前尚无研究关注其对抗越狱攻击的安全性。作者旨在填补这一空白，探索ARMs在面对音频越狱攻击时的脆弱性。

Method: 作者首先展示了标准推理训练对基础音频越狱攻击的防御效果，然后提出了更高级的音频越狱攻击方法。基于表示漂移的观察，提出了Rebellion——一种鲁棒的推理训练方法，训练ARMs对最坏情况下的表示漂移具有鲁棒性。

Result: 在Qwen2-Audio上的实验结果表明：1）Rebellion能够有效防御高级音频越狱攻击，同时不影响良性任务的性能；2）与标准推理训练方法相比，Rebellion显著改善了准确性-安全性的权衡。

Conclusion: Rebellion方法为音频推理模型提供了一种有效的安全防护机制，能够在保持模型性能的同时显著提升对抗越狱攻击的鲁棒性，为ARMs的安全部署提供了重要保障。
...
TL;DR: 论文研究了多智能体对话中的行为漂移现象，特别是回音效应——智能体放弃自身角色而模仿对话伙伴，这在不同LLM提供商中普遍存在且难以通过增强推理消除。
...
Method: 通过60种智能体配置、3个领域、2000+次对话的实验，研究了回音效应在不同LLM提供商中的表现，分析了提示影响和对话动态，并引入了结构化响应的协议级缓解方法。

Result: 回音效应在三大主要LLM提供商中普遍存在，发生率从5%到70%不等，即使在先进推理模型中仍保持32.8%的高发生率。回音效应随交互时间增长而出现（实验中7+轮次），且无法通过增强推理减少。

Conclusion: 智能体间对话中的回音效应是一个普遍且持久的问题，但可以通过结构化响应的协议级方法有效缓解，将回音率降至9%。
...
### [172] [AI Annotation Orchestration: Evaluating LLM verifiers to Improve the Quality of LLM Annotations in Learning Analytics](https://arxiv.org/abs/2511.09785)
*Bakhtawar Ahtisham,Kirk Vanacore,Jinsook Lee,Zhuqian Zhou,Doug Pietrzak,Rene F. Kizilcec*

Main category: cs.AI

TL;DR: 该研究测试了通过自我验证和交叉验证的编排策略来提高大语言模型在辅导对话标注中的可靠性，结果显示编排策略使Cohen's kappa提高了58%，其中自我验证的效果最佳。


<details>
  <summary>Details</summary>
Motivation: 大语言模型越来越多地用于标注学习互动，但可靠性问题限制了其实用性，需要验证导向的编排策略来提高标注质量。

Method: 使用30个一对一数学辅导对话，比较三种前沿大语言模型在未验证标注、自我验证和交叉验证三种条件下的表现，并与盲审的人类标注进行基准比较。

Result: 编排策略整体使kappa提高了58%，自我验证相对于未验证基线几乎翻倍了标注一致性，交叉验证平均提高了37%，但效果因验证者-标注者组合而异。

Conclusion: 验证策略是提高学习分析中大语言模型辅助标注可靠性和可扩展性的原则性设计杠杆。
...
### [173] [Why Open Small AI Models Matter for Interactive Art](https://arxiv.org/abs/2511.09788)
*Mar Canet Sola,Varvara Guljajeva*

Main category: cs.AI

TL;DR: 本文主张开源小型AI模型对互动艺术创作独立性的重要性，这些模型可本地部署，为艺术家提供对基础设施和代码的控制权，与大型闭源企业系统形成对比。


<details>
  <summary>Details</summary>
Motivation: 大型闭源AI系统作为不透明的黑箱，对互动艺术作品施加严重限制，包括内容过滤、保存问题和延迟等技术挑战。艺术家需要更多自主权和控制力来支持长期艺术创作。

Method: 通过使用开源小型AI模型，艺术家可以本地部署、自定义模型、修改代码集成新接口，或通过重新训练和微调来创建定制模型。

Result: 小型AI模型赋予创作者更多自主权、控制力和可持续性，支持技术自决，减少对不适合互动艺术需求的企业AI的依赖。

Conclusion: 开源小型AI模型方法赋能艺术家，支持包含AI组件的艺术作品的长期保存和展览，为互动艺术实践提供更可持续的技术基础。
...
TL;DR: 提出一种热激活对抗可穿戴设备，在普通黑色T恤中集成热致变色染料和柔性加热单元，通过加热激活隐藏的对抗图案，在可见光和红外模态下有效躲避AI监控系统检测。
...
Method: 将热致变色染料与柔性加热单元集成到服装表面，默认状态下服装呈普通黑色，加热后激活隐藏的对抗图案，实现用户可控的对抗效果。
...
TL;DR: EgoEMS是首个端到端、高保真、多模态、多人的EMS数据集，包含233个模拟紧急场景的20多小时数据，由62名参与者（包括46名EMS专业人员）完成，从第一人称视角捕捉真实的EMS活动。


<details>
  <summary>Details</summary>
Motivation: EMS急救人员在高压情况下面临巨大的认知负担，AI认知助手作为虚拟伙伴可以通过支持实时数据收集和决策来减轻这种负担。

Method: 开发了开源、低成本、可复制的数据收集系统，与EMS专家合作，按照国家标准创建数据集，包含关键步骤、时间戳音频转录、动作质量指标、边界框和分割掩码等注释。

Result: 创建了EgoEMS数据集，强调真实性，包含反映真实世界紧急动态的响应者-患者互动，并提供了用于实时多模态关键步骤识别和动作质量评估的基准套件。

Conclusion: EgoEMS数据集旨在激励研究社区推动智能EMS系统的边界，最终改善患者预后。
...
### [176] [Boosting In-Silicon Directed Evolution with Fine-Tuned Protein Language Model and Tree Search](https://arxiv.org/abs/2511.09900)
...
Result: 广泛的基准实验显示，AlphaDE即使使用少样本微调也能显著优于先前的最先进方法。案例研究还表明AlphaDE支持通过计算进化压缩蛋白质序列空间。

Conclusion: AlphaDE成功地将蛋白质语言模型的创新范式应用于蛋白质进化，为蛋白质工程提供了有效的计算工具。
...
### [177] [Learning to Pose Problems: Reasoning-Driven and Solver-Adaptive Data Synthesis for Large Reasoning Models](https://arxiv.org/abs/2511.09907)
*Yongxian Wei,Yilin Zhao,Li Shen,Xinrui Chen,Runxi Cheng,Sinan Du,Hao Yu,Gang Liu,Jiahong Yan,Chun Yuan,Dian Li*

Main category: cs.AI

TL;DR: 本文提出了一种能够明确推理并适应求解器能力的问题生成器，通过构建相关的问题对并增强推理模型生成的问题设计链式思维，从而引导问题生成方向并校准难度。
...
Method: 构建相关的问题对，并用推理模型生成的问题设计链式思维进行增强，这些数据引导生成器的问题设计策略。将求解器对合成问题的反馈作为奖励信号，使生成器能够校准难度并在求解器能力边缘产生补充性问题。

Result: 在10个数学和通用推理基准测试中，该方法平均提升2.5%，并能泛化到语言和视觉语言模型。在合成数据上训练的求解器为持续生成器训练提供改进的奖励，实现共同进化并带来额外0.7%的性能增益。
...
### [178] [OIDA-QA: A Multimodal Benchmark for Analyzing the Opioid Industry Documents Archive](https://arxiv.org/abs/2511.09914)
...
<details>
  <summary>Details</summary>
Motivation: 阿片类药物危机暴露了监管系统、医疗实践、企业治理和公共政策等多个系统的系统性缺陷。分析这些相互关联的系统如何同时失效需要创新的分析方法来处理UCSF-JHU阿片类药物行业文档档案中的大量数据和文件。
...
Conclusion: 通过构建专门针对医疗相关法律和企业文档的多模态分析系统，结合先进的数据处理方法和领域特定的AI模型，能够有效应对复杂医疗文档分析的挑战，为理解阿片类药物危机背后的系统性失败提供了有力的分析工具。
...
### [179] [Adaptive Hyperbolic Kernels: Modulated Embedding in de Branges-Rovnyak Spaces](https://arxiv.org/abs/2511.09921)
*Leping Si,Meimei Yang,Hui Xue,Shipeng Zhu,Pengfei Fang*

Main category: cs.AI

TL;DR: 本文提出了一种曲率感知的de Branges-Rovnyak空间和自适应双曲核函数，用于改进层次数据的表示学习。


<details>
  <summary>Details</summary>
Motivation: 双曲空间在嵌入层次结构方面具有优势，但现有的双曲核方法仍存在几何失真或缺乏适应性的问题。

Method: 构建曲率感知的de Branges-Rovnyak空间（与Poincare球等距的RKHS），设计可调节乘子来自适应选择对应任意曲率双曲空间的RKHS，并构建自适应双曲核函数族。

Result: 在视觉和语言基准测试上的广泛实验表明，所提出的核函数在建模层次依赖关系方面优于现有双曲核方法。

Conclusion: 通过曲率感知的RKHS和自适应双曲核函数，能够更有效地建模层次结构，提升双曲表示能力。
...
### [180] [SPAN: Benchmarking and Improving Cross-Calendar Temporal Reasoning of Large Language Models](https://arxiv.org/abs/2511.09993)
*Zhongjian Miao,Hao Fu,Chen Wei*

Main category: cs.AI

TL;DR: SPAN是一个跨日历时间推理基准，需要LLMs进行日历内时间推理和跨日历时间转换。该基准包含10个跨日历推理方向、2种推理类型和2种问题格式，涵盖6种日历。通过动态实例生成协议进行评估，发现现有LLMs平均准确率仅为34.5%，存在未来日期退化和日历不对称偏差问题。提出的Time Agent使用工具增强代码生成方法，准确率达到95.31%。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在跨日历时间推理方面存在显著不足，需要开发专门的基准来评估和提升其跨文化时间理解能力，以实现更具时间适应性和文化适应性的语言模型。

Method: 提出SPAN基准，采用模板驱动的动态实例生成协议，支持在指定公历日期上进行评估。开发Time Agent，利用工具增强的代码生成方法来提升跨日历时间推理能力。

Result: 在1960-2060年100年时间跨度上的实验显示，现有SOTA LLMs平均准确率仅34.5%，无一超过80%。Time Agent达到95.31%的准确率，显著优于其他基线方法。

Conclusion: 跨日历时间推理对现有LLMs仍具挑战性，工具增强的代码生成方法能有效提升此能力，为开发更具时间适应性和文化适应性的语言模型提供了方向。
...
TL;DR: 该研究评估了语言模型在危险化学品应急响应中的能力，开发了ChEmREF基准测试，包含1035种化学品，涵盖化学表示转换、应急响应生成和领域知识问答三个任务。


<details>
  <summary>Details</summary>
Motivation: 应急响应人员在处理危险化学品事故时需要快速可靠地获取关键信息、识别危险并提供建议，研究旨在评估当前语言模型是否能有效协助这一过程。

Method: 引入化学应急响应评估框架(ChEmREF)，包含1035种危险化学品数据，分为三个任务：化学表示转换、应急响应生成和领域知识问答。

Result: 最佳模型在非结构化化学表示转换任务中获得68.0%的精确匹配率，在事故响应建议任务中获得52.7%的LLM评分，在危险化学品考试中获得63.9%的多项选择准确率。
...
Method: 引入新颖的Planner模型进行全局有向无环图规划，开发ComplexTool-Plan大规模基准数据集，采用两阶段训练方法结合监督微调和组相对策略优化。
...
TL;DR: HPR框架通过强大的提示者模型在关键决策点提供概率指导，由高效的小型实践者模型执行主要推理步骤，使用分布不一致性减少指标动态识别干预点，在保持相似或更低计算成本的同时实现最先进的效率-准确率权衡。


<details>
  <summary>Details</summary>
Motivation: 当前推理时扩展方法通过穷尽采样产生过高计算成本，观察到大多数下一个标记预测与黄金输出一致，只有少数关键标记会导致偏差，因此需要更高效的推理框架。

Method: 提出Hint-Practice Reasoning框架，包含提示者（强大LLM）提供概率指导和实践者（高效小模型）执行推理步骤，使用分布不一致性减少指标动态识别干预点，在树结构概率空间中迭代更新推理路径。

Result: 在算术和常识推理基准测试中，HPR实现了与自一致性和MCTS基线相当的性能，同时仅解码1/5的标记，在保持相似或更低FLOPs的情况下比现有方法最多提高5.1%的绝对准确率。

Conclusion: HPR框架通过协同利用强大和高效模型，在关键决策点进行干预，实现了最先进的效率-准确率权衡，为LLM推理提供了更高效的解决方案。
...
TL;DR: 提出了RadFlow框架，通过分层工作流引导的强化学习优化医疗报告生成，解决现有方法忽视报告层次结构的问题。


<details>
  <summary>Details</summary>
Motivation: 现有医疗报告生成系统将报告视为扁平序列，忽视了放射科医生实际工作中的结构化工作流程，导致描述性内容和诊断内容不一致。

Method: RadFlow采用分层强化优化框架，包括全局奖励（整合语言流畅性、医学正确性和跨部分一致性）和局部奖励（强调印象部分质量），以及针对高风险病例的自适应策略优化机制。
...
TL;DR: MuSeR通过自我评估和精炼的方法，从决策制定、沟通和安全三个关键维度增强LLM在医疗领域的上下文感知能力，显著提升了在HealthBench基准上的表现。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在医疗基准测试中表现良好，但在需要更强上下文感知能力的真实医疗场景中仍表现不佳，无法有效识别缺失或关键细节并提供安全、有用的响应。

Method: 提出多维度自我精炼方法：1）设计属性条件查询生成器模拟多样化用户上下文；2）LLM响应查询并沿三个关键维度自我评估；3）精炼响应以更好对齐各维度要求；4）使用查询和精炼响应进行监督微调。

Result: 在HealthBench数据集上显著提升LLM性能，特别是在上下文感知维度。通过知识蒸馏，较小的骨干LLM（Qwen3-32B）性能超过其教师模型，在HealthBench上达到63.8%，在困难子集上达到43.1%，创下开源LLM新纪录。
...
TL;DR: 本文研究如何将可学习的类脑控制器与细胞状的Gray-Scott底物耦合，以最小努力引导模式形成。通过嵌入可微分反应-扩散模拟器的紧凑卷积策略，在暖-保持-衰减增益调度下产生空间平滑、有界的参数调制。


<details>
  <summary>Details</summary>
Motivation: 研究如何构建可引导、稳健且节能的具身系统，通过集中式学习和分布式自组织之间的最优分工来利用形态计算。

Method: 在可微分PyTorch反应-扩散模拟器中嵌入紧凑卷积策略，使用暖-保持-衰减增益调度产生空间平滑的参数调制，训练优化图灵带谱目标同时惩罚控制努力和不稳定性。

Result: 混合耦合实现了可靠快速的目标纹理形成：100%严格收敛约165步，匹配细胞仅有的谱选择性（0.436 vs. 0.434），同时使用比NN主导控制少约15倍的ℓ1努力和>200倍的ℓ2功率。

Conclusion: 控制器先播种后让步，提供短暂稀疏的推动将系统置于正确的吸引盆中，之后局部物理维持模式。这为构建可引导、稳健且节能的具身系统提供了实用方案。
...
TL;DR: RAGFort是一个针对检索增强生成系统的防御框架，通过对比重索引实现类间隔离和约束级联生成实现类内保护，有效抵御知识库重构攻击。


<details>
  <summary>Details</summary>
Motivation: 现有的RAG系统面临重构攻击威胁，攻击者通过聚合模型响应来复制知识库，而现有防御方法只针对单一攻击路径，无法提供全面保护。

Method: 提出RAGFort防御框架，包含两个核心模块：对比重索引用于类间隔离，约束级联生成用于类内保护。

Result: 实验表明RAGFort在安全性、性能和鲁棒性方面表现优异，显著降低重构成功率同时保持回答质量。
...
Conclusion: DenoGrad通过任务特定定义噪声并利用梯度信息，在保持数据分布的同时显著提升了可解释AI模型的鲁棒性和性能。
...
TL;DR: 该论文提出了两种无需基础化（grounding）即可编译PDDL中状态轨迹约束的方法，解决了现有编译器在大规模规划问题中的可扩展性问题。


<details>
  <summary>Details</summary>
Motivation: 现有编译器在处理具有大量对象和高元动作的规划问题时需要先进行基础化，导致可扩展性差。本文旨在解决这一瓶颈。

Method: 提出了两种无需基础化的约束编译方法，将具有状态轨迹约束的PDDL问题转换为标准规划问题，并证明了编译器的正确性和时间复杂度。

Result: 在国际规划竞赛领域的实证评估显示，新方法产生的规划规范比基础化编译器简洁数个数量级，且在使用最先进规划器时仍保持竞争力。

Conclusion: 所提出的无基础化编译方法在大规模规划问题中高效且实用，显著提升了约束编译的可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 现有交通信号建模方法主要依赖传感器的原始数值数据，忽略了多模态异构城市数据中的语义信息，这限制了对交通信号的全面理解和复杂交通动态的准确预测。

Method: 提出MTP多模态框架，包含三个分支：1）视觉增强：将原始信号转换为频域图像和周期性图像；2）文本增强：基于特定主题、背景信息和项目描述生成描述性文本；3）数值学习：使用频域多层感知器处理原始模态。通过分层对比学习融合三个模态的信息。
...
### [191] [Bridging Synthetic and Real Routing Problems via LLM-Guided Instance Generation and Progressive Adaptation](https://arxiv.org/abs/2511.10233)
*Jianghan Zhu,Yaoxin Wu,Zhuoyi Lin,Zhengyuan Zhang,Haiyan Yin,Zhiguang Cao,Senthilnath Jayavelu,Xiaoli Li*

Main category: cs.AI

TL;DR: EvoReal方法通过进化模块和LLM指导生成具有真实结构模式的合成实例，显著提升了神经组合优化方法在真实VRP场景中的泛化能力。
...
Result: 在TSPLib和CVRPLib基准测试上显著改善了神经求解器的泛化能力，与最优解的差距分别降低到1.05%和2.71%。

Conclusion: EvoReal方法有效提升了神经组合优化方法在真实世界路由问题中的泛化性能。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在推理方面表现出色，但存在幻觉和透明度有限的问题。现有的知识图谱增强LLM方法在复杂知识密集型任务中仍面临检索不准确和推理失败等挑战，特别是在长输入上下文和不同问题类型所需的逻辑方向捕捉方面。

Method: ProgRAG将复杂问题分解为子问题，逐步扩展部分推理路径，通过回答每个子问题来推进推理。在每一步中，外部检索器收集候选证据，然后通过LLM进行不确定性感知剪枝，最后通过组织和重新排列子问题答案获得的部分推理路径来优化LLM推理的上下文。

Result: 在三个知名数据集上的实验表明，ProgRAG在多跳知识图谱问答任务中优于现有基线方法，提供了更高的可靠性和推理质量。

Conclusion: ProgRAG通过渐进式推理路径扩展和不确定性感知证据剪枝，有效解决了知识图谱增强LLM中的检索和推理失败问题，为复杂知识密集型任务提供了更可靠的解决方案。
...
### [193] [Beyond Single-Step Updates: Reinforcement Learning of Heuristics with Limited-Horizon Search](https://arxiv.org/abs/2511.10264)
*Gal Hadar,Forest Agostinelli,Shahaf S. Shperberg*

Main category: cs.AI

TL;DR: 本文提出了一种增强启发式搜索的通用方法，通过有限范围搜索和基于搜索边界的最短路径更新启发值，改进了状态采样和启发式更新过程。


<details>
  <summary>Details</summary>
Motivation: 传统启发式搜索方法依赖单步贝尔曼更新，仅基于最佳邻居状态更新启发值。本文旨在通过更全面的搜索和更新策略来提升启发式学习效果。

Method: 采用有限范围搜索策略，在更新状态启发值时不仅考虑单步邻居，还基于到搜索边界的最短路径，同时结合边成本和边界状态的启发值。

Result: 该方法能够更有效地学习启发函数，通过更全面的状态探索和更准确的启发值更新机制提升搜索性能。

Conclusion: 提出的广义方法通过有限范围搜索和基于搜索边界的启发式更新，显著改进了传统单步贝尔曼更新的局限性，为启发式学习提供了更有效的框架。
...
### [194] [Causal-HalBench: Uncovering LVLMs Object Hallucinations Through Causal Intervention](https://arxiv.org/abs/2511.10268)
*Zhe Xu,Zhicai Wang,Junkang Wu,Jinda Lu,Xiang Wang*

Main category: cs.AI

TL;DR: 该论文提出大型视觉语言模型存在物体幻觉问题，主要源于训练中高度共现物体间的伪相关性。作者引入因果分析建立结构因果模型，构建了Causal-HalBench基准来量化伪相关性影响，并开发了反事实样本生成流程。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型经常出现物体幻觉问题，错误判断图像中物体的存在。作者认为这主要源于训练过程中高度共现物体间的伪相关性，导致模型受视觉上下文影响产生幻觉物体。现有基准主要关注幻觉检测，但缺乏对伪相关性的形式化描述和量化评估。

Method: 在LVLMs物体识别场景中引入因果分析，建立结构因果模型(SCM)。正式定义由共现偏差引起的伪相关性，开发Causal-HalBench基准，包含反事实样本和综合因果指标。提出利用专有LVLMs和文本到图像模型生成反事实样本的可扩展流程。

Result: 在主流LVLMs上使用Causal-HalBench进行评估，结果表明这些模型对伪相关性表现出不同程度的敏感性。

Conclusion: 大型视觉语言模型确实存在由共现偏差引起的伪相关性问题，Causal-HalBench基准能够有效量化这种影响，为评估模型对伪相关性的鲁棒性提供了系统方法。
...
### [195] [Bidirectional Bounded-Suboptimal Heuristic Search with Consistent Heuristics](https://arxiv.org/abs/2511.10272)
*Shahaf S. Shperberg,Natalie Morad,Lior Siag,Ariel Felner,Dor Atzmon*

Main category: cs.AI

TL;DR: 本文提出了几种基于BAE*算法的有界次优双向搜索变体，用于解决在有界次优性约束下的双向搜索问题。
...
Conclusion: 不同算法各有优缺点，每种方法在特定条件下表现最佳，这突显了各种方法的优势和局限性。
...
### [196] [FactGuard: Event-Centric and Commonsense-Guided Fake News Detection](https://arxiv.org/abs/2511.10281)
...
<details>
  <summary>Details</summary>
Motivation: 随着攻击者模仿真实新闻的写作风格，基于写作风格的假新闻检测方法效果逐渐下降。大语言模型在假新闻检测中的潜力尚未充分挖掘，存在功能探索浅、可用性模糊和推理成本高等问题。

Method: 提出FactGuard框架，利用LLMs提取事件中心内容以减少写作风格影响；引入动态可用性机制识别事实推理中的矛盾和模糊案例；采用知识蒸馏技术生成FactGuard-D用于冷启动和资源受限场景。

Result: 在两个基准数据集上的综合实验表明，该方法在鲁棒性和准确性方面持续优于现有方法，有效解决了假新闻检测中的风格敏感性和LLM可用性问题。

Conclusion: FactGuard框架通过结合LLMs的事件内容提取和动态可用性机制，显著提升了假新闻检测的性能，同时通过知识蒸馏确保了实际部署的效率和可行性。
...
### [197] [Beyond Verification: Abductive Explanations for Post-AI Assessment of Privacy Leakage](https://arxiv.org/abs/2511.10284)
...
Result: 在德国信用数据集上的实验表明，敏感文字在模型决策过程中的重要性会影响隐私泄露程度，该方法能提供严格的隐私保证并生成人类可理解的解释。

Conclusion: 溯因推理能够实现可解释的隐私审计，为协调AI决策中的透明度、模型可解释性和隐私保护提供了实用途径。
...
### [198] [Massively Parallel Proof-Number Search for Impartial Games and Beyond](https://arxiv.org/abs/2511.10339)
*Tomáš Čížek,Martin Balko,Martin Schmid*

Main category: cs.AI

TL;DR: 本文提出了首个可在大规模CPU核心上高效扩展的并行证明数搜索算法，应用于Sprouts游戏求解，在1024核心上实现332.9倍加速，验证了Sprouts猜想的新位置。
...
Result: 在1024核心上实现332.9倍加速，性能比最先进的Sprouts求解器GLOP快四个数量级，生成复杂度高1000倍的证明，验证了Sprouts猜想的42个新位置。

Conclusion: 成功开发了首个可大规模并行扩展的证明数搜索算法，显著提升了游戏求解能力，为Sprouts猜想研究提供了重要进展。
...
### [199] [SITA: A Framework for Structure-to-Instance Theorem Autoformalization](https://arxiv.org/abs/2511.10356)
*Chenyi Li,Wanli Ma,Zichen Wang,Zaiwen Wen*

Main category: cs.AI

TL;DR: 本文提出了一个结构到实例定理自动形式化框架，用于弥合抽象数学理论与具体应用之间的差距，通过将形式化的抽象结构作为模块化模板来指导具体实例的形式化。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在数学推理方面取得了进展，但在将抽象结构实例化产生的定理形式化方面仍面临挑战。本文旨在实现研究级数学结果的自动形式化。

Method: 开发了SITA框架，将形式化的抽象结构作为包含定义、假设、操作和定理的模块化模板，利用Lean证明助手的类型类机制生成具体实例的定义和声明，并结合基于LLM的生成与反馈引导的优化来确保自动化和形式正确性。
...
### [200] [Explaining Decentralized Multi-Agent Reinforcement Learning Policies](https://arxiv.org/abs/2511.10409)
...
<details>
  <summary>Details</summary>
Motivation: 现有解释方法主要关注集中式MARL，无法处理去中心化设置中的不确定性和非确定性，需要开发专门针对去中心化MARL的解释方法。

Method: 生成策略总结以捕捉任务顺序和智能体合作，并提供基于查询的解释（When、Why Not、What类型）来回答特定智能体行为的相关问题。

Result: 在四个MARL领域和两种去中心化MARL算法上评估，证明方法具有通用性和计算效率。用户研究表明总结和解释显著提高了用户问答性能，并提升了理解和满意度等主观评分。

Conclusion: 提出的方法有效解决了去中心化MARL中的解释需求，能够生成有意义的策略总结和查询解释，显著提升了用户理解和满意度。
...
### [201] [Generalizing Analogical Inference from Boolean to Continuous Domains](https://arxiv.org/abs/2511.10416)
...
Result: 在平滑性假设下推导了最坏情况和平均情况下的误差边界，提供了跨离散和连续域的通用类比推理理论。
...
### [202] [Using Certifying Constraint Solvers for Generating Step-wise Explanations](https://arxiv.org/abs/2511.10428)
*Ignace Bleukx,Maarten Flippo,Bart Bogaerts,Emir Demirović,Tias Guns*

Main category: cs.AI

TL;DR: 本文提出了一种基于约束求解器生成的证明来构建逐步解释序列的新方法，显著提高了解释生成效率，同时保持了与现有方法相似的输出质量。


<details>
  <summary>Details</summary>
Motivation: 在可解释约束求解领域，向用户解释问题为何不可满足是一个常见需求。现有的逐步解释方法计算成本高昂，限制了其应用范围。

Method: 定义了一个抽象证明框架，将证明和逐步解释统一表示。提出了多种将证明转换为逐步解释序列的方法，特别关注修剪和简化技术以保持序列简洁。

Result: 实验结果表明，该方法显著加快了逐步解释序列的生成速度，同时生成的逐步解释质量与当前最先进方法相当。

Conclusion: 通过利用约束求解器生成的证明作为起点，可以有效提高逐步解释的生成效率，为更大规模问题的可解释性分析提供了可行方案。
...
### [203] [Preference Elicitation for Step-Wise Explanations in Logic Puzzles](https://arxiv.org/abs/2511.10436)
*Marco Foschini,Marianne Defresne,Emilio Gamba,Bart Bogaerts,Tias Guns*

Main category: cs.AI

TL;DR: 本文研究了在逻辑谜题等满意度问题中，通过交互式偏好学习来优化逐步解释的质量。提出了动态归一化技术和MACHOP查询生成策略，在数独和逻辑网格谜题上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 在逐步解释逻辑谜题时，存在多种候选解释步骤，需要用户定义目标函数来量化解释质量。但定义好的目标函数具有挑战性，因此需要从交互式偏好学习中学习用户偏好。

Method: 提出两种动态归一化技术来重新缩放特征并稳定学习过程；引入MACHOP（多臂选择感知器）查询生成策略，将非支配约束与基于上置信界的多样化相结合。

Result: 在数独和逻辑网格谜题上使用人工用户进行评估，并通过真实用户验证。在这两种设置下，MACHOP始终比标准方法产生更高质量的解释。

Conclusion: 交互式偏好学习可以有效优化逐步解释的质量，MACHOP策略在生成高质量解释方面优于标准方法。
...
### [204] [Non-Monotonic S4F Standpoint Logic](https://arxiv.org/abs/2511.10449)
*Piotr Gorczyca,Hannes Strass*

Main category: cs.AI

TL;DR: S4F立场逻辑是一种新的形式化框架，结合了S4F模态逻辑和立场命题逻辑，能够表达多视角、非单调的语义承诺。该逻辑在计算复杂度上不高于其组成逻辑，并提供了轻信和怀疑接受机制。


<details>
  <summary>Details</summary>
Motivation: 统一表示多个异构视角的立场逻辑与能够自然捕获非单调推理框架的S4F模态逻辑相结合，创建一个能够表达多视角、非单调语义承诺的新形式化框架。
...
### [205] [Proceedings of The third international workshop on eXplainable AI for the Arts (XAIxArts)](https://arxiv.org/abs/2511.10482)
...
<details>
  <summary>Details</summary>
Motivation: 汇集跨学科研究者社区，探索可解释AI在艺术领域的应用潜力和挑战。

Method: 在ACM创意与认知会议(C&C 2025)上在线举办研讨会，促进不同领域专家的交流与合作。

Result: 成功组织了第三届XAIxArts国际研讨会，为艺术与可解释AI的交叉研究提供了平台。

Conclusion: 该研讨会为艺术与可解释AI的跨学科研究建立了重要的交流平台，推动了该领域的发展。
...
### [206] [Rethinking Science in the Age of Artificial Intelligence](https://arxiv.org/abs/2511.10524)
...
<details>
  <summary>Details</summary>
Motivation: 探讨AI如何改变研究工作的构思、实施和交流方式，以及这种转变对科学实践的影响和挑战。

Method: 通过评论性分析，考察AI在研究工作流程中的各个应用环节，包括文献管理、跨学科联系、假设生成和实验设计等。

Result: AI系统能够帮助研究人员管理信息过载、筛选文献、发现跨学科联系、生成假设以及设计和执行实验。

Conclusion: 需要在科学实践中审慎采用AI，通过促进透明度、可重复性和问责制的政策，确保AI作为人类判断的补充而非替代。
...
### [207] [Regular Games -- an Automata-Based General Game Playing Language](https://arxiv.org/abs/2511.10593)
*Radosław Miernik,Marek Szykuła,Jakub Kowalski,Jakub Cieśluk,Łukasz Galas,Wojciech Pawlik*

Main category: cs.AI

TL;DR: 提出了名为Regular Games (RG)的新通用游戏系统，旨在实现计算效率和游戏设计便利性的平衡。该系统使用有限自动机定义游戏规则，具有高效的前向模型生成能力，超越了现有GGP系统。
...
Method: RG系统采用分层语言设计：核心是低级的有限自动机语言定义游戏规则，高级语言用于游戏设计，最终编译为低级语言。系统包含编辑器、可视化工具和调试器等完整生态系统。

Result: RG系统的前向模型生成速度超过了当前最先进的GGP系统（如Regular Boardgames、Ludii），在效率方面表现优异。

Conclusion: RG系统为通用游戏提供了一个高效且便利的解决方案，其分层语言设计和完整工具生态系统使其在计算效率和游戏设计方面都具有优势。
