...
TL;DR: 提出了一种新颖的原生3D编辑框架，通过单次前向传播直接操作3D表示，解决了现有方法在几何一致性和视觉质量上的问题。
...
Method: 创建了大规模多模态指令引导3D编辑数据集，探索了两种条件策略：传统交叉注意力机制和新型3D标记拼接方法。

Result: 标记拼接方法在参数效率上更优且性能更佳，在生成质量、3D一致性和指令保真度方面超越了现有的2D提升方法。

Conclusion: 该方法为指令引导的3D编辑设立了新的基准，在保持未编辑区域一致性的同时，能够忠实遵循指令变化。
...
TL;DR: 本文展示了如何使用持续同调（PH）这一拓扑数据分析方法，来客观识别和区分艺术风格，包括不同艺术家、同一艺术流派内的艺术家，以及真实艺术家作品与AI生成作品的区别。
...
Method: 使用持续同调（PH）这一拓扑数据分析方法，对艺术作品进行定量分析。

Result: PH方法能够以统计确定性区分不同艺术家（包括不同艺术流派和同一流派内的艺术家），并能区分艺术家真实作品与AI生成的作品。

Conclusion: 持续同调为艺术风格分析提供了客观且可解释的洞察，能够有效识别艺术作品的风格特征。
...
Method: 使用StyleGAN2模型，实施：1）通过运动转移合成新面部表情图像的数据增强；2）基于潜在表示的样本选择确保训练数据集的多样性和均匀采样；3）损失函数优化以准确再现细微运动如眼球运动。

Result: 该方法能够为多个猕猴个体生成多样化的面部表情，性能优于仅使用原始静态图像训练的模型。模型在基于风格的图像编辑中有效，特定风格参数对应不同的面部运动。
...
TL;DR: 本文提出了PairHuman数据集，这是首个专为生成高质量双人肖像而设计的大规模基准数据集，包含超过10万张图像，涵盖多种场景、服装和互动。同时提出了DHumanDiff基线方法，用于增强面部一致性并平衡个性化人物生成与语义驱动场景创建。
...
Method: 构建PairHuman数据集（包含10万+图像，丰富元数据），并提出DHumanDiff基线方法，该方法增强面部一致性，同时平衡个性化人物生成和语义驱动场景创建。

Result: 实验结果表明，所提出的数据集和方法能够生成高度定制化的肖像，具有卓越的视觉质量，并符合人类偏好。
...
<details>
  <summary>Details</summary>
Motivation: 中子成像在惯性约束聚变分析中至关重要，但图像常被高斯和泊松混合噪声所退化，传统滤波和阈值方法难以有效去除这些重叠噪声，需要开发能保持图像保真度的去噪技术。
...
TL;DR: 本文提出SaFeR-CLIP框架，通过将不安全概念重定向到语义上最接近的安全替代品，在保持鲁棒安全性的同时，比先前方法恢复高达8.0%的零样本准确率。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型如CLIP通过微调提高安全性时，往往导致泛化性能显著下降，这种权衡源于将不安全概念强制对齐到单一预定义安全目标的刚性对齐策略。

Method: 提出邻近感知方法，将不安全概念重定向到语义上最接近的安全替代品以最小化表示变化，开发SaFeR-CLIP微调框架应用最小干预原则。

Result: SaFeR-CLIP成功调和安全性与性能，在保持鲁棒安全性的同时，比先前方法恢复高达8.0%的零样本准确率。
...
Method: 三阶段方法：1) 将栅格化输入提升为3D表示并渲染多视图图像；2) 扩展SAM2的时间记忆机制到空间域，构建空间记忆库建立部件级对应关系；3) 在栅格到矢量转换中进行路径整合和结构优化。

Result: 生成的SVG在多个视图间表现出强烈的几何和颜色一致性，显著减少冗余路径，同时保留精细结构细节。
...
TL;DR: Mesh RAG是一个无需训练、即插即用的框架，通过检索增强生成来解决自回归网格生成模型中的序列依赖问题，显著提升网格质量、加速生成速度并支持增量编辑。


<details>
  <summary>Details</summary>
Motivation: 传统网格制作依赖人工艺术家，耗时且难以扩展。现有自回归网格生成方法存在严重的质量-速度权衡，且序列依赖限制了增量编辑能力。

Method: 受语言模型RAG启发，通过点云分割、空间变换和点云注册技术来检索、生成和集成网格组件，将生成过程与严格序列依赖解耦。

Result: Mesh RAG在各种基础自回归网格生成模型上均能显著提升网格质量，加速生成速度，并实现增量编辑功能，且无需模型重新训练。

Conclusion: Mesh RAG框架有效解决了自回归网格生成中的序列依赖问题，在保持高质量的同时实现了并行化推理和编辑灵活性。
...
TL;DR: WorldGen是一个从文本提示自动创建大规模交互式3D世界的系统，结合LLM场景布局推理、程序生成和扩散式3D生成技术，可直接在游戏引擎中探索和编辑。
...
TL;DR: REO-Instruct是首个面向地球观测的统一基准数据集，专注于描述性和回归任务，特别针对森林生态场景，建立了从人类活动到地上生物量的认知逻辑链。
...
Method: 通过混合人机协作流程，整合配准的Sentinel-2和ALOS-2影像，生成结构化文本标注，建立包含人类活动、土地覆盖分类、生态斑块计数和地上生物量回归的认知逻辑链。

Result: 对通用视觉语言模型的综合评估显示，当前模型在数值推理方面存在困难，凸显了科学视觉语言模型面临的关键挑战。
...
TL;DR: BOP-ASK是一个用于物体交互推理的大规模数据集，包含超过15万张图像和3300万个问答对，涵盖六个任务，旨在解决现有VLMs在细粒度空间理解方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在空间推理基准测试中表现优异，但忽略了真实世界应用所需的细粒度空间理解能力，包括精确3D定位、物体物理兼容性、物体可供性和多步空间规划。

Method: 利用BOP数据集中的6D物体姿态数据，生成细粒度标注，包括抓取姿态、参考物体姿态、路径规划轨迹、相对空间和深度关系以及物体间关系。

Result: 在BOP-ASK上训练的模型优于基线模型，并展现出精确物体和抓取姿态估计、轨迹规划以及在杂乱环境中进行细粒度物体中心空间推理的新兴能力。

Conclusion: BOP-ASK为训练和评估VLMs提供了丰富资源，填补了现有基准测试在细粒度物体交互推理方面的空白，并将公开发布数据集和生成流程。
...
TL;DR: 提出Parts-Mamba混合GCN-Mamba模型，通过部件特定扫描特征和部件-身体融合模块增强远距离关节上下文信息捕获能力，在骨架动作识别中处理遮挡和不完整骨架问题。


<details>
  <summary>Details</summary>
Motivation: 现实场景中由于遮挡或通信质量问题，捕获的骨架通常不完整或存在缺失帧，现有GCN模型在缺失局部上下文时性能较差。

Method: 提出Parts-Mamba混合模型，结合GCN和Mamba架构，使用部件特定扫描特征捕获部件特定信息，通过部件-身体融合模块保持非邻近关节上下文。

Result: 在NTU RGB+D 60和NTU RGB+D 120数据集的不同遮挡设置下评估，准确率最高提升12.9%。

Conclusion: Parts-Mamba模型能有效处理骨架不完整问题，通过增强远距离上下文信息捕获能力显著提升骨架动作识别性能。
...
Method: 通过引入JGW目标，将GW框架扩展到同时匹配对象集合，采用最优传输算法（包括熵正则化）进行求解。

Result: 在部分匹配任务中，JGW方法在准确性和计算效率方面优于其他GW变体，在合成和真实数据集上验证了其在多形状匹配中的有效性。
...
TL;DR: 该论文提出REPA方法，通过将扩散或流模型的内部表示与预训练自监督编码器（如DINOv2）对齐来改进逆问题求解，显著提升重建质量和效率。
...
Method: 提出REPA方法，在推理时对齐扩散或流模型与预训练自监督视觉编码器的表示，使用近似目标特征指导重建过程。

Result: 在超分辨率、框内修复、高斯去模糊和运动去模糊等任务中，REPA一致提升重建质量，同时通过减少离散化步骤提供显著效率增益。

Conclusion: REPA方法通过表示对齐有效提升逆问题求解的感知保真度，具有通用性和高效性，可集成到多种最先进的求解器中。
...
<details>
  <summary>Details</summary>
Motivation: 现有玻璃表面检测方法依赖边界线索或反射线索，但未能充分利用玻璃本身的内在特性。观察到真实场景中玻璃两侧光照强度差异导致反射变化的现象，利用这一特性改进检测精度。

Method: 提出NFGlassNet方法，包含反射对比挖掘模块(RCMM)用于提取反射特征，反射引导注意力模块(RGAM)用于融合反射和玻璃表面特征。构建了包含3.3K对闪光/无闪光图像的数据集。

Result: 大量实验证明该方法优于现有最先进方法，在玻璃表面检测任务上取得了更好的性能。
...
TL;DR: 本文提出了R-AVST数据集用于音频-视觉时空推理，包含5000多个未修剪视频和27000个对象，涵盖100种音频-视觉事件。同时提出了AVST-Zero模型，使用强化学习直接优化行为，在音频-视觉时空推理任务上表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视频理解任务中进展迅速，但主要关注简单视频场景，无法反映真实世界中复杂多样的音频-视觉事件。需要构建能够支持音频-视觉时空推理的数据集和模型。

Method: 构建R-AVST数据集：使用基于LLM的关键对象提取、自动空间标注和人工质量检查的流程；提出AVST-Zero模型：基于强化学习的方法，避免中间监督，通过精心设计的多维度奖励直接优化行为。

Result: 构建了包含5K+未修剪视频、27K对象、100种音频-视觉事件的R-AVST数据集；生成了8K+高质量、均匀分布的问题-答案对；AVST-Zero在音频-视觉时空推理任务上表现出竞争力。

Conclusion: R-AVST是首个为真实世界音频-视觉时空推理设计的数据集，AVST-Zero为该领域未来挑战提供了新的解决视角，实验验证了它们在推进音频-视觉时空推理方面的有效性。
...
TL;DR: 本文提出了Warm Diffusion，一种统一的模糊-噪声混合扩散模型（BNMD），通过联合控制模糊和噪声来整合热扩散和冷扩散的优势，利用图像的光谱依赖性简化了分数模型估计。


<details>
  <summary>Details</summary>
Motivation: 热扩散完全依赖噪声，未能利用高频图像细节与低频结构之间的强相关性；冷扩散仅使用模糊而忽视噪声在塑造数据流形中的作用，导致性能下降。需要整合两者的优势。

Method: 提出Warm Diffusion（BNMD模型），采用分治策略利用图像的光谱依赖性，通过解耦去噪和去模糊过程简化分数模型估计，并使用光谱分析研究模糊-噪声比（BNR）。
...
<details>
  <summary>Details</summary>
Motivation: 旨在实现模型、任务和表示三个层面的统一，消除模态差异，构建完全视觉原生的多模态学习框架。

Method: 使用统一的扩散变换器，在像素空间中训练整流流，将文本提示渲染为绘制文本图像，所有输入输出都作为RGB像素处理。

Result: 在文本到图像合成和图像到文本理解任务上表现出强大的跨模态对齐和新兴可控性，如循环一致的图像-标题-图像循环。
...
TL;DR: DeltaDeno是一种无需训练、零样本的异常生成方法，通过对比两个扩散分支在共享调度下的去噪差异来定位和编辑缺陷，无需真实异常样本或训练。


<details>
  <summary>Details</summary>
Motivation: 现有的异常生成方法通常需要少量异常样本进行微调，这与异常稀缺的实际情况相矛盾，容易过拟合类别先验。本文旨在解决完全无真实异常样本或训练的场景。

Method: 提出Delta-Denoising方法：1）使用最小提示对驱动两个扩散分支对比；2）通过累积每步去噪差异生成图像特定定位图；3）基于定位图指导潜在修复，在保留上下文的同时生成局部缺陷；4）进行token级提示优化和空间注意力偏置。

Result: 在公共数据集上的实验表明，DeltaDeno实现了优秀的生成效果、真实性和下游检测性能的持续提升。

Conclusion: DeltaDeno提供了一种无需训练、零样本的异常生成解决方案，能够有效定位和生成局部缺陷，在异常检测任务中表现优异。
...
TL;DR: 本文提出了一种基于扩散模型的视频超分辨率方法DGAF-VSR，通过特征域对齐和补偿机制解决了现有方法中的错误累积、空间伪影以及感知质量与保真度之间的权衡问题。
...
Conclusion: 特征域比像素域更适合信息补偿，在放大分辨率下进行变形能更好保留高频信息，所提出的DGAF-VSR方法在视频超分辨率的关键方面均实现了显著提升。
...
<details>
  <summary>Details</summary>
Motivation: CBCT图像中的牙齿分割在邻牙粘连情况下面临严重挑战，因为粘连会导致解剖形状严重扭曲，影响分割准确性。
...
TL;DR: 本文提出了OmniGround基准测试，包含3,475个视频和81个类别，用于解决时空视频定位中的类别偏见和推理简化问题，并提出了PG-TAF训练免费框架，在多个基准测试上取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在时空视频定位任务中与现实需求存在显著差距，主要由于基准测试范围有限导致模型出现类别偏见、推理过于简化和语言鲁棒性差的问题。

Method: 提出了OmniGround基准测试，采用前向-后向-精炼标注流程，结合多方向跟踪和智能错误校正；开发了DeepSTG系统评估框架；提出了PG-TAF训练免费两阶段框架，将STVG分解为高层时间定位和细粒度时空传播。

Result: 在复杂现实场景中性能平均下降10.4%，特别是在小/遮挡对象和复杂空间关系上表现较差；PG-TAF在OmniGround上m_tIoU和m_vIoU分别提升25.6%和35.6%，在四个基准测试上均取得一致增益。
...
TL;DR: MultiPriv是首个系统评估视觉语言模型个体级隐私推理能力的基准，揭示了当前模型在隐私推理方面存在重大风险，且现有安全对齐措施对此类攻击无效。


<details>
  <summary>Details</summary>
Motivation: 现代视觉语言模型展现出复杂推理能力，带来了超越简单属性感知的个体级链接隐私风险。当前隐私基准主要评估隐私感知，无法应对更关键的隐私推理风险。

Method: 提出隐私感知与推理框架，构建包含合成个体档案的双语多模态数据集，设计9个挑战性任务评估从属性检测到跨图像重识别和链式推理的完整隐私推理能力。

Result: 对50多个基础模型和商业模型的大规模评估显示：许多模型存在显著的未测量推理隐私风险；感知级指标无法预测推理风险；现有安全对齐措施对此类推理攻击无效。
...
Method: 使用两个隐式神经表示：一个参数化时空图像内容，另一个表示光学流场，通过光学流方程耦合作为物理正则化，并结合数据一致性损失。
...
TL;DR: 本文提出FingerCap任务用于生成描述手指级手部动作的文本，构建了包含4万对手部动作视频和文本的FingerCap-40K数据集，并开发了FiGOP方法来增强视频MLLMs对细微手指动作的捕捉能力。


<details>
  <summary>Details</summary>
Motivation: 理解精细的人类手部动作对于视觉感知、具身智能和多模态通信至关重要，但现有方法难以捕捉手指级的细微动作语义。

Method: 提出FiGOP方法，将每个RGB关键帧与后续手部关键点配对，通过轻量级时间编码器将关键点转换为运动嵌入并与RGB特征集成，在不增加RGB采样密度的情况下恢复精细时间线索。

Result: 在FingerCap-40K数据集上的实验表明，现有的开源和闭源视频MLLMs在手指级推理方面仍有困难，而FiGOP增强的模型在HandJudge评估和人类研究中均取得一致提升。

Conclusion: FiGOP方法有效解决了视频MLLMs在捕捉精细手指动作时的时间稀疏性问题，为手指级手部动作理解提供了有效的解决方案。
...
TL;DR: 本文提出了一种基于点监督的面部表情检测方法，通过双分支框架解决单时间戳标注下的表情实例识别问题，包括基于高斯分布的实例自适应强度建模和类别感知的顶点分类。


<details>
  <summary>Details</summary>
Motivation: 现有面部表情检测方法主要依赖完整的时间边界标注，成本高昂且耗时。本文研究点监督面部表情检测，只需每个表情实例的单个时间戳标注，大幅降低标注成本。

Method: 提出双分支框架：1）基于高斯分布的实例自适应强度建模模块，通过检测伪顶点帧、估计持续时间和构建实例级高斯分布来进行软伪标注；2）类别感知顶点分类分支，仅基于伪顶点帧区分宏表情和微表情。还引入强度感知对比损失来增强特征学习。

Result: 在SAMM-LV、CAS(ME)^2和CAS(ME)^3数据集上的广泛实验证明了所提框架的有效性。

Conclusion: 提出的点监督面部表情检测框架能够有效利用单时间戳标注，通过软伪标注和双分支设计实现了准确的表情检测和分类。
...
<details>
  <summary>Details</summary>
Motivation: 现有材料生成方法缺乏统一的表示来桥接自然图像外观和PBR属性，导致任务特定流程碎片化且无法利用大规模RGB图像数据。

Method: 提出联合RGB-PBR表示，将材料编码为两个相互依赖的潜在变量：RGB外观和四个PBR贴图，将其表述为5帧序列并使用视频扩散架构。

Result: 在MatHybrid-410K混合数据集上训练，实现了原生1024×1024合成，在质量和多样性上显著超越现有方法。

Conclusion: MatPedia提供了一个统一框架，能够处理文本到材料生成、图像到材料生成和内在分解等多种材料任务。
...
<details>
  <summary>Details</summary>
Motivation: 解决密集3D重建中的漂移和冗余点云问题，这些限制了SLAM系统的效率和下游任务（如新视角合成）的性能。

Method: 首先通过轻量级跟踪和重建模块构建局部一致的子地图，然后逐步对齐和融合到全局高斯地图中，强制跨视角几何一致性，全局地图反过来提供反馈校正局部漂移。

Result: 在真实世界数据集上实现了最先进的跟踪、3D重建和新视角渲染性能，跟踪精度提升超过12%，生成更精细的几何细节，同时保持紧凑的内存高效全局表示。

Conclusion: SING3R-SLAM通过全局高斯表示有效解决了SLAM中的漂移和冗余问题，为多种下游应用提供了高效、精确的3D建图解决方案。
...
TL;DR: QueryOcc是一个基于查询的自监督框架，通过4D时空查询直接学习连续3D语义占用，支持从视觉基础模型生成的伪点云或原始激光雷达数据进行监督，在保持恒定内存的同时实现长距离推理。


<details>
  <summary>Details</summary>
Motivation: 解决从图像学习3D场景几何和语义的挑战，避免昂贵的大规模3D标注，克服现有方法依赖2D渲染一致性或离散化体素网格的限制。

Method: 使用独立的4D时空查询采样相邻帧，引入压缩场景表示保持近场细节同时平滑压缩远场区域，支持伪点云或原始激光雷达监督。

Result: 在自监督Occ3D-nuScenes基准上，语义RayIoU比先前相机方法提升26%，运行速度为11.6 FPS。

Conclusion: 直接4D查询监督能够实现强大的自监督占用学习。
...
TL;DR: 该论文提出了一个面向边缘设备的烹饪过程图像生成模型，通过厨师标注的熟度级别数据集和烹饪状态引导的生成器，能够根据生食图像合成逼真的熟食图像。


<details>
  <summary>Details</summary>
Motivation: 解决在边缘设备上从生食图像合成逼真熟食图像的挑战，现有方法要么生成不真实的结果，要么资源消耗过大不适合边缘部署。
...
TL;DR: 本文提出SeeCLIP框架，通过细粒度语义增强解决开放集域泛化中的结构风险和开放空间风险困境，特别针对与已知类别视觉相似的“硬未知”类别。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理域偏移和新类别时面临结构风险与开放空间风险的困境，容易产生过度自信，特别是在区分与已知类别具有细粒度视觉相似性的“硬未知”类别时。

Method: 提出语义增强的CLIP框架，包括语义感知提示增强模块分解图像为判别性语义标记，双工对比学习保持与已知类别的分离性和语义邻近性，以及语义引导扩散模块生成伪未知样本。

Result: 在五个基准测试中，相比最先进方法实现了3%准确率和5% H-score的持续改进。
...
TL;DR: 提出受自然选择启发的3D高斯泼溅修剪框架，通过将生存压力建模为应用于不透明度的正则化梯度场，让优化梯度自主决定保留或修剪哪些高斯基元，无需人工干预。
...
Method: 提出自然选择启发的修剪框架，将生存压力建模为不透明度的正则化梯度场，让优化梯度自主决定高斯基元的保留或修剪。引入具有有限不透明度先验的不透明度衰减技术，加速选择过程。

Result: 相比3DGS，在15%预算下实现超过0.6 dB PSNR增益，在紧凑3DGS方面达到最先进性能。

Conclusion: 该方法实现了完全可学习的修剪过程，无需人工干预，在保持高质量渲染的同时显著降低了存储和计算开销。
...
TL;DR: FLUID是一个无需训练的框架，通过在预训练扩散模型的潜在空间中进行身份替换来实现人脸去识别，利用化学中的替代机制概念，在潜在h空间中进行语义位移来编辑身份。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸去识别方法在身份抑制和属性保留之间难以达到良好平衡，需要开发一种更有效的训练无关框架来同时实现身份消除和属性保护。
...
Conclusion: FLUID框架成功实现了无需训练的人脸去识别，通过潜在空间中的身份替换机制，在保持图像质量的同时有效保护个人隐私。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的深度通常不是单一的，透射材料会产生分层模糊，而现有模型只能被动估计静态深度图，无法像人类那样主动调整焦点来感知所需深度。

Method: 引入可操控的Vision Transformer，通过标量深度偏好进行条件化，使模型能够动态适应计算以聚焦到目标深度。训练主要使用新构建的50万张多层合成数据集，捕捉各种透视效果。

Result: 在BOOSTER等传统单深度基准测试中达到最先进性能，在新提出的真实和合成多深度数据集上定量展示了意图对齐的估计能力，并在未见过的透视场景中表现出强大的泛化能力。

Conclusion: DepthFocus代表了向主动和类人3D感知迈出的重要一步，展示了在复杂场景中进行选择性感知的能力。
...
<details>
  <summary>Details</summary>
Motivation: 推动体育分析中的计算机视觉研究，通过提供首个大规模细粒度球拍姿态标注数据集，解决动态物体跟踪、条件运动预测和多模态分析等挑战。

Method: 构建包含乒乓球、网球和羽毛球的大规模数据集，提供球拍姿态细粒度标注和球位置标注，设计三个互相关联的任务：细粒度球跟踪、关节式球拍姿态估计和预测性球轨迹预测。

Result: 评估发现多模态融合的关键见解：简单拼接球拍姿态特征会降低性能，而CrossAttention机制能有效利用这些特征，使轨迹预测结果超越强单模态基线。

Conclusion: RacketVision为动态物体跟踪、条件运动预测和体育多模态分析的未来研究提供了多功能资源和强大起点。
...
TL;DR: 本文介绍了ConfusedTourist，一个评估视觉语言模型在文化对抗性扰动下稳定性的新测试套件，发现即使是最先进的模型在面临多重文化线索混合时也会出现严重性能下降。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法通常只关注单一文化概念，忽略了现实世界中多种文化线索共存的情况，而视觉语言模型在这种多文化混合场景下的稳定性尚未得到充分测试。

Method: 开发了ConfusedTourist文化对抗性鲁棒性测试套件，通过图像堆叠扰动和基于图像生成的变体来评估模型对地理线索扰动的稳定性。

Result: 实验显示模型在简单图像堆叠扰动下准确率大幅下降，基于图像生成的扰动效果更差。可解释性分析表明失败源于注意力系统性地转向干扰线索。

Conclusion: 视觉文化概念混合会严重损害最先进视觉语言模型的性能，迫切需要开发更具文化鲁棒性的多模态理解系统。
...
TL;DR: 提出了一个统一的行人跟踪框架OmniPT，能够交互式地跟踪、基于参考跟踪并生成跟踪对象的语义理解，通过RL-Mid Training-SFT-RL训练阶段解决了将跟踪任务建模为基础模型可执行任务的问题。
...
Method: 实现RL-Mid Training-SFT-RL训练阶段：基于LVLM预训练权重，先进行简单RL阶段使模型输出固定可监督的边界框格式；随后使用大量行人相关数据集进行中间训练；最后在多个行人跟踪数据集上进行监督微调，再进行RL阶段提升跟踪性能和指令跟随能力。
...
TL;DR: ReBrain是一个基于检索增强扩散模型的框架，用于从稀疏CT扫描重建完整脑部MRI体积，通过结合扩散模型和相似CT切片检索来解决低剂量CT分辨率不足的问题。


<details>
  <summary>Details</summary>
Motivation: 由于物理或临床限制，某些患者无法进行MRI检查，而低剂量CT协议导致稀疏的CT体积和较差的平面分辨率，使得从CT合成完整脑部MRI具有挑战性。
...
Conclusion: ReBrain框架通过检索增强扩散方法有效解决了从稀疏CT重建完整脑部MRI的挑战，在跨模态重建任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 传统自回归模型在视觉生成中采用随机排列序列顺序时，要么性能下降，要么在推理时序列顺序选择不够灵活。需要一种方法既能保持性能又能提供灵活的序列顺序。

Method: STAR利用图像块位置定义的网格中采样的均匀生成树的遍历顺序，通过广度优先搜索获得遍历顺序，并通过拒绝采样确保图像的部分连接观察作为序列前缀出现。

Result: STAR通过定制但结构化的随机化策略相比随机排列，保持了后缀补全能力，同时维持了采样性能，无需对语言AR模型中广泛采用的架构进行重大改变。

Conclusion: STAR建模能够结合图像先验知识，在保持采样性能的同时提供足够灵活的序列顺序来适应推理时的图像编辑。
...
TL;DR: 本文提出了一个针对大型视觉语言模型幻觉问题的综合干预框架，发现幻觉源于图像到输入文本、图像到输出文本和文本到文本三种路径的相互作用，并提出了针对不同问答对齐格式的干预方法。
...
Result: 实验表明，该方法在多个基准测试中能持续减少不同对齐类型的幻觉。

Conclusion: LVLMs的幻觉并非源于单一因果路径，而是多种路径相互作用的结果，且模型依赖的路径因问答对齐格式而异，提出的干预方法能有效减少幻觉。
...
TL;DR: REArtGS++提出了一种基于平面高斯抛光和时序几何约束的通用铰接物体重建方法，改进了REArtGS在螺旋关节和多部件物体上的表现，通过解耦螺旋运动建模和时序一致性正则化实现更好的部件级表面重建和关节参数估计。


<details>
  <summary>Details</summary>
Motivation: 现有方法REArtGS在处理螺旋关节或多部件物体时存在困难，且缺乏对未见状态的几何约束。需要开发更通用的铰接物体重建方法来解决这些问题。

Method: 首先为每个关节建模解耦螺旋运动（无需类型先验），通过部件运动混合联合优化部件感知高斯和关节参数。引入时序几何约束，鼓励高斯平面化，并通过泰勒一阶展开在平面法向和深度之间建立时序一致性正则化。

Result: 在合成和真实世界铰接物体上的大量实验表明，该方法在通用部件级表面重建和关节参数估计方面优于现有方法。

Conclusion: REArtGS++通过时序几何约束和平面高斯抛光，实现了更通用和准确的铰接物体重建，解决了螺旋关节和多部件物体的挑战。
...
TL;DR: 提出了一种基于语义掩码引导的LiDAR点云编辑方法，通过将点云转换为2D距离图像，结合语义掩码进行扩散生成，能够高效生成复杂边缘案例和动态场景的合成LiDAR数据。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统训练需要大量多样的点云数据，特别是复杂边缘案例，但真实数据获取困难且成本高，现有模拟方法无法完全捕捉真实场景复杂性。

Method: 将LiDAR点云转换为2D距离图像，使用基于凸包的语义掩码作为引导，实现扩散生成方法，确保几何一致性和真实性。
...
Method: 通过分析发现特征图中的关键组件在早期尺度上控制多样性形成，DiverseVAR在模型输入中抑制关键组件，在输出中放大关键组件。

Result: 实证结果表明该方法显著增强生成多样性，对性能影响可忽略不计。

Conclusion: DiverseVAR有效恢复了VAR模型的生成多样性，同时保持高保真合成质量，无需额外训练。
...
TL;DR: 本文分析了多语言文本到图像模型在跨语言文化一致性方面的问题，发现现有模型往往产生文化中立或英语偏向的结果。作者提出了一种定位文化敏感神经元的方法，并开发了两种对齐策略来增强文化一致性。


<details>
  <summary>Details</summary>
Motivation: 当前多语言文本到图像模型在视觉真实性和语义对齐方面进展迅速，但输出结果在不同文化背景下存在差异。由于语言承载文化内涵，多语言提示生成的图像应该保持跨语言文化一致性，但现有模型往往产生文化中立或英语偏向的结果。

Method: 提出了一种探测方法，将文化敏感信号定位到少数固定层中的一小部分神经元。基于这一发现，引入了两种互补的对齐策略：(1) 无需主干微调的推理时文化激活，放大已识别的神经元；(2) 仅更新文化相关层的层定向文化增强。

Result: 在CultureBench上的实验表明，与强基线相比，在保持保真度和多样性的同时，文化一致性得到了持续改进。

Conclusion: 研究表明，当前T2I模型的文化一致性问题并非源于缺失文化知识，而是文化相关表征激活不足。通过定位文化敏感神经元并应用针对性对齐策略，可以有效提升模型的文化一致性表现。
...
### [45] [MuM: Multi-View Masked Image Modeling for 3D Vision](https://arxiv.org/abs/2511.17309)
*David Nordström,Johan Edstedt,Fredrik Kahl,Georg Bökman*

Main category: cs.CV

TL;DR: 本文提出MuM模型，通过将掩码自编码器扩展到任意多视图，学习专门用于3D视觉的特征，在多个下游任务中超越了DINOv3和CroCo v2等最先进的视觉编码器。


<details>
  <summary>Details</summary>
Motivation: 大多数自监督学习方法主要优化语义理解而非几何推理，而Cross-View Completion (CroCo)是为3D理解量身定制的掩码自编码方法。本文沿着CroCo的思路，专注于学习适合3D视觉的特征。

Method: 将掩码自编码器扩展到同一场景的任意多视图，通过统一掩码所有视图并使用具有帧间注意力的轻量级解码器，方法比CroCo更简单且更具可扩展性。

Result: 在包括前馈重建、密集图像匹配和相对姿态估计在内的下游任务中进行了广泛评估，发现MuM模型优于最先进的视觉编码器DINOv3和CroCo v2。

Conclusion: 通过扩展掩码自编码器到多视图的方法能够有效学习3D视觉特征，在多个几何推理任务中表现出色，为3D视觉理解提供了更优的解决方案。
...
Conclusion: 该方法仅使用单状态下的稀疏视角RGB图像就能实现高质量的铰接物体重建，具有重要的应用价值。
...
TL;DR: ReCoVAD是一个基于预训练大模型的免训练视频异常检测框架，通过模拟人类神经系统的双通路机制实现选择性帧处理，显著减少计算量，在保持SOTA性能的同时仅处理28.55%和16.04%的帧。
...
Method: 提出ReCoVAD框架，包含两个核心通路：(1)反射通路：使用轻量级CLIP模块融合视觉特征与原型提示，查询动态记忆进行快速响应；(2)意识通路：使用中等规模视觉语言模型生成文本事件描述和精炼异常分数，持续更新记忆和原型提示，并通过大语言模型定期审查识别未见异常。

Result: 在UCF-Crime和XD-Violence数据集上，ReCoVAD实现了最先进的免训练性能，同时分别仅处理了先前方法28.55%和16.04%的帧数。
...
TL;DR: 本文提出了一种利用生成模型创建多样化合成人脸图像的方法，并发布了包含27,780张图像、926个独特身份的DIF-V数据集，以解决现有面部验证系统中的偏见问题。
...
Conclusion: 通过解决现有数据集的不公平问题，这项工作不仅丰富了AI多样性和伦理讨论，还为开发更包容可靠的面部验证技术奠定了基础。
...
TL;DR: 提出分区自适应对比学习(PACL)方法，通过利用预训练文本模型的知识来增强视觉模型的情感感知能力，解决视觉情感识别中的'情感鸿沟'问题。


<details>
  <summary>Details</summary>
Motivation: 视觉情感识别中预训练视觉模型缺乏事实级特征与情感类别的直接关联，存在'情感鸿沟'，而文本模态具有明确的情感表达和高信息密度，可以弥补这一缺陷。

Method: 提出分区自适应对比学习(PACL)，关注社交媒体数据中图像与文本的事实和情感联系，分离不同类型的样本并为每种类型设计不同的对比学习策略，通过动态构建正负样本对充分利用噪声样本的潜力。
...
### [50] [ChainV: Atomic Visual Hints Make Multimodal Reasoning Shorter and Better](https://arxiv.org/abs/2511.17106)
*Yuan Zhang,Ming Lu,Junwen Pan,Tao Huang,Kuan Cheng,Qi She,Shanghang Zhang*

Main category: cs.CV

TL;DR: ChainV是一个动态整合视觉提示的多模态推理框架，通过粗粒度视觉补丁选择和基于注意力强度的精炼过程，结合一致性评估机制，显著提升推理准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有多模态推理模型在生成长推理链时存在冗余自反思问题，而基于静态视觉参考的CoT压缩方法在多模态推理中效果有限。
...
### [51] [PEGS: Physics-Event Enhanced Large Spatiotemporal Motion Reconstruction via 3D Gaussian Splatting](https://arxiv.org/abs/2511.17116)
*Yijun Xu,Jingrui Zhang,Hongyi Liu,Yuhan Chen,Yuanyang Wang,Qingyao Guo,Dingwen Wang,Lei Yu,Chu He*

Main category: cs.CV

TL;DR: PEGS是一个将物理先验与事件流增强集成到3D高斯泼溅管道中的框架，用于执行去模糊的目标聚焦建模和运动恢复，在大时空尺度上重建刚性运动方面表现出色。
...
Method: 提出了一个三重监督方案，通过加速度约束强制执行物理合理性，利用事件流进行高时间分辨率指导，并使用卡尔曼正则化器融合多源观测。还设计了基于实时运动状态自适应调度训练过程的运动感知模拟退火策略。

Result: 实验表明，与主流动态方法相比，PEGS在重建大时空尺度运动方面表现出优越性能。

Conclusion: PEGS框架成功解决了大时空尺度刚性运动重建的挑战，通过物理先验和事件流增强实现了高质量的运动恢复和建模。
...
TL;DR: 本文提出从1D相关色温(CCT)映射转向2D色度空间，并使用轻量级多层感知机(MLP)替代传统CCT插值，以提升非普朗克光源(如LED)下的色度精度。


<details>
  <summary>Details</summary>
Motivation: 传统基于CCT的色度映射方法主要针对普朗克光源(如CIE A和D65)进行优化，但现代LED光源可能显著偏离普朗克轨迹，暴露了依赖一维CCT进行光照表征的局限性。

Method: 采用2D色度空间替代1D CCT，使用轻量级MLP利用2D色度特征进行稳健的色度映射，通过包含代表性LED光源的光箱校准程序训练MLP。

Result: 在多样化LED照明下验证，该方法平均减少LED照明场景中22%的角度再现误差，保持与传统光源的向后兼容性，支持多光源场景，且实时部署计算成本可忽略。

Conclusion: 从1D CCT转向2D色度空间并结合MLP方法，能显著提升非普朗克光源下的色度精度，同时保持传统方法的优势。
...
### [53] [A Multi-Stage Optimization Framework for Deploying Learned Image Compression on FPGAs](https://arxiv.org/abs/2511.17135)
*Jiaxun Fang,Li Chen*
...
Method: 1. 动态范围感知量化(DRAQ)：使用统计校准激活裁剪和权重正则化处理极端数据异常值；2. 渐进混合精度搜索算法：为每层分配最优非均匀位宽；3. 通道剪枝方法：针对GDN层消除非活跃通道。

Result: DRAQ方法将GDN模型的BD-rate开销从30%降至6.3%；硬件感知优化进一步降低计算复杂度20%以上，对RD性能影响可忽略；最终模型在效率和性能上均优于现有FPGA实现。

Conclusion: 该框架成功实现了高性能图像压缩模型向FPGA的高效部署，在保持压缩质量的同时显著提升了硬件效率。
...
TL;DR: 本文介绍了RSFM数据库和REMSA智能体，用于解决遥感基础模型选择困难的问题。RS-FMD包含150多个模型，REMSA基于LLM实现自动化模型选择，并在专家验证的基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 遥感领域基础模型越来越多，但选择合适的模型面临文档分散、格式异构、部署约束多样等挑战，需要系统化的解决方案来简化模型选择过程。

Method: 构建RSFM数据库（RS-FMD）覆盖150多个模型，开发基于LLM的REMSA智能体，通过自然语言查询解析用户需求、补全缺失约束、使用上下文学习对候选模型排序，并提供透明解释。

Result: 在75个专家验证的查询场景和900个配置的基准测试中，REMSA优于多个基线方法（包括朴素智能体、密集检索和非结构化RAG的LLM），完全基于公开元数据运行。

Conclusion: RS-FMD和REMSA为遥感基础模型选择提供了系统化解决方案，显著提升了模型选择的效率和准确性，为遥感应用提供了实用的工具支持。
...
### [55] [One-Step Diffusion Transformer for Controllable Real-World Image Super-Resolution](https://arxiv.org/abs/2511.17138)
*Yushun Fang,Yuxiang Chen,Shibo Yin,Qiang Hu,Jiangchao Yao,Ya Zhang,Xiaoyun Zhang,Yanfeng Wang*

Main category: cs.CV

TL;DR: ODTSR是一种基于Qwen-Image的一步扩散变换器，通过噪声混合视觉流设计同时考虑真实图像超分辨率的保真度和可控性，无需特定数据集训练即可在中文场景文本图像超分辨率等挑战性场景中实现提示可控性。


<details>
  <summary>Details</summary>
Motivation: 现有扩散式真实图像超分辨率方法在保真度和可控性之间存在平衡问题：多步方法因生成多样性和随机性导致保真度低，而一步方法因保真度特定微调而失去控制灵活性。

Method: 提出噪声混合视觉流设计：新引入的视觉流接收带可调噪声的低质量图像，原始视觉流接收带一致噪声的低质量图像；采用保真度感知对抗训练增强可控性并实现一步推理。

Result: 大量实验表明，ODTSR不仅在通用真实图像超分辨率上达到最先进性能，还能在中文场景文本图像超分辨率等挑战性场景中实现提示可控性，无需特定数据集训练。

Conclusion: ODTSR成功解决了真实图像超分辨率中保真度与可控性的平衡问题，为扩散模型在真实世界应用提供了有效解决方案。
...
TL;DR: SketchVerify是一个无需训练的草图验证规划框架，通过测试时采样和验证循环来改进运动规划质量，生成更动态连贯的轨迹，然后进行完整的视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法主要使用单次规划或迭代细化，前者限于简单运动，后者计算成本高。需要一种既能提高运动规划质量又高效的方法。

Method: 给定提示和参考图像，预测多个候选运动计划，使用视觉语言验证器评估语义对齐和物理合理性。通过将轨迹渲染为轻量级视频草图来高效评分，避免昂贵的重复扩散合成。

Result: 在WorldModelBench和PhyWorldBench上的实验表明，该方法显著提高了运动质量、物理真实性和长期一致性，同时效率更高。扩展轨迹候选数量能持续提升性能。

Conclusion: SketchVerify框架通过草图验证规划有效提升了视频生成的运动质量和物理合理性，同时保持了高效率。
...
TL;DR: 提出CC-DiceCE损失函数解决医学图像分割中传统Dice损失对小病灶分割不足的问题，基于CC-Metrics框架，在nnU-Net中与blob损失对比，提高检测召回率但略微增加假阳性。


<details>
  <summary>Details</summary>
Motivation: 传统医学图像分割损失函数（如Dice）由于小病灶相对体积小，对整体损失贡献可忽略，导致对小病灶分割不足，需要基于病灶实例的损失函数和评估指标。

Method: 基于CC-Metrics框架开发CC-DiceCE损失函数，在nnU-Net标准化框架中与blob损失和DiceCE基线进行对比评估。

Result: CC-DiceCE损失提高了检测召回率，分割性能仅有最小程度下降，但略微增加了假阳性；在多数据集研究中，CC-DiceCE通常优于blob损失。

Conclusion: CC-DiceCE损失函数能有效改善小病灶检测性能，在保持分割质量的同时提高召回率，是优于现有blob损失的解决方案。
...
### [58] [FireScope: Wildfire Risk Prediction with a Chain-of-Thought Oracle](https://arxiv.org/abs/2511.17171)
*Mario Markov,Stefan Maria Ailuro,Luc Van Gool,Konrad Schindler,Danda Pani Paudel*

Main category: cs.CV

TL;DR: FireScope是一个基于视觉语言模型(VLM)的推理生成框架，通过结合强化学习和视觉监督来预测野火风险栅格图，并在跨大陆评估中展现出优异的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有野火风险预测方法缺乏因果推理和多模态理解能力，导致泛化性能不足。需要开发能够整合视觉、气候和地理因素的综合框架。

Method: 提出FireScope-Bench数据集和基准，结合Sentinel-2影像、气候数据和专家定义的风险栅格；开发FireScope框架，使用VLM进行推理生成，结合强化学习和视觉监督训练。

Result: 在美国训练并在欧洲测试时，FireScope取得显著性能提升；专家反馈和自动分析确认其推理轨迹具有忠实性和语义意义。

Conclusion: 基于语言的推理可以提升视觉生成模型的泛化能力和可解释性；FireScope是首个展示语言推理能改善视觉生成泛化性的框架，也是首个可跨大陆应用的高分辨率野火风险模型。
...
### [59] [Investigating self-supervised representations for audio-visual deepfake detection](https://arxiv.org/abs/2511.17181)
...
Result: 发现大多数自监督特征能捕获深度伪造相关信息且具有互补性，模型主要关注语义上有意义的区域而非虚假伪影，但无法可靠地跨数据集泛化。

Conclusion: 自监督表示在深度伪造检测中既有潜力又面临根本挑战：虽然能学习有意义的模式，但实现稳健的跨域性能仍然困难。
...
### [60] [DiffRefiner: Coarse to Fine Trajectory Planning via Diffusion Refinement with Semantic Interaction for End to End Autonomous Driving](https://arxiv.org/abs/2511.17150)
...
<details>
  <summary>Details</summary>
Motivation: 现有的生成式方法（如扩散模型）通常基于人工设计的轨迹锚点或随机噪声进行去噪，存在改进空间。需要结合判别式方法提供更强指导来提升扩散模型的规划性能。
...
### [61] [UI-Styler: Ultrasound Image Style Transfer with Class-Aware Prompts for Cross-Device Diagnosis Using a Frozen Black-Box Inference Network](https://arxiv.org/abs/2511.17155)
*Nhat-Tuong Do-Tran,Ngoc-Hoang-Lam Le,Ching-Chun Huang*

Main category: cs.CV

TL;DR: UI-Styler是一个针对超声图像的类别感知风格迁移框架，通过模式匹配机制将目标图像纹理模式转移到源图像，同时保持源图像结构内容，并使用类别感知提示策略确保语义对齐。


<details>
  <summary>Details</summary>
Motivation: 超声图像在不同采集设备间存在外观差异，导致域偏移问题，影响固定黑盒下游推理模型的性能。现有无配对图像翻译方法在域适应过程中忽视类别特定的语义对齐，导致内容-类别映射错位，影响诊断准确性。

Method: 提出UI-Styler框架，使用模式匹配机制进行纹理模式迁移，保持源图像结构内容；引入基于目标域伪标签的类别感知提示策略，确保与诊断类别的准确语义对齐。

Result: 在超声跨设备任务上的广泛实验表明，UI-Styler在分布距离和下游任务（如分类和分割）中始终优于现有无配对图像翻译方法，达到最先进性能。

Conclusion: UI-Styler通过类别感知的风格迁移有效解决了超声图像跨设备域偏移问题，在保持结构内容的同时实现准确的语义对齐，显著提升了诊断任务的性能。
...
TL;DR: 本文提出了非参数概率鲁棒性(NPPR)，一种不依赖预定义扰动分布的更实用的概率鲁棒性度量方法，通过直接从数据中学习优化的扰动分布来进行保守的鲁棒性评估。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型对小的输入扰动很脆弱，现有的概率鲁棒性方法假设固定且已知的扰动分布，这在实践中不现实。

Method: 采用非参数统计建模范式，基于高斯混合模型(GMM)和多层感知机(MLP)头以及双三次上采样开发NPPR估计器，覆盖输入依赖和输入独立的扰动场景。

Result: 在CIFAR-10、CIFAR-100和Tiny ImageNet数据集上，使用ResNet18/50、WideResNet50和VGG16验证，NPPR相比现有方法提供更保守的鲁棒性估计，PR估计值降低达40%。

Conclusion: NPPR是一个更实用的鲁棒性度量标准，能够在分布不确定性下进行保守的概率鲁棒性评估。
...
### [63] [Navigating in the Dark: A Multimodal Framework and Dataset for Nighttime Traffic Sign Recognition](https://arxiv.org/abs/2511.17183)
*Aditya Mishra,Akshay Agarwal,Haroon Lone*

Main category: cs.CV

TL;DR: 本文提出了INTSD数据集和LENS-Net方法，用于解决夜间交通标志识别问题。INTSD是一个大规模印度夜间交通标志数据集，包含41个类别。LENS-Net结合自适应图像增强检测器和多模态CLIP-GCNN分类器，在夜间条件下实现鲁棒的交通标志识别。
...
Method: 1) 引入INTSD数据集：包含印度多个地区采集的街景夜间交通标志图像，涵盖41个类别和不同光照天气条件；2) 提出LENS-Net：集成自适应图像增强检测器进行联合光照校正和标志定位，随后使用结构化多模态CLIP-GCNN分类器，利用跨模态注意力和基于图的推理实现鲁棒识别。

Result: 该方法超越了现有框架，消融研究证实了其关键组件的有效性。数据集和代码已公开供研究使用。

Conclusion: INTSD数据集和LENS-Net方法为解决夜间交通标志识别问题提供了有效的解决方案，通过结合图像增强和多模态推理，显著提升了夜间条件下的识别性能。
...
### [64] [PostCam: Camera-Controllable Novel-View Video Generation with Query-Shared Cross-Attention](https://arxiv.org/abs/2511.17185)
*Yipeng Chen,Zhichao Ye,Zhenzhou Fang,Xinyu Chen,Xiaoyu Zhang,Jialing Liu,Nan Wang,Haomin Liu,Guofeng Zhang*

Main category: cs.CV

TL;DR: PostCam是一个用于动态场景中新颖视角视频生成的框架，支持拍摄后编辑相机轨迹。通过查询共享交叉注意力模块融合6-DoF相机位姿和2D渲染视频帧，提高控制精度和生成质量。
...
Method: 引入查询共享交叉注意力模块，融合6-DoF相机位姿和2D渲染视频帧控制信号；采用两阶段训练策略：先学习粗略相机控制，再结合视觉信息细化运动精度。

Result: 在真实世界和合成数据集上的实验表明，PostCam在相机控制精度和视角一致性上比最先进方法提升超过20%，并达到最高的视频生成质量。

Conclusion: PostCam通过统一的特征表示和两阶段训练策略，实现了更准确和灵活的相机运动操控，显著提升了动态场景中新颖视角视频的生成效果。
...
TL;DR: VLA-4D是一个具有4D感知能力的通用视觉-语言-动作模型，通过4D感知的视觉表示和时空动作表示，实现机器人操作的时空一致性控制。
...
Result: 在机器人操作的不同任务中验证了方法的优越性，实现了空间平滑和时间连贯的操作。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割中，机构间的异构隐私政策使得联合训练不可行，需要持续学习以避免灾难性遗忘。SAM模型虽然提供强大的零样本先验，但参数量大、计算开销高，限制了实际部署。

Method: 提出对齐层模块，轻量级地调整编码器-解码器特征分布，高效适配SAM到特定医学图像；在此基础上提出CA-SAM持续学习策略，自动适配对齐层以缓解灾难性遗忘，同时利用SAM的零样本先验保持对未见数据集的强性能。

Result: 在九个医学分割数据集上的持续学习场景实验中，CA-SAM实现了最先进的性能。

Conclusion: SAM范式在平衡计算效率和性能后具有很大潜力，CA-SAM方法通过轻量级对齐层和持续学习策略，有效解决了医学图像分割中的实际部署挑战。
...
<details>
  <summary>Details</summary>
Motivation: 解决体积CT面临的独特挑战，如极端token扩展、几何各向异性和弱临床监督，使标准Transformer和对比学习方法难以直接应用。
...
Conclusion: 该框架能够直接从鱼眼图像生成连续、不确定性感知的语义地图，有效解决了鱼眼图像BEV分割的挑战。
...
### [69] [Blind Deconvolution for Color Images Using Normalized Quaternion Kernels](https://arxiv.org/abs/2511.17253)
*Yuming Yang,Michael K. Ng,Zhigang Jia,Wei Wang*

Main category: cs.CV

TL;DR: 本文提出了一种基于四元数保真度项的颜色图像盲反卷积方法，通过四元数卷积核同时处理整体模糊和RGB通道间的未知依赖关系，显著改善了去模糊效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将彩色图像转为灰度图或分别处理各颜色通道，忽略了颜色通道间的关系，这限制了盲反卷积的效果。

Method: 设计了一个新颖的四元数保真度项，使用包含四个核的四元数卷积核：一个用于捕捉整体模糊的非负卷积核，三个无约束的卷积核分别对应RGB通道以建模其未知依赖关系，并在盲反卷积过程中使用归一化四元数核来保持图像强度。

Result: 在真实模糊彩色图像数据集上的大量实验表明，该方法有效消除了伪影，显著改善了去模糊效果。

Conclusion: 该方法作为彩色图像反卷积的强大工具具有重要潜力，能够有效处理颜色通道间的关系并提升去模糊性能。
...
### [70] [A Little More Like This: Text-to-Image Retrieval with Vision-Language Models Using Relevance Feedback](https://arxiv.org/abs/2511.17255)
*Bulat Khaertdinov,Mirela Popa,Nava Tintarev*

Main category: cs.CV

TL;DR: 本文提出了一种基于相关性反馈的机制来改进视觉语言模型在推理时的检索性能，包括四种反馈策略：伪相关性反馈、生成式相关性反馈、注意力反馈总结器和显式反馈基准。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然支持自然语言查询的视觉搜索，但提升性能通常需要微调或使用更大模型。本文受传统文本搜索启发，旨在通过推理时的相关性反馈机制来改进检索性能，这种模型无关的设计也可与微调后的视觉语言模型结合使用。

Method: 提出了四种相关性反馈策略：1）伪相关性反馈（PRF）基于排名靠前的结果优化查询嵌入；2）生成式相关性反馈（GRF）使用合成标题进行查询优化；3）注意力反馈总结器（AFS）整合相关项目的多模态细粒度特征；4）使用真实标题作为显式反馈的上限基准。

Result: 在Flickr30k和COCO数据集上的实验表明，GRF、AFS和显式反馈相比无反馈检索，对较小视觉语言模型的MRR@5提升了3-5%，对较大模型提升了1-3%。AFS与显式反馈类似，能减轻查询漂移，在迭代多轮检索设置中比GRF更稳健。
...
Result: 实验结果表明，MolSight在（立体）化学光学结构识别方面达到了最先进的性能，即使参数规模相对紧凑，GRPO算法也能进一步提升模型在立体分子上的表现。

Conclusion: MolSight框架通过创新的三阶段训练方法，成功解决了OCSR中立体化学信息识别的关键挑战，为化学信息学、药物发现和大型语言模型应用提供了有效的解决方案。
...
TL;DR: BiFingerPose是一种基于双模态的指尖姿态估计算法，结合电容图像和指纹补丁，能够准确预测全面的指尖姿态信息，包括之前无法实现的滚转角，并在预测性能、任务完成效率和用户操作准确性方面显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于电容图像的指尖姿态估计算法在便携设备上实现有限，只能估计俯仰角和偏航角，且在大角度输入时精度下降。需要一种能够同时准确预测全面指尖姿态信息的方法。

Method: 提出BiFingerPose算法，探索双模态输入，包括来自触摸屏的电容图像和屏下指纹传感器获取的指纹补丁。这种方法能够可靠地估计滚转角，并显著提高其他姿态参数的预测性能。

Result: 在12人用户研究中，BiFingerPose相比之前SOTA方法在预测性能上提升超过21%，任务完成效率提高2.5倍，用户操作准确性提高23%。算法在连续和离散交互任务中均表现出优势。

Conclusion: BiFingerPose通过双模态方法实现了全面准确的指尖姿态估计，展示了在增强认证安全性和改善交互体验方面的应用潜力，开发了相应原型展示交互可能性。
...
TL;DR: 提出了SpatialGeo方法，通过层次化融合几何和语义特征来增强多模态大语言模型的空间推理能力，在空间推理任务中准确率提升至少8.0%，推理内存成本减少约50%。
...
Method: 提出基于几何和语义特征层次化融合的视觉编码器，通过层次适配器将自监督学习的几何特征与CLIP语义特征互补，使用随机特征丢弃训练避免仅依赖CLIP编码器的平凡解。

Result: 在SpatialRGPT-Bench上准确率提升至少8.0%，推理内存成本减少约50%。

Conclusion: SpatialGeo有效提升了MLLMs的空间感知能力，通过几何特征补充解决了空间模糊性问题。
...
TL;DR: NoPe-NeRF++是一种新颖的局部到全局优化算法，用于无需姿态先验训练神经辐射场（NeRF）。该方法通过相对姿态初始化、局部联合优化和全局捆绑调整，显著提高了姿态估计精度和NeRF质量，在基准数据集上表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法（特别是NoPe-NeRF）仅关注图像内的局部关系，在复杂场景中难以恢复准确的相机姿态。为了解决这一挑战，需要结合局部和全局线索来改进姿态估计和NeRF表示。

Method: 1. 通过显式特征匹配进行相对姿态初始化；2. 局部联合优化以增强姿态估计；3. 引入全局优化阶段，通过捆绑调整整合特征轨迹来进一步优化姿态；4. 将局部和全局线索与NeRF无缝结合。

Result: 该方法在姿态估计精度和新视角合成方面均优于现有最先进方法，在基准数据集上表现出优越性能和鲁棒性，即使在挑战性场景中也能有效工作。

Conclusion: NoPe-NeRF++成功地将局部和全局优化策略与NeRF相结合，显著提高了无需姿态先验的NeRF训练质量，验证了设计选择的合理性。
...
<details>
  <summary>Details</summary>
Motivation: 当前生成式图像模型在合成透明物体时表现不佳，特别是折射效果，因为它们未能充分学习光学规律。折射像素光线经常与图像其他部分的表面相交，对颜色产生约束。
...
Conclusion: 提出的方法显著提高了生成图像中透明物体折射效果的物理准确性，通过同步像素和光学定律的应用实现了更逼真的折射渲染。
...
TL;DR: 提出了一个统一的框架，用于生成多媒体绘画过程，通过语义驱动的风格控制机制嵌入多种媒介到扩散模型的条件空间，并使用跨媒介风格增强，实现一致的纹理演化和过程转移。
...
Method: 使用语义驱动的风格控制机制将多种媒介嵌入扩散模型的条件空间，采用跨媒介风格增强和反向绘画训练策略，确保平滑且符合人类创作习惯的生成过程。

Result: 构建了大规模真实绘画过程数据集，在跨媒介一致性、时间连贯性和最终图像保真度方面取得了良好结果，在LPIPS、DINO和CLIP指标上表现优异。

Conclusion: 提出的感知距离剖面曲线能够量化建模创作序列，反映人类艺术创作过程中的构图、色块划分和细节精炼等阶段，有效模拟艺术创作进展。
...
TL;DR: DSeq-JEPA是一种改进的视觉表示学习方法，通过识别主要判别区域并按顺序预测，结合了JEPA的潜在预测和GPT风格的序列推理，在多项视觉任务中表现优于I-JEPA变体。


<details>
  <summary>Details</summary>
Motivation: 受人类视觉感知的启发，传统I-JEPA方法对所有区域进行统一和独立的处理，缺乏对预测顺序和位置的明确指导。人类视觉系统会从最具信息量的区域开始，选择性、顺序性地部署注意力。

Method: DSeq-JEPA首先基于Transformer衍生的显著性图识别主要判别区域，然后按此判别顺序预测后续区域，形成从主要到次要线索的课程式语义进展，整合了JEPA风格的潜在预测和GPT风格的序列推理。

Result: 在图像分类、细粒度视觉分类、检测与分割以及低级推理任务上的广泛实验表明，DSeq-JEPA始终比I-JEPA变体关注更具判别性和泛化性的表示。

Conclusion: DSeq-JEPA通过引入判别性顺序预测，成功地将预测性和自回归自监督学习相结合，在多种视觉任务中实现了更优的性能。
...
TL;DR: SuperQuadricOcc提出了一种基于超二次曲面的语义占用估计方法，相比高斯表示方法，在保持竞争力的同时实现了75%内存占用减少、124%推理速度提升和5.9% mIoU改进。


<details>
  <summary>Details</summary>
Motivation: 高斯表示在自监督占用估计中广泛使用，但大量高斯基元显著增加内存需求，不适合实时推理。超二次曲面允许减少基元数量和降低内存需求，但缺乏超二次曲面光栅化器来支持模型监督。

Method: 采用超二次曲面场景表示，通过多层二十面体细分的高斯近似超二次曲面，实现训练期间的高斯光栅化监督。

Result: 在Occ3D数据集上，相比先前高斯方法实现了75%内存占用减少、124%推理速度提升和5.9% mIoU改进，无需使用时间标签。基元数量减少84%。

Conclusion: 这是第一个在保持竞争力性能的同时实现实时推理的占用模型，超二次曲面的使用显著减少了场景建模所需的基元数量。
...
<details>
  <summary>Details</summary>
Motivation: CLIP在零样本图像-文本匹配方面表现出色，但对图像对抗性扰动高度脆弱。由于对抗性微调成本过高，现有测试时防御方法鲁棒性有限。

Method: 在CLIP嵌入空间中操作，计算增强诱导的漂移向量来推断语义恢复方向，基于这些潜在漂移的角度一致性来纠正嵌入。

Result: 在各种基准测试中，ATAC始终实现极高的鲁棒性，平均比先前最先进方法高出近50%，同时计算开销极小。在非常规和极端设置下保持最先进的鲁棒性，甚至对自适应攻击也实现了非平凡鲁棒性。
...
TL;DR: SVRecon将稀疏体素栅格化与符号距离函数(SDF)结合用于高保真表面重建，通过几何初始化和空间平滑损失解决稀疏体素优化中的局部最小值问题。
...
Method: 1. 使用视觉几何模型进行鲁棒的几何初始化；2. 通过空间平滑损失强制父-子体素组和兄弟体素组之间的相干关系。

Result: 在各种基准测试上的广泛实验表明，该方法实现了强大的重建精度，同时具有持续快速的收敛速度。

Conclusion: SVRecon方法通过结合稀疏体素和SDF，成功解决了稀疏体素优化中的局部最小值问题，实现了高精度且快速收敛的表面重建。
...
<details>
  <summary>Details</summary>
Motivation: 解决医学图像配准中高维变形空间和体素级监督稀缺的挑战，现有强化学习框架将空间投影到粗糙的低维表示中，限制了捕捉空间变异变形的能力。

Method: 在编码器顶部引入随机高斯策略头来建模潜在特征的分布，集成无监督预热和弱监督微调，通过组相对策略优化实现多轨迹采样以稳定训练。

Result: 在三个3D配准基准测试中（OASIS脑部MRI、LiTS肝脏CT和腹部MR-CT），相比竞争基线实现了持续的Dice改进，同时保持高标签效率和低参数成本。
...
TL;DR: 提出MMT-ARD多模态多教师对抗鲁棒蒸馏框架，通过双教师知识融合架构提升视觉语言模型的对抗鲁棒性，在ImageNet和零样本基准上显著提升鲁棒准确率和训练效率。
...
Method: 采用双教师知识融合架构，协同优化清洁特征保留和鲁棒特征增强；引入基于教师置信度的动态权重分配策略处理困难对抗样本；设计自适应sigmoid加权函数平衡跨模态知识传递强度。
...
<details>
  <summary>Details</summary>
Motivation: 当前世界模型主要基于事实观察进行前向模拟，但许多应用需要回答反事实查询，如"如果移除这个物体会发生什么"，因此需要能够处理假设性场景修改的反事实世界模型。
...
TL;DR: Radar2Shape是一个去噪扩散模型，能够从部分可观测的雷达信号进行3D重建，通过将雷达频率与多分辨率形状特征相关联，在有限视角下成功重建任意3D形状。
...
Method: 采用两阶段方法：首先学习具有层次分辨率形状特征的规范化潜在空间，然后通过以类似粗到细的方式基于雷达信号频率进行扩散，进入该潜在空间。

Result: Radar2Shape能够成功从部分观测的雷达信号重建任意3D形状，在两个不同模拟方法和真实数据上表现出鲁棒的泛化能力。

Conclusion: 该方法为高频雷达领域的3D重建提供了有效解决方案，并发布了两个合成基准数据集以促进该领域的未来研究，使模型能够安全地适应到真实雷达系统中。
...
<details>
  <summary>Details</summary>
Motivation: 脊柱是人体重要结构，易受年龄相关退行性变影响。需要开发自动方法来评估脊柱健康状况，识别退行性变化。

Method: 使用UMAP和HDBSCAN聚类识别年龄相关脊柱退行性疾病，通过消融研究选择模型，计算脊柱年龄差(SAG)来评估临床效用。

Result: 发现SAG与椎间盘突出、骨赘、椎管狭窄、骨折等退行性疾病相关，也与吸烟、体力劳动等生活方式因素相关。

Conclusion: 脊柱年龄差(SAG)可作为衡量整体脊柱健康的有用生物标志物，有助于评估退行性变化和生活方式影响。
...
TL;DR: 本文分析了多模态模型中缩小LLM规模对视觉能力的影响，发现视觉能力比推理能力下降更严重，并提出了Extract+Think方法来解决这一瓶颈。


<details>
  <summary>Details</summary>
Motivation: 实际应用需要更小、更高效的多模态系统，但缩小LLM规模会不成比例地影响视觉能力而非推理能力，需要解决这一性能下降问题。

Method: 引入视觉提取调优，明确训练模型提取与指令相关的视觉细节，然后应用逐步推理生成答案，形成Extract+Think方法。

Result: 研究发现LLM缩小主要导致视觉感知能力严重下降，而Extract+Think方法在该领域建立了效率和性能的新标准。

Conclusion: Extract+Think方法通过分离视觉提取和推理步骤，有效解决了多模态模型缩小规模时的视觉能力瓶颈问题。
...
TL;DR: Video-R4是一个视频推理LMM，通过视觉反刍机制迭代选择帧、放大信息区域、重新编码像素并更新推理状态，解决了文本丰富视频中细粒度证据识别的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频QA模型依赖单次感知固定帧，导致幻觉和细粒度证据识别失败。受人类暂停、放大和重读关键区域的行为启发，需要迭代视觉反刍机制。

Method: 构建两个数据集用于监督练习和强化学习，采用多阶段反刍学习框架，通过SFT和GRPO-based RL逐步微调7B LMM学习原子和混合视觉操作。
...
### [89] [PhysMorph-GS: Differentiable Shape Morphing via Joint Optimization of Physics and Rendering Objectives](https://arxiv.org/abs/2511.16988)
*Chang-Yong Song,David Hyde*

Main category: cs.GR

TL;DR: PhysMorph-GS通过可微分材料点法（MPM）与3D高斯泼溅的耦合，解决了物理模拟中表面提取不可微分导致的渲染差距问题，实现了从图像损失直接指导物理优化的形状变形。


<details>
  <summary>Details</summary>
Motivation: 现有物理模拟方法存在"渲染差距"：不可微分的表面提取阻碍了图像损失直接指导物理优化，需要一种能够将稀疏粒子状态映射到密集高斯表示的方法。

Method: 提出变形感知上采样桥，将稀疏粒子状态（位置x，变形梯度F）映射到密集高斯（均值mu，协方差Sigma）；通过多模态渲染损失（轮廓和深度）沿两条路径反向传播；使用多通道交错优化方案将渲染梯度注入连续物理步骤。

Result: 在挑战性变形序列上，PhysMorph-GS相比可微分MPM基线提高了边界保真度和时间稳定性，更好地重建了耳朵和尾巴等薄结构；深度监督变体将Chamfer距离减少了约2.5%。

Conclusion: 通过提供可微分粒子到高斯桥，PhysMorph-GS填补了物理感知渲染管道中的关键差距，实现了直接从图像空间监督进行逆向设计。
...
### [90] [A*-based Temporal Logic Path Planning with User Preferences on Relaxed Task Satisfaction](https://arxiv.org/abs/2511.16844)
...
### [91] [Single-Pixel Tactile Skin via Compressive Sampling](https://arxiv.org/abs/2511.16898)
*Ariel Slepyan,Laura Xing,Rudy Zhang,Nitish Thakor*

Main category: cs.RO

TL;DR: SPTS是一种单像素触觉皮肤系统，通过压缩采样技术从整个传感器阵列中重建丰富的触觉信息，仅使用单一输出通道，显著简化了布线复杂性和数据瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 开发大面积、高速电子皮肤面临布线复杂性和数据瓶颈的根本限制，这是机器人技术、假肢和人机界面的重大挑战。

Method: 采用直接电路级实现，每个传感元件配备微型微控制器，向全局总和贡献动态加权的模拟信号，在硬件中执行分布式压缩感知。采用灵活的可菊花链设计，将布线简化为几条输入线和一条输出线。

Result: 系统实现了3500 FPS的有效物体分类，并捕获瞬态动力学，将8毫秒的弹丸撞击解析为23帧。支持自适应重建，仅使用7%的总数据即可实现快速接触定位，随后逐步细化到高保真图像。
...
### [92] [Multi-UAV Swarm Obstacle Avoidance Based on Potential Field Optimization](https://arxiv.org/abs/2511.16911)
*Yendo Hu,Yiliang Wu,Weican Chen*

Main category: cs.RO

TL;DR: 提出了一种结合改进多机器人编队避障算法和增强人工势场法的混合算法，用于解决多无人机场景中传统APF方法导致的路径冗余、航向突变和无人机间碰撞问题。
...
Method: 结合改进的多机器人编队避障算法和增强的单无人机路径规划APF，整合三种交互力（障碍排斥力、无人机间交互力、目标吸引力），并引入碰撞风险评估和辅助子目标策略，在面临高碰撞威胁时生成临时航点引导避障。

Result: 仿真结果表明，相比传统基于APF的编队算法，所提算法在路径长度优化和航向稳定性方面取得显著改进，能有效避障并快速恢复编队构型。

Conclusion: 该算法在未知障碍物的静态环境中具有适用性和有效性，验证了其优越性能。
...
### [93] [MobileOcc: A Human-Aware Semantic Occupancy Dataset for Mobile Robots](https://arxiv.org/abs/2511.16949)
*Junseo Kim,Guido Dumont,Xinyu Gao,Gang Chen,Holger Caesar,Javier Alonso-Mora*

Main category: cs.RO

TL;DR: 提出了MobileOcc数据集，用于移动机器人在拥挤人类环境中的语义占据感知，包含静态物体占据标注和专门为人体占据建模设计的网格优化框架。


<details>
  <summary>Details</summary>
Motivation: 密集3D语义占据感知在移动机器人领域研究不足，特别是在行人密集环境中，相比自动驾驶应用存在差距。

Method: 构建了包含静态物体占据标注的数据集，开发了新颖的网格优化框架，从2D图像重建可变形人体几何，并使用LiDAR点云数据进行优化。

Result: 建立了占据预测和行人速度预测两个任务的基准，评估了单目、立体和全景占据等方法，并在3D人体姿态估计数据集上验证了标注方法的有效性。

Conclusion: 该方法在不同数据集上表现出稳健性能，为移动机器人在拥挤人类环境中的语义占据感知提供了有效解决方案。
...
### [94] [Stable Offline Hand-Eye Calibration for any Robot with Just One Mark](https://arxiv.org/abs/2511.17001)
*Sicheng Xie,Lingchen Meng,Zhiying Du,Shuyuan Tu,Haidong Cao,Jiaqi Leng,Zuxuan Wu,Yu-Gang Jiang*

Main category: cs.RO

TL;DR: CalibAll是一种简单有效的相机外参估计方法，仅需单个标记即可在多样机器人平台上实现免训练的稳定准确校准，通过粗到细的校准流程解决相机外参缺失问题。


<details>
  <summary>Details</summary>
Motivation: 模仿学习在机器人任务中取得了显著成功，但相机外参信息通常不可得，现有估计方法存在局部最优和泛化能力差的问题。

Method: 在末端执行器上标注单个标记，利用视觉基础模型的对应能力自动定位标记，结合点跟踪和3D轨迹通过时间PnP获得粗估计，再通过渲染优化对齐掩码进行精炼。

Result: 实验结果表明该方法优于现有最优方法，在三个机器人平台上表现出强鲁棒性和通用有效性，并能生成深度图、链接掩码等有用辅助标注。

Conclusion: CalibAll提供了一种简单可靠的相机外参估计解决方案，仅需单个标记即可实现跨平台的高精度校准，支持下游任务。
...
### [95] [MfNeuPAN: Proactive End-to-End Navigation in Dynamic Environments via Direct Multi-Frame Point Constraints](https://arxiv.org/abs/2511.17013)
*Yiwen Ying,Hanjing Ye,Senzi Luo,Luyao Liu,Yu Zhan,Li He,Hong Zhang*

Main category: cs.RO

TL;DR: 提出了一种利用多帧点约束的主动端到端导航框架，通过预测模块预测移动障碍物未来路径，增强机器人在未知动态环境中的导航鲁棒性和效率。
...
Method: 采用多帧点约束框架，包含当前帧和预测模块预测的未来帧，通过多帧观测预测移动障碍物未来路径，实现主动规划。

Result: 仿真和真实世界实验验证了方法的有效性，显著提升了在未知动态环境中的导航鲁棒性和效率。

Conclusion: 提出的多帧约束主动导航框架成功解决了动态环境中的实时避障问题，为机器人导航提供了更可靠的解决方案。
...
### [96] [H-GAR: A Hierarchical Interaction Framework via Goal-Driven Observation-Action Refinement for Robotic Manipulation](https://arxiv.org/abs/2511.17079)
*Yijie Zhu,Rui Shao,Ziyang Liu,Jie He,Jizhihui Liu,Jiuru Wang,Zitong Yu*

Main category: cs.RO

TL;DR: H-GAR提出了一种分层交互框架，通过目标驱动的观察-动作精炼来解决机器人操作中的视频和动作预测问题。该方法首先生成目标观察和粗略动作草图，然后通过两个协同模块进行细粒度精炼，实现更准确的操作。


<details>
  <summary>Details</summary>
Motivation: 现有的统一视频和动作预测方法通常以整体和目标无关的方式处理观察和动作生成，导致语义不匹配的预测和不连贯的行为。需要一种能够将预测锚定到任务目标并实现观察与动作显式交互的方法。

Method: H-GAR采用分层方法：1）首先生成目标观察和粗略动作草图；2）目标条件观察合成器基于粗略动作和预测目标合成中间观察；3）交互感知动作精炼器利用中间观察反馈和历史动作记忆库精炼动作，确保时间一致性。
...
### [97] [Progress-Think: Semantic Progress Reasoning for Vision-Language Navigation](https://arxiv.org/abs/2511.17097)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言动作模型关注直接动作预测，早期进度方法预测数值成就，都忽视了观察序列和指令序列的单调共进特性。

Method: 三阶段框架：1) 自对齐进度预训练通过视觉历史和指令前缀的微分对齐引导推理模块；2) 进度引导策略预训练将学习到的进度状态注入导航上下文；3) 进度策略协同微调使用进度感知强化目标联合优化两个模块。
...
### [98] [Reflection-Based Relative Localization for Cooperative UAV Teams Using Active Markers](https://arxiv.org/abs/2511.17166)
*Tim Lakemann,Daniel Bonilla Licea,Viktor Walter,Martin Saska*

Main category: cs.RO

TL;DR: 提出一种利用主动标记在环境中的反射进行多机器人相对定位的新方法，无需机器人尺寸或标记配置的先验知识，在室内外实验中验证了其有效性和高精度。


<details>
  <summary>Details</summary>
Motivation: 解决多机器人团队中主动标记反射造成的定位模糊问题，特别是在未知环境中工作的异构微型空中蜂群需要不依赖表面特性的定位方法。

Method: 利用主动标记在环境中的反射进行相对定位，无需机器人尺寸或预定义标记配置的先验知识，特别考虑了非平坦表面（尤其是动态水面）引起的不确定性。

Result: 在室内外实验中验证了方法的可靠性，相比现有技术具有更大的有效范围（超过30米）和更高的精度。

Conclusion: 提出的基于反射的定位系统能够在无需团队成员尺寸先验知识的情况下可靠运行，为海洋部署等场景提供了有效的解决方案。
...
### [99] [Efficient Robot Design with Multi-Objective Black-Box Optimization and Large Language Models](https://arxiv.org/abs/2511.17178)
*Kento Kawaharazuka,Yoshiki Obinata,Naoaki Kanazawa,Haoyu Jia,Kei Okada*

Main category: cs.RO

TL;DR: 提出了一种利用大语言模型增强机器人身体设计黑盒优化效率的方法，通过结合黑盒优化和LLM采样来提高设计解决方案的探索效率。
...
Method: 在黑盒优化采样过程的同时，使用大语言模型进行采样，为LLM提供问题设置和广泛反馈。

Result: 该方法能够更有效地探索设计解决方案。

Conclusion: 讨论了该方法的特性和局限性，证明了利用LLM可以增强机器人设计优化的效率。
...
### [100] [A ROS2 Interface for Universal Robots Collaborative Manipulators Based on ur_rtde](https://arxiv.org/abs/2511.17237)
*Alessio Saccuti,Riccardo Monica,Jacopo Aleotti*

Main category: cs.RO

TL;DR: 提出了一种基于ur_rtde C++库的新型ROS2驱动程序，用于UR机器人操纵器，具有灵活性和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 为UR机器人提供一个灵活、可适应多种应用的ROS2驱动解决方案。

Method: 基于ur_rtde C++库开发，暴露URScripts的高级命令，并通过插件系统支持自定义命令。

Result: 实现了多种命令，包括基于路径点的运动执行，并将驱动程序开源发布。

Conclusion: 成功开发了一个功能丰富、可扩展的UR机器人ROS2驱动程序。
...
### [101] [Simulation of Active Soft Nets for Capture of Space Debris](https://arxiv.org/abs/2511.17266)
*Leone Costi,Dario Izzo*

Main category: cs.RO

TL;DR: 基于MuJoCo物理引擎开发软机器人网模拟器，用于太空碎片自主清除，重点研究不同机械模型和控制策略对捕获Envisat卫星碎片的效果。


<details>
  <summary>Details</summary>
Motivation: 开发软机器人网模拟器来解决太空碎片清除问题，特别是针对大型ESA卫星Envisat的捕获，填补了现有研究中从静态配置开始而非弹道投掷的空白。

Method: 使用MuJoCo物理引擎构建模拟器，包含网动力学、网与碎片接触、网自接触、轨道力学和控制器，研究不同柔顺度机械模型和基于相对位置的控制策略。

Result: 更柔顺的网在捕获Envisat时表现更好，结合滑模控制器时软网在100%测试案例中成功捕获，具有更大的有效接触面积和更多接触点。

Conclusion: 软网配合滑模控制器是实现太空碎片高效捕获的有效方案，柔顺性对捕获性能有显著积极影响。
...
### [102] [Leveraging CVAE for Joint Configuration Estimation of Multifingered Grippers from Point Cloud Data](https://arxiv.org/abs/2511.17276)
*Julien Merand,Boris Meden,Mathieu Grossard*

Main category: cs.RO

TL;DR: 本文提出了一种基于点云数据的多指抓取器关节配置高效确定方法，使用条件变分自编码器从关键结构元素的点云输入中重建对应的关节配置。


<details>
  <summary>Details</summary>
Motivation: 传统逆运动学方法需要基于指尖位姿进行数学精确求解，但往往需要后验决策考虑所有中间指节位置，或依赖数值近似算法处理复杂运动学。本文旨在通过机器学习隐式克服这些挑战。

Method: 采用条件变分自编码器(CVAE)，以关键结构元素的点云数据作为输入，重建对应的关节配置。在MultiDex抓取数据集上使用Allegro Hand进行验证。

Result: 方法在0.05毫秒内运行，准确率与最先进方法相当，证明了在AI驱动的抓取规划技术中关节配置估计的有效性。

Conclusion: 该方法为多指抓取器的关节配置确定提供了一种高效的机器学习解决方案，在抓取规划领域具有重要应用价值。
...
### [103] [MonoSpheres: Large-Scale Monocular SLAM-Based UAV Exploration through Perception-Coupled Mapping and Planning](https://arxiv.org/abs/2511.17299)
...
<details>
  <summary>Details</summary>
Motivation: 解决仅配备单目相机的移动机器人在未知环境中的自主探索问题，这是目前尚未完全解决的关键能力。

Method: 通过显式考虑稀疏单目SLAM前端的特性，在映射模块中处理稀疏深度数据、自由空间间隙和大深度不确定性，在规划模块中通过快速重新规划和感知感知的航向控制来处理自由空间不确定性。

Result: 在多样化的真实世界和模拟环境中进行了广泛评估，包括消融研究。据作者所知，这是首个在真实世界非结构化室外环境中实现3D单目探索的方法。

Conclusion: 证明了基于稀疏单目深度数据的边界探索是可行的，并开源了实现以支持未来研究。
...
### [104] [FORWARD: Dataset of a forwarder operating in rough terrain](https://arxiv.org/abs/2511.17318)
...
TL;DR: FORWARD是一个高分辨率多模态数据集，记录了瑞典中部两个采伐现场的伐木集材机在崎岖地形中的操作数据，包含RTK-GNSS、360度摄像头、IMU等多种传感器数据，旨在开发林业机械的通行性、感知和自主控制模型。


<details>
  <summary>Details</summary>
Motivation: 开发林业机械的自主控制系统需要高质量的真实操作数据，特别是在崎岖地形中的操作数据，以支持人工智能、仿真和物理测试平台的研究。

Method: 使用装备多种传感器的大型Komatsu集材机，在瑞典中部两个采伐现场收集数据，包括RTK-GNSS、360度摄像头、振动传感器、CAN总线信号和多个IMU，数据采集频率为5Hz，同时包含激光扫描的高分辨率地形数据。

Result: 收集了约18小时的常规木材采伐工作数据，包括在森林道路和地形中进行的实验场景，如使用/不使用钢轨、不同负载重量和不同目标行驶速度等条件下的重复路线行驶数据。

Conclusion: 该开放数据集为开发林业机械的通行性、感知和自主控制算法提供了宝贵资源，同时支持林业机械模拟器的自动生成和校准，以及自动化场景描述的探索。
...
### [105] [Robot Confirmation Generation and Action Planning Using Long-context Q-Former Integrated with Multimodal LLM](https://arxiv.org/abs/2511.17335)
*Chiori Hori,Yoshiki Masuyama,Siddarth Jain,Radu Corcodel,Devesh Jha,Diego Romeres,Jonathan Le Roux*

Main category: cs.RO

TL;DR: 本文提出了一种长上下文Q-former方法，通过整合视频的左右上下文依赖关系来改进人机交互中的动作确认和动作规划性能，并采用文本条件化方法直接向LLM解码器输入文本嵌入以缓解信息抽象问题。
...
Method: 提出长上下文Q-former整合完整视频中的左右上下文依赖关系，并采用文本条件化方法直接向LLM解码器输入文本嵌入。

Result: 在YouCook2语料库上的实验表明，确认生成的准确性是动作规划性能的主要因素，长上下文Q-former通过整合VideoLLaMA3改进了确认和动作规划。

Conclusion: 长上下文Q-former方法能够有效提升人机交互中动作确认和动作规划的性能，验证了长上下文信息整合的重要性。
...
### [106] [METIS: Multi-Source Egocentric Training for Integrated Dexterous Vision-Language-Action Model](https://arxiv.org/abs/2511.17366)
*Yankai Fu,Ning Chen,Junkai Zhao,Shaozhe Shan,Guocai Yao,Pengwei Wang,Zhongyuan Wang,Shanghang Zhang*

Main category: cs.RO

TL;DR: METIS是一个用于灵巧操作的视觉-语言-动作模型，通过整合多源自我中心数据集，构建统一的动作空间和运动感知动态表示，在六项真实世界任务中达到最高平均成功率。


<details>
  <summary>Details</summary>
Motivation: 构建能够跨任务感知、推理和行动的通用机器人仍面临挑战，特别是灵巧操作领域。主要瓶颈在于缺乏大规模动作标注数据，而人类数据提供了丰富的操作行为先验。

Method: 提出METIS模型，首先构建EgoAtlas数据集整合多源人类和机器人数据，提取运动感知动态作为紧凑离散化运动表示，在统一框架中集成推理和行动。

Result: 在六项真实世界灵巧操作任务中达到最高平均成功率，实验结果显示在分布外场景下具有优越的泛化能力和鲁棒性。
...
### [107] [Agility Meets Stability: Versatile Humanoid Control with Heterogeneous Data](https://arxiv.org/abs/2511.17373)
...
TL;DR: AMS框架首次将动态运动跟踪和极端平衡维护统一在单一策略中，利用人类动作捕捉数据和合成平衡动作，通过混合奖励方案和自适应学习策略实现敏捷性与稳定性的统一。


<details>
  <summary>Details</summary>
Motivation: 人形机器人需要在人类中心环境中执行各种任务，但现有方法要么专注于敏捷动态技能，要么专注于稳定性关键行为，缺乏统一框架。需要同时实现敏捷性和鲁棒平衡的控制器。

Method: 利用异构数据源：人类动作捕捉数据集提供丰富敏捷行为，物理约束的合成平衡动作捕捉稳定性配置。设计混合奖励方案，在所有数据上应用通用跟踪目标，仅在合成动作中注入平衡特定先验。采用自适应学习策略，包括性能驱动采样和动作特定奖励塑造。

Result: 在仿真和真实Unitree G1人形机器人上验证，单一策略能够执行舞蹈、跑步等敏捷技能，同时也能零样本执行Ip Man蹲等极端平衡动作。

Conclusion: AMS作为未来人形机器人应用的通用控制范式，成功统一了动态运动跟踪和平衡维护能力。
...
### [108] [Vector Cost Behavioral Planning for Autonomous Robotic Systems with Contemporary Validation Strategies](https://arxiv.org/abs/2511.17375)
...
<details>
  <summary>Details</summary>
Motivation: 现有的多目标决策方法往往无法同时优化多个目标并避免最坏情况，特别是在竞争性运动规划场景中。需要一种能够处理高维决策空间并提供可解释性的综合框架。

Method: 使用向量成本双矩阵游戏方法进行多目标决策，结合可解释人工智能（XAI）软件分析高维决策数据，应用SEMBAS探索参数空间中的性能模式，构建了一个综合的仿真管道。

Result: 向量成本方法相比标量化方法表现出显著改进，提供了一个可解释和可推广的机器人行为规划框架。

Conclusion: 向量成本方法在多目标决策中优于传统标量化方法，为机器人行为规划提供了一个全面且可解释的解决方案。
...
### [109] [IndustryNav: Exploring Spatial Reasoning of Embodied Agents in Dynamic Industrial Navigation](https://arxiv.org/abs/2511.17384)
*Yifan Li,Lichi Li,Anh Dao,Xinyu Zhou,Yicheng Qiao,Zheda Mai,Daeun Lee,Zichen Chen,Zhen Tan,Mohit Bansal,Yu Kong*

Main category: cs.RO

TL;DR: IndustryNav是首个动态工业导航基准，用于评估视觉大语言模型在动态仓库环境中的主动空间推理能力，包含12个高保真Unity场景，评估了9个最先进VLLM在路径规划、碰撞避免和安全行为方面的表现。


<details>
  <summary>Details</summary>
Motivation: 现有具身基准主要关注静态家庭环境，无法评估动态现实世界中的整体性能，需要填补动态工业导航评估的空白。

Method: 使用12个手动创建的高保真Unity仓库场景，结合PointGoal导航管道，将自我中心视觉与全局里程计结合，评估局部-全局规划，并引入碰撞率和警告率等安全指标。

Result: 闭源模型保持优势，但所有智能体在稳健路径规划、碰撞避免和主动探索方面都存在显著缺陷。

Conclusion: 具身研究需要超越被动感知，转向需要稳定规划、主动探索和动态现实环境中安全行为的任务。
...
### [110] [Feasibility of Embodied Dynamics Based Bayesian Learning for Continuous Pursuit Motion Control of Assistive Mobile Robots in the Built Environment](https://arxiv.org/abs/2511.17401)
...
<details>
  <summary>Details</summary>
Motivation: 当前脑机接口运动控制系统大多限于离散命令，缺乏支持连续追踪的能力，而连续控制对于轮椅用户在复杂环境中灵活导航至关重要。

Method: 采用脑启发的贝叶斯推理框架，解码基于加速度的运动表征中的体现动态，使用自动相关确定进行特征选择，并采用持续在线学习。

Result: 在包含4名受试者16小时EEG数据的公开数据集上，该方法在会话累积迁移学习设置中，相比自回归和EEGNet方法将归一化均方误差降低了72%。

Conclusion: 该方法从理论上支持体现认知理论，揭示了大脑内在运动控制的体现性和预测性本质，为实现更稳定直观的脑机接口控制提供了有前景的路径。
...
### [111] [SPEAR-1: Scaling Beyond Robot Demonstrations via 3D Understanding](https://arxiv.org/abs/2511.17411)
*Nikolay Nikolov,Giuliano Albanese,Sombit Dey,Aleksandar Yanev,Luc Van Gool,Jan-Nico Zaech,Danda Pani Paudel*

Main category: cs.RO

TL;DR: SPEAR-1是一个机器人基础模型，通过增强预训练视觉语言模型的3D空间理解能力，在减少20倍机器人演示数据的情况下，实现与最先进模型相当或更好的性能。
...
Method: 提出SPEAR-VLM，通过为易于收集的非机器人图像数据添加3D标注来增强预训练VLM的3D理解能力，然后基于此构建SPEAR-1模型，在24个Open X-Embodiment数据集上进行训练。

Result: SPEAR-1在仅使用1/20机器人演示数据的情况下，性能优于或匹配π₀-FAST和π₀.₅等最先进模型，证明了该方法能有效提升具身控制的可靠性。

Conclusion: 通过精心设计的训练策略解锁新的VLM能力，可以超越仅使用机器人数据所能达到的具身控制可靠性，模型权重和3D标注数据集已公开。
...
### [112] [RoboCOIN: An Open-Sourced Bimanual Robotic Data COllection for INtegrated Manipulation](https://arxiv.org/abs/2511.17441)
...
TL;DR: RoboCOIN是一个包含超过18万次演示的多平台双臂操作数据集，涵盖15个机器人平台和16个场景，通过分层能力金字塔提供多级标注，并开发了CoRobot处理框架用于质量评估和统一管理。


<details>
  <summary>Details</summary>
Motivation: 解决双臂操作数据集稀缺和机器人平台硬件异构性的问题，为机器人实现类人灵巧性提供大规模、多样化的数据支持。

Method: 构建包含180,000次演示的多平台双臂操作数据集，采用分层能力金字塔进行多级标注，开发Robot Trajectory Markup Language (RTML)进行质量评估和自动化标注生成。

Result: 实验证明RoboCOIN在多平台双臂学习中的可靠性和有效性，在各种模型架构和机器人平台上均显示出显著的性能提升。

Conclusion: RoboCOIN数据集和框架为双臂操作研究提供了全面的多平台解决方案，已开源供进一步研究使用。
...
### [113] [MDG: Masked Denoising Generation for Multi-Agent Behavior Modeling in Traffic Environments](https://arxiv.org/abs/2511.17496)
...
Method: MDG使用连续、按智能体和时间步的噪声掩码，实现局部去噪和可控轨迹生成，在单次或少量前向传播中完成，无需扩散时间步或离散标记化。

Result: 在Waymo Sim Agents和nuPlan Planning基准测试中达到竞争性闭环性能，同时提供高效、一致和可控的开环多智能体轨迹生成。

Conclusion: MDG作为一个简单而通用的范式，能够统一处理开环预测、闭环仿真、运动规划和条件生成等多种多智能体行为建模任务。
...
### [114] [HALO: High-Altitude Language-Conditioned Monocular Aerial Exploration and Navigation](https://arxiv.org/abs/2511.17497)
*Yuezhan Tao,Dexter Ong,Fernando Cladera,Jason Hughes,Camillo J. Taylor,Pratik Chaudhari,Vijay Kumar*

Main category: cs.RO

TL;DR: HALO系统使用单目相机、GPS和IMU实现实时高空航空度量语义建图与探索，能够在大型室外环境中进行密集3D重建，并通过自然语言指令完成多任务任务规划。
...
Method: 使用单目相机、GPS和IMU组合，开发了能够实时运行在机器人上的HALO系统，支持自然语言任务规划。

Result: 在模拟环境中，HALO在高达78,000平方米的大规模环境中完成任务，探索时间更少，相比最先进的语义探索基线，距离旅行竞争比提高高达68%。在真实世界实验中，在定制四旋翼平台上成功运行所有模块，在40米高度覆盖24,600平方米区域。

Conclusion: HALO系统证明了在大型室外环境中实现实时高空航空度量语义建图与探索的可行性，支持有效的自主任务执行。
...
### [115] [RynnVLA-002: A Unified Vision-Language-Action and World Model](https://arxiv.org/abs/2511.17502)
...
Method: 结合VLA模型（从图像观察生成动作）和世界模型（利用动作和视觉输入预测未来图像状态），通过联合学习实现相互增强。

Result: 在LIBERO模拟基准测试中达到97.4%的成功率（无需预训练），在真实世界LeRobot实验中，集成世界模型使整体成功率提升50%。

Conclusion: RynnVLA-002证明了VLA模型和世界模型的统一框架能够相互增强，在机器人任务中显著优于单独的模型。
...
### [116] [DDTime: Dataset Distillation with Spectral Alignment and Information Bottleneck for Time-Series Forecasting](https://arxiv.org/abs/2511.16715)
...
<details>
  <summary>Details</summary>
Motivation: 时间序列预测通常需要大规模数据集和大量计算资源，而数据集蒸馏可以合成紧凑的数据集来替代完整数据。但时间序列蒸馏面临两个挑战：时间偏差（强自相关导致的师生模型值项对齐失真）和样本多样性不足（缺乏明确的类别先验来规范轨迹多样性）。

Method: 基于一阶压缩分解构建DDTime框架：1）通过时间统计重新审视值项对齐，引入频域对齐机制减轻自相关引起的偏差；2）设计基于信息瓶颈原理的样本间正则化，增强合成轨迹的多样性和信息密度。

Result: 在20个基准数据集和多种预测架构上的实验表明，DDTime始终优于现有蒸馏方法，实现了约30%的相对准确率提升，同时仅引入约2.49%的计算开销。

Conclusion: DDTime是一个高效的时间序列数据集蒸馏框架，通过频域对齐和样本间正则化有效解决了时间序列蒸馏的关键挑战，在保持低计算开销的同时显著提升了预测性能。
...
### [117] [GCL-OT: Graph Contrastive Learning with Optimal Transport for Heterophilic Text-Attributed Graphs](https://arxiv.org/abs/2511.16778)
*Yating Ren,Yikun Ban,Huobin Tan*

Main category: cs.LG

TL;DR: 本文提出GCL-OT框架，通过最优传输解决文本属性图中的多粒度异质性挑战，实现结构和文本的灵活双向对齐。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖同质性假设和硬优化目标，在处理异质性图时表现受限，且通常将文本嵌入视为静态目标，导致对齐效果不佳。

Method: 提出GCL-OT框架，包含RealSoftMax相似性估计器处理部分异质性、基于提示的过滤器处理完全异质性，以及OT引导的软监督学习潜在同质性。

Result: 在九个基准测试上的实验表明，GCL-OT持续优于最先进方法，验证了其有效性和鲁棒性。

Conclusion: GCL-OT通过最优传输和针对性机制成功解决了文本属性图中的多粒度异质性挑战，提升了结构和文本的对齐效果。
...
### [118] [Revisiting Multimodal KV Cache Compression: A Frequency-Domain-Guided Outlier-KV-Aware Approach](https://arxiv.org/abs/2511.16786)
*Yaoxin Yang,Peng Ye,Xudong Tan,Chongjun Tu,Maosen Zhao,Jia Hao,Tao Chen*

Main category: cs.LG

TL;DR: FlashCache是一个基于频域分析和异常KV感知的多模态KV缓存压缩框架，通过识别和保留偏离主成分的异常KV对，在保持任务性能的同时显著降低KV内存使用并加速解码。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型存在显著推理开销，因为多模态KV缓存随视觉输入长度比例增长。现有压缩方法主要依赖注意力分数，与高效注意力内核不兼容且忽略了值向量对注意力输出的贡献。

Method: 从KV矩阵分布角度重新审视多模态KV缓存压缩：1）通过低通滤波器提取KV矩阵频域能量的主成分；2）提出异常KV识别模块，优先保留显著偏离主成分的KV对；3）设计动态预算分配模块自适应确定每层KV缓存大小。
...
### [119] [A Vector Symbolic Approach to Multiple Instance Learning](https://arxiv.org/abs/2511.16795)
*Ehsan Ahmed Dhrubo,Mohammad Mahmudul Alam,Edward Raff,Tim Oates,James Holt*

Main category: cs.LG

TL;DR: 提出基于向量符号架构（VSA）的多示例学习框架，通过高维向量表示和代数运算严格强制执行MIL的iff约束，在标准基准和医学影像数据集上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习MIL方法大多违反iff约束条件，导致性能指标虚高和泛化能力差，需要一种能严格遵循MIL逻辑约束的框架。

Method: 使用VSA将实例和概念表示为近似正交的高维向量，通过代数运算在分类过程中强制执行iff约束，并设计学习编码器将原始数据转换为VSA兼容向量。
...
### [120] [A Robust Federated Learning Approach for Combating Attacks Against IoT Systems Under non-IID Challenges](https://arxiv.org/abs/2511.16822)
*Eyad Gad,Zubair Md Fadlullah,Mostafa M. Fouda*

Main category: cs.LG

TL;DR: 本文研究了联邦学习算法（FedAvg、FedProx、Scaffold）在不同数据分布下对大规模物联网攻击的分类性能，重点关注统计异构性带来的挑战。


<details>
  <summary>Details</summary>
Motivation: 随着物联网设备激增和数据量暴涨，传统机器学习在资源受限和隐私敏感环境中面临挑战。联邦学习虽能解决这些问题，但非独立同分布数据的统计异构性严重影响了其有效性。

Method: 使用CICIoT2023数据集，在非独立同分布数据环境下比较FedAvg、FedProx和Scaffold三种联邦学习算法的性能。

Result: 通过细致的分析和实验，揭示了这些联邦学习方法在统计异构性条件下的性能差异。

Conclusion: 研究为物联网攻击检测领域的联邦学习应用提供了有价值的见解，帮助研究者和从业者更好地应对统计异构性挑战。
...
### [121] [Monte Carlo Expected Threat (MOCET) Scoring](https://arxiv.org/abs/2511.16823)
*Joseph Kim,Saahith Potluri*

Main category: cs.LG

TL;DR: MOCET是一个可解释且双重可扩展的指标，用于量化AI模型在生物安全领域的现实世界风险，特别是评估ASL-3+模型提升非国家行为者能力的风险。
...
Method: 引入MOCET指标，这是一个可解释且双重可扩展（可自动化和开放式）的度量方法，能够量化现实世界风险。

Result: MOCET能够可靠地量化AI模型在生物安全领域的现实世界风险，特别是评估模型对非国家行为者的能力提升风险。

Conclusion: MOCET填补了现有评估指标的空白，为LLM安全案例提供了更好的风险量化工具，能够跟上AI技术的快速发展步伐。
...
### [122] [ManifoldFormer: Geometric Deep Learning for Neural Dynamics on Riemannian Manifolds](https://arxiv.org/abs/2511.16828)
...
### [123] [Analysis of heart failure patient trajectories using sequence modeling](https://arxiv.org/abs/2511.16839)
*Falk Dippela,Yinan Yu,Annika Rosengren,Martin Lindgren,Christina E. Lundberg,Erik Aerts,Martin Adiels,Helen Sjöland*

Main category: cs.LG

TL;DR: 本文系统比较了三种架构（Transformers、Transformers++、Mambas）在瑞典心力衰竭队列中的表现，发现Llama和Mamba在临床预测任务中表现最佳，且具有高效表示学习能力。
...
Method: 在42820名瑞典心力衰竭患者队列中，评估六种序列模型在三个一年预测任务上的表现，包括输入患者序列修改、架构配置和时间预处理技术。

Result: Llama获得最高预测区分度和最佳校准，在所有任务中表现稳健，其次是Mamba。两种架构都展示了高效表示学习能力，小配置超越其他大型Transformer。

Conclusion: 本研究为临床预测任务提供了系统设计选择的首次消融研究，可作为未来模型开发的起点。
...
### [124] [Provably Minimum-Length Conformal Prediction Sets for Ordinal Classification](https://arxiv.org/abs/2511.16845)
*Zijian Zhang,Xinyu Chen,Yuanjie Shi,Liyuan Lillian Ma,Zifan Xu,Yan Yan*

Main category: cs.LG

TL;DR: 本文提出了一种新的序数分类共形预测方法，该方法具有模型无关性，并提供实例级最优预测区间，显著提高了预测效率。
...
Method: 将序数共形分类制定为实例级的最小长度覆盖问题，开发了一种滑动窗口算法，在每个校准数据上达到最优，时间复杂度仅为标签候选数的线性级别。

Result: 在四个不同领域的基准数据集上的实验表明，所提方法相比基线显著提高了预测效率（在四个数据集上平均减少15%）。

Conclusion: 提出的序数共形预测方法填补了现有方法的空白，实现了模型无关的实例级最优预测，在保持统计有效性的同时显著提升了预测效率。
...
### [125] [Sex and age determination in European lobsters using AI-Enhanced bioacoustics](https://arxiv.org/abs/2511.16848)
...
Result: 年龄分类中大多数模型准确率超过97%（朴素贝叶斯：91.31%）；性别分类中除朴素贝叶斯外所有模型准确率超过93.23%。
...
### [126] [PepEVOLVE: Position-Aware Dynamic Peptide Optimization via Group-Relative Advantage](https://arxiv.org/abs/2511.16912)
*Trieu Nguyen,Hao-Wei Pang,Shasha Feng*

Main category: cs.LG

TL;DR: PepEVOLVE是一个位置感知的动态框架，用于多目标肽序列优化，相比现有方法在不知道最佳编辑位置的情况下能更有效地优化大环肽。


<details>
  <summary>Details</summary>
Motivation: 现有方法如PepINVENT需要预先指定可突变位置，且使用静态预训练和优化算法，限制了模型的泛化能力和优化效果。

Method: 采用动态掩码和CHUCKLES移位增强预训练，使用上下文无关多臂老虎机路由器发现高奖励残基，结合进化优化算法和组相对优势稳定强化学习更新。

Result: 在Rev结合大环肽基准测试中，PepEVOLVE平均得分约0.8（vs 0.6），最佳候选得分0.95（vs 0.87），收敛步骤更少。

Conclusion: PepEVOLVE为肽先导物优化提供了实用、可重复的路径，在未知最优编辑位点时能实现更高效的探索和多目标设计质量提升。
...
### [127] [A Hybrid Computational Intelligence Framework for scRNA-seq Imputation: Integrating scRecover and Random Forests](https://arxiv.org/abs/2511.16923)
*Ali Anaissi,Deshao Liu,Yuanzhe Jia,Weidong Huang,Widad Alyassine,Junaid Akram*

Main category: cs.LG

TL;DR: SCR-MF是一个用于单细胞RNA测序数据中dropout事件处理的模块化两阶段工作流，结合了scRecover的dropout检测和missForest的非参数插补方法，在保持生物保真度和透明度的同时提供稳健性能。


<details>
  <summary>Details</summary>
Motivation: 单细胞RNA测序技术虽然能在细胞分辨率下进行转录组分析，但普遍存在dropout事件，这些事件会掩盖生物信号，影响数据分析的准确性。

Method: 采用模块化两阶段工作流：第一阶段使用scRecover进行基于原理的dropout检测，第二阶段使用missForest进行稳健的非参数插补。

Result: 在公共和模拟数据集上的测试表明，SCR-MF在大多数情况下达到或超过现有插补方法的性能，同时保持生物保真度和透明度，在准确性和计算效率之间提供了有竞争力的平衡。

Conclusion: SCR-MF适用于中等规模单细胞数据集，为单细胞RNA测序数据分析提供了一个稳健且可解释的dropout处理解决方案。
...
### [128] [CroTad: A Contrastive Reinforcement Learning Framework for Online Trajectory Anomaly Detection](https://arxiv.org/abs/2511.16929)
*Rui Xue,Dan He,Fengmei Jin,Chen Zhang,Xiaofang Zhou*

Main category: cs.LG

TL;DR: 本文提出了一种名为CroTad的对比强化学习框架，用于在线轨迹异常检测。该方法无需阈值设置，对噪声和不规则采样数据具有鲁棒性，能够在子轨迹和点级别实时识别异常行为。


<details>
  <summary>Details</summary>
Motivation: 智能交通系统中的轨迹异常检测面临三个主要挑战：子轨迹异常检测研究不足、现有方法依赖阈值调优、不规则采样和训练集噪声影响模型性能。

Method: 结合对比学习和强化学习，CroTad框架学习提取不同行程的多样化正常出行模式，并通过深度强化学习模块进行在线实时异常评分。

Result: 在两个真实世界数据集上的广泛实验表明，该框架在各种评估场景下都具有有效性和鲁棒性。

Conclusion: CroTad框架成功解决了轨迹异常检测中的关键挑战，提供了一种无需阈值、对噪声数据鲁棒的在线检测方法。
...
### [129] [A novel approach to classification of ECG arrhythmia types with latent ODEs](https://arxiv.org/abs/2511.16933)
...
Method: 训练潜在ODE模型来建模连续ECG波形，从高频单通道信号创建鲁棒特征向量；通过将初始360Hz ECG降采样到90Hz和45Hz，为每个波形构建三个潜在向量；使用梯度提升树对这些向量进行分类，并测试跨频率的鲁棒性。

Result: 性能下降极小，在360Hz、90Hz和45Hz下的宏平均AUC-ROC值分别为0.984、0.978和0.976，表明该方法能有效规避信号保真度与电池寿命之间的权衡。

Conclusion: 该方法使更小的可穿戴设备成为可能，促进了心脏健康的长期监测。
...
### [130] [ToC: Tree-of-Claims Search with Multi-Agent Language Models](https://arxiv.org/abs/2511.16972)
...
### [131] [FIRM: Federated In-client Regularized Multi-objective Alignment for Large Language Models](https://arxiv.org/abs/2511.16992)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型与人类价值观对齐需要平衡多个冲突目标，但集中化训练存在计算密集和数据隐私问题，现有联邦多目标优化方法面临严重通信瓶颈。

Method: 提出FIRM算法，每个客户端本地解决正则化多目标优化问题，通过客户端内正则化直接缓解客户端分歧漂移，仅需传输单一参数集。

Result: 算法收敛到帕累托稳定点，提供有限时间收敛保证，实证显示训练动态更平滑、客户端分歧漂移减少、奖励权衡改善。

Conclusion: FIRM能有效平衡多目标对齐中的通信效率和性能，支持根据偏好平滑调整目标权衡。
...
### [132] [Mask the Redundancy: Evolving Masking Representation Learning for Multivariate Time-Series Clustering](https://arxiv.org/abs/2511.17008)
*Zexi Tan,Xiaopeng Luo,Yunlin Liu,Yiqun Zhang*

Main category: cs.LG

TL;DR: 本文提出了一种演化掩码多变量时间序列聚类方法（EMTC），通过重要性感知变量掩码和多内生视图表示学习模块，解决传统方法中冗余时间戳对聚类性能的限制问题。


<details>
  <summary>Details</summary>
Motivation: 多变量时间序列聚类中，时间序列包含大量冗余信息（如稳态机器运行记录、太阳能发电零输出期），这些冗余降低了表示学习中对判别性时间戳的关注，导致聚类性能瓶颈。现有掩码策略多为独立预处理步骤，无法动态适应聚类关键时间戳的重要性变化。

Method: 提出EMTC方法，包含重要性感知变量掩码（IVM）和多内生视图（MEV）表示学习模块。IVM自适应引导模型学习更具判别性的聚类表示，MEV通过重构和对比学习路径增强泛化能力。MEV重构防止掩码过早收敛，聚类引导的对比学习促进表示与聚类的联合优化。

Result: 在15个真实基准数据集上的实验表明，EMTC优于8种最先进方法，平均比最强基线提升4.85%。

Conclusion: EMTC通过动态掩码策略和联合优化框架，有效提升了多变量时间序列聚类的性能，解决了冗余时间戳对表示学习的负面影响。
...
### [133] [Energy Scaling Laws for Diffusion Models: Quantifying Compute and Carbon Emissions in Image Generation](https://arxiv.org/abs/2511.17031)
...
Result: 能耗缩放定律在单个架构内达到高预测精度（R平方>0.9），并在跨架构泛化中保持高秩相关性，能够可靠估计未见过的模型-硬件组合的能耗。
...
### [134] [Hash Collisions in Molecular Fingerprints: Effects on Property Prediction and Bayesian Optimization](https://arxiv.org/abs/2511.17078)
*Walter Virany,Austin Tripp*

Main category: cs.LG

TL;DR: 本文研究了在分子性质预测和贝叶斯优化中使用精确指纹与标准压缩指纹的比较。研究发现精确指纹在预测准确性上有小幅但一致的改进，但在贝叶斯优化性能上没有显著提升。


<details>
  <summary>Details</summary>
Motivation: 分子指纹方法使用哈希函数创建分子的固定长度向量表示，但哈希冲突会导致不同子结构具有相同特征，从而在分子相似性计算中产生高估。本研究旨在调查使用精确指纹是否能比标准压缩指纹提高准确性。

Method: 在分子性质预测和贝叶斯优化中使用高斯过程作为基础预测模型，比较精确指纹和标准压缩指纹的性能。使用了DOCKSTRING数据集中的五个分子性质预测基准进行测试。

Result: 在五个分子性质预测基准上，使用精确指纹相比标准压缩指纹在预测准确性上产生了小幅但一致的改进。然而，这些增益并未转化为贝叶斯优化性能的显著改善。

Conclusion: 虽然精确指纹在分子性质预测中能提供更好的准确性，但这种改进在贝叶斯优化任务中并不显著，表明哈希冲突对贝叶斯优化的影响可能较小。
...
### [135] [Geometric-Disentangelment Unlearning](https://arxiv.org/abs/2511.17100)
*Duo Zhou,Yuji Zhang,Tianxin Wei,Ruizhong Qiu,Ke Yang,Xiao Lin,Cheng Qian,Jingrui He,Hanghang Tong,Heng Ji,Huan Zhang*

Main category: cs.LG

TL;DR: 本文提出了一种基于几何解耦的机器学习遗忘方法(GU)，通过将遗忘梯度更新分解为保留梯度子空间的正交分量，在保证有效遗忘的同时最小化对保留知识的影响。


<details>
  <summary>Details</summary>
Motivation: 现有的机器学习遗忘方法在有效遗忘和保留知识保护之间存在权衡，缺乏对遗忘更新如何影响保留知识的理论分析。本文旨在探索具有理论保证的解决方案。

Method: 提出几何解耦遗忘(GU)方法，将候选遗忘梯度更新分解为保留梯度子空间的切向和法向分量，仅执行法向分量。在信任区域约束下，该方法提供了最优的一阶保留不变移动。

Result: GU方法在TOFU、MUSE和WMDP三个基准测试中均取得了持续改进，能够有效缓解现有基于梯度的遗忘程序的副作用。

Conclusion: 基于几何解耦的遗忘方法提供了一种理论上有保证的简单解决方案，能够在不损害保留知识的情况下实现有效遗忘，并且可以与现有梯度遗忘方法结合使用。
...
### [136] [Four decades of circumpolar super-resolved satellite land surface temperature data](https://arxiv.org/abs/2511.17134)
*Sonia Dupuis,Nando Metzger,Konrad Schindler,Frank Göttsche,Stefan Wunderle*

Main category: cs.LG

TL;DR: 本文开发了一个42年的泛北极地表温度数据集，通过基于深度各向异性扩散模型的超分辨率算法将AVHRR GAC数据从粗分辨率降尺度到1公里。


<details>
  <summary>Details</summary>
Motivation: 北极地区快速变暖，需要长期卫星地表温度记录来监测气候变化。但AVHRR GAC数据的粗空间分辨率限制了其在分析北极细尺度永久冻土动态和其他地表过程方面的应用。
...
### [137] [FlexiFlow: decomposable flow matching for generation of flexible molecular ensemble](https://arxiv.org/abs/2511.17249)
...
Result: 在QM9和GEOM Drugs数据集上实现了最先进的分子生成结果，能够生成有效、无应变、独特且新颖的分子，同时捕捉分子的构象多样性。模型生成的构象集合在推理时间的一小部分内提供了与最先进的基于物理的方法相似的覆盖范围。

Conclusion: FlexiFlow可以成功转移到蛋白质条件配体生成任务，即使数据集仅包含静态口袋而没有伴随构象。该模型在分子生成和构象多样性捕捉方面表现出色。
...
### [138] [Enforcing governing equation constraints in neural PDE solvers via training-free projections](https://arxiv.org/abs/2511.17258)
*Omer Rochman,Gilles Louppe*

Main category: cs.LG

TL;DR: 该论文评估了两种无需训练的后处理投影方法，用于减少神经PDE求解器违反非线性约束的问题，特别是在动力学PDE中。
...
Method: 提出了两种无需训练的后处理投影方法：基于非线性优化的投影，以及使用雅可比-向量和向量-雅可比积的局部线性化投影。

Result: 两种投影方法都显著减少了约束违反，并在代表性PDE上比物理信息基线提高了准确性。

Conclusion: 两种后处理投影方法都能有效改善神经PDE求解器的约束满足性和准确性，为处理非线性约束提供了实用解决方案。
...
### [139] [Automobile demand forecasting: Spatiotemporal and hierarchical modeling, life cycle dynamics, and user-generated online information](https://arxiv.org/abs/2511.17275)
*Tom Nahrendorf,Stefan Minner,Helfried Binder,Richard Zinck*

Main category: cs.LG

TL;DR: 本研究针对德国高端汽车制造商的多产品、多市场、多层次需求预测问题，结合点预测和概率预测方法，使用LightGBM集成模型和混合整数线性规划进行预测协调，发现时空依赖性和整数化偏差显著影响预测精度。


<details>
  <summary>Details</summary>
Motivation: 高端汽车制造商面临产品种类繁多、变体级数据稀疏和市场动态波动等复杂的预测挑战，需要开发有效的多层级需求预测方法。

Method: 结合战略和运营层面的点预测和概率预测，使用LightGBM模型集成、分位数回归和混合整数线性规划协调方法，利用池化训练集进行预测。

Result: 时空依赖性和整数化偏差显著影响预测精度，短期需求受生命周期成熟度、自回归动量和运营信号影响，中期需求受在线参与度、规划目标和竞争指标等前瞻性驱动因素影响，在线行为数据显著提高了细分层面的预测准确性。

Conclusion: 研究强调了整数预测对运营可行性的重要性，并展示了在线行为数据在提高细分层面预测准确性方面的价值，为高端汽车制造商的多层级需求预测提供了有效解决方案。
...
### [140] [SAVeD: Semantic Aware Version Discovery](https://arxiv.org/abs/2511.17298)
...
Method: 采用对比学习框架，通过随机变换（如行删除、编码扰动）生成增强表视图，使用自定义transformer编码器进行嵌入，在潜在空间中优化语义相似性，最小化同一数据集增强视图间的距离，最大化不相关表间的距离。

Result: 在五个标准数据集上的实验显示，SAVeD在完全未见过的表上实现了显著更高的准确率，分离分数显著提升，能够有效区分语义改变的版本。相比未训练基线和现有最先进方法（如Starmie），取得了竞争性或更优的结果。

Conclusion: SAVeD框架能够有效识别结构化数据集的版本，无需依赖元数据或标签，在版本检测任务上表现出色，为数据科学中的重复劳动问题提供了有效解决方案。
...
### [141] [Self-supervised denoising of raw tomography detector data for improved image reconstruction](https://arxiv.org/abs/2511.17312)
*Israt Jahan Tulin,Sebastian Starke,Dominic Windisch,André Bieberle,Peter Steinbach*

Main category: cs.LG

TL;DR: 本文研究了两种自监督深度学习方法来降低超快电子束X射线CT的噪声，相比非学习方法取得了更好的去噪效果。
...
Method: 研究并比较了两种自监督深度学习方法和一种非学习方法的原始探测器数据去噪效果。

Result: 深度学习方法能够增强探测器数据的信噪比，并在重建图像中带来一致性的改善，优于非学习方法。

Conclusion: 自监督深度学习方法是降低超快电子束X射线CT噪声的有效方法，能够显著提升图像质量。
...
### [142] [ReBaPL: Repulsive Bayesian Prompt Learning](https://arxiv.org/abs/2511.17339)
*Yassir Bendou,Omar Ezzahir,Eduardo Fernandes Montesuma,Gabriel Mahuas,Victoria Shevchenko,Mike Gartrell*

Main category: cs.LG

TL;DR: 本文提出了ReBaPL方法，通过贝叶斯框架和排斥力机制改进提示学习，解决过拟合和泛化问题，在多个基准数据集上表现优于现有方法。
...
Method: 结合循环步长调度和SGHMC算法，交替进行探索和优化；引入基于概率度量的排斥力机制，在表示空间产生排斥作用防止过早收敛到单一模式。

Result: 在多个基准数据集上验证了ReBaPL的有效性，相比现有最先进的提示学习方法表现出更优越的性能。

Conclusion: ReBaPL提供了一种模块化的贝叶斯扩展方案，能够更全面地刻画提示后验分布，显著提升了泛化能力。
...
### [143] [Convergence and stability of Q-learning in Hierarchical Reinforcement Learning](https://arxiv.org/abs/2511.17351)
*Massimiliano Manenti,Andrea Iannelli*

Main category: cs.LG

TL;DR: 本文提出了封建Q学习方案，分析了其收敛和稳定性的条件，通过随机逼近理论和ODE方法证明了收敛性，并展示了更新收敛到博弈均衡点，实验验证了理论结果。


<details>
  <summary>Details</summary>
Motivation: 分层强化学习在捕捉决策问题的时间结构和增强持续学习能力方面具有潜力，但理论保证落后于实践，需要为封建强化学习提供收敛和稳定性分析。

Method: 提出封建Q学习方案，利用随机逼近理论和ODE方法进行收敛性分析，将更新过程建模为博弈均衡问题。

Result: 证明了封建Q学习在特定条件下的收敛性和稳定性，更新收敛到博弈均衡点，实验支持理论预期。

Conclusion: 为封建强化学习提供了原则性的收敛和稳定性分析，开启了将博弈论方法应用于分层强化学习的可能性。
...
### [144] [R2PS: Worst-Case Robust Real-Time Pursuit Strategies under Partial Observability](https://arxiv.org/abs/2511.17367)
*Runyu Lu,Ruochuan Shi,Yuanheng Zhu,Dongbin Zhao*

Main category: cs.LG

TL;DR: 该论文提出了首个在部分可观测性下的最坏情况鲁棒实时追捕策略（R2PS）方法，通过信念保持机制和强化学习框架，实现了在未知图结构上的零样本泛化。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理部分可观测性和逃避者预测能力方面存在局限，缺乏实时适用的图基追捕策略，特别是在追捕者只有不完美信息的情况下。

Method: 首先证明传统动态规划算法在异步移动下保持最优性，然后提出信念保持机制扩展DP策略到部分可观测设置，最后将信念保持嵌入EPG框架进行跨图强化学习。

Result: 强化学习后的策略在未见过的真实世界图结构上实现了鲁棒的零样本泛化，并且持续优于现有游戏RL方法直接在测试图上训练的策略。

Conclusion: R2PS方法成功解决了部分可观测性下的实时追捕策略问题，通过信念保持和强化学习的结合，实现了对未知环境的鲁棒泛化能力。
...
### [145] [Stable Coresets via Posterior Sampling: Aligning Induced and Full Loss Landscapes](https://arxiv.org/abs/2511.17399)
*Wei-Kai Chang,Rajiv Khanna*

Main category: cs.LG

TL;DR: 提出了一种基于后验采样的新框架来解决梯度核心集选择方法中的挑战，包括在数据污染场景下的鲁棒性、损失曲率不匹配问题，并通过平滑损失函数提高稳定性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习模型规模扩大，计算需求增加，需要有效的核心集选择技术来加速训练。梯度方法虽然有理论优势，但面临SGD基线强和损失曲率不匹配导致代表性失效的问题。

Method: 建立后验采样与损失景观的联系，提出基于后验采样的平滑损失函数，并进行收敛性分析。

Result: 实验表明该方法在多个数据集上实现了比现有技术更快的训练速度和更好的泛化性能。

Conclusion: 该框架通过后验采样解决了核心集选择的关键挑战，在保持计算效率的同时提高了稳定性和泛化能力。
...
### [146] [DS-Span: Single-Phase Discriminative Subgraph Mining for Efficient Graph Embeddings](https://arxiv.org/abs/2511.17419)
*Yeamin Kaiser,Muhammed Tasnim Bin Anwar,Bholanath Das,Chowdhury Farhan Ahmed,Md. Tanvir Alam*

Main category: cs.LG

TL;DR: DS-Span是一个单阶段判别子图挖掘框架，统一了模式增长、剪枝和监督驱动评分，通过覆盖上限机制和信息增益指导选择，生成紧凑且判别性的子图特征用于图嵌入和分类。


<details>
  <summary>Details</summary>
Motivation: 现有频繁或判别子图挖掘方法存在冗余多阶段流程、高计算成本和挖掘结构与判别相关性弱耦合的问题，需要更高效统一的解决方案。

Method: 提出DS-Span单阶段框架，引入覆盖上限资格机制动态限制探索，使用信息增益指导选择具有强类别分离能力的子图并最小化冗余。

Result: 在基准测试中，DS-Span比多阶段方法生成更紧凑和判别性的子图特征，以显著减少的运行时间达到更高或相当的准确率。
...
### [147] [Towards fully differentiable neural ocean model with Veros](https://arxiv.org/abs/2511.17427)
*Etienne Meunier,Said Ouala,Hugo Frezat,Julien Le Sommer,Ronan Fablet*

Main category: cs.LG

TL;DR: 提出了VEROS海洋模型的可微分扩展，使其与JAX自动微分框架兼容，并展示了基于梯度的海洋状态修正和物理参数校准两个应用。


<details>
  <summary>Details</summary>
Motivation: 使海洋模型能够进行自动微分，从而支持端到端学习和参数调优，提高海洋建模的效率和准确性。
...
### [148] [Unmasking Airborne Threats: Guided-Transformers for Portable Aerosol Mass Spectrometry](https://arxiv.org/abs/2511.17446)
*Kyle M. Regan,Michael McLoughlin,Wayne A. Bryden,Gonzalo R. Arce*

Main category: cs.LG

TL;DR: 提出了MS-DGFormer框架，使用Transformer架构直接处理原始质谱数据，通过字典编码器结合SVD去噪，实现单次光谱的病原体识别，使MALDI-MS系统能够在现场条件下进行自主实时分析。


<details>
  <summary>Details</summary>
Motivation: 传统MALDI-MS依赖繁琐的样品制备和多光谱平均，限制了其在实时环境监测中的应用，特别是在新兴的气溶胶MALDI-MS系统中，自主采样会产生噪声光谱，需要单次检测能力。

Method: 采用Transformer架构捕捉时间序列光谱的长程依赖关系，引入基于奇异值分解(SVD)的字典编码器集成去噪光谱信息，直接从原始质谱数据中提取关键生物分子模式。

Result: 实现了对单次光谱的稳健病原体识别性能，为气溶胶样本提供卓越的病原体识别系统。

Conclusion: 该方法消除了广泛预处理的需求，释放了便携式可部署MALDI-MS平台的潜力，彻底改变了环境病原体检测和生物威胁快速响应能力。
...
### [149] [PersonaAgent with GraphRAG: Community-Aware Knowledge Graphs for Personalized LLM](https://arxiv.org/abs/2511.17467)
...
### [150] [Harnessing Data from Clustered LQR Systems: Personalized and Collaborative Policy Optimization](https://arxiv.org/abs/2511.17489)
*Vinay Kanakeri,Shivam Bajaj,Ashwin Verma,Vijay Gupta,Aritra Mitra*

Main category: cs.LG

TL;DR: 该论文提出了一种在强化学习中通过聚类提高样本效率的方法，针对多智能体线性二次调节器设置，通过同时进行聚类和学习来为每个聚类输出个性化策略。


<details>
  <summary>Details</summary>
Motivation: 强化学习通常数据需求量大，现有方法尝试利用'近似相似'过程的数据来提高样本效率，但由于过程模型未知，识别相似过程具有挑战性。

Method: 结合顺序消除和零阶策略优化的思想，提出新算法在多智能体环境中同时执行聚类和学习，每个智能体对应一个线性过程副本，基于动态和任务的相似性进行聚类。

Result: 在适当的聚类分离概念下，证明方法能以高概率保证正确聚类，每个聚类的策略次优性差距与聚类大小成反比，且没有额外偏差。

Conclusion: 这是首个揭示聚类如何在数据驱动控制中用于学习个性化策略的工作，既能享受协作带来的统计增益，又不会因包含不相似过程数据而遭受次优性，且通信开销仅为对数级别。
...
### [151] [Stable diffusion models reveal a persisting human and AI gap in visual creativity](https://arxiv.org/abs/2511.16814)
...
### [152] [Cognitive BASIC: An In-Model Interpreted Reasoning Language for LLMs](https://arxiv.org/abs/2511.16837)
*Oliver Kramer*
...
Conclusion: Cognitive BASIC为LLM推理提供了一种结构化的、可解释的方法，能够有效支持多步推理任务。
...
### [153] [Fantastic Bugs and Where to Find Them in AI Benchmarks](https://arxiv.org/abs/2511.16842)
...
TL;DR: 提出了一个利用响应模式统计分析的系统性基准修订框架，通过检测超出预期范围的统计指标来识别无效问题，结合专家评审和LLM预审，在九个基准测试中达到84%的识别精度。


<details>
  <summary>Details</summary>
Motivation: 基准测试对AI发展至关重要，但无效的基准问题会破坏其可靠性。手动从数千个问题中识别和修正错误既不可行，也是可靠评估的关键瓶颈。

Method: 基于AI评估中常用的核心假设——平均分足以概括模型性能，构建单维潜在结构测量实验。通过分析响应模式统计指标，当项目统计值超出预期范围时标记为潜在问题，结合专家评审和LLM预审减少人工工作量。

Result: 在九个广泛使用的基准测试中，该方法指导专家评审识别问题问题的精度高达84%，并显著减少了人工工作量。

Conclusion: 该框架为系统性基准修订提供了高效且可扩展的解决方案，通过统计分析和自动化预审相结合的方式，有效提升了基准测试的可靠性。
...
### [154] [Hybrid Differential Reward: Combining Temporal Difference and Action Gradients for Efficient Multi-Agent Reinforcement Learning in Cooperative Driving](https://arxiv.org/abs/2511.16916)
*Ye Han,Lijun Zhang,Dejian Meng,Zhuang Zhang*

Main category: cs.AI

TL;DR: 提出混合差分奖励机制解决多车协同驾驶中传统状态奖励函数存在的奖励差异消失问题，通过整合时间差分奖励和动作梯度奖励来提高策略梯度的信噪比，显著提升算法收敛速度和策略稳定性。


<details>
  <summary>Details</summary>
Motivation: 在多车协同驾驶的高频连续控制任务中，传统状态奖励函数存在奖励差异消失问题，导致策略梯度的信噪比低，严重影响算法收敛和性能提升。

Method: 提出混合差分奖励机制，包含两个互补组件：基于全局势函数的时间差分奖励和直接衡量动作边际效用的动作梯度奖励。将协同驾驶问题建模为具有时变智能体集的多智能体部分可观测马尔可夫博弈。

Result: 通过在线规划和多智能体强化学习算法的广泛实验证明，HDR机制显著提高了收敛速度和策略稳定性，指导智能体学习到能够有效平衡交通效率和安全的优质协同策略。

Conclusion: HDR机制成功解决了多车协同驾驶中的奖励信号问题，为高频连续控制任务提供了有效的奖励设计方法，能够指导智能体学习高质量的协同策略。
...
### [155] [Budget-Aware Tool-Use Enables Effective Agent Scaling](https://arxiv.org/abs/2511.17006)
...
Method: 提出预算跟踪器插件提供持续预算意识，并开发BATS框架，利用预算意识动态调整规划和验证策略，决定是深入挖掘还是转向新路径。同时建立了统一的成本度量标准，联合考虑token和工具消耗。
...
### [156] [DAPS++: Rethinking Diffusion Inverse Problems with Decoupled Posterior Annealing](https://arxiv.org/abs/2511.17038)
*Hao Chen,Renzheng Zhang,Scott S. Howard*

Main category: cs.AI

TL;DR: 该论文提出DAPS++方法，将扩散模型在逆问题求解中的角色重新解释为EM框架中的初始化阶段，实现了扩散阶段与数据驱动优化的解耦，从而更直接地利用似然项指导推理，提高计算效率和重建性能。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯视角下的基于分数的扩散方法在解决逆问题时存在局限性：先验提供有限指导，重建主要由测量一致性项驱动，导致推理过程与扩散动力学实际解耦。需要重新理解扩散在逆问题求解中的结构。

Method: 提出DAPS++方法，将扩散重新解释为EM框架中的初始化阶段，实现扩散阶段与数据驱动优化的完全解耦，允许似然项更直接地指导推理，同时保持数值稳定性。

Result: DAPS++需要更少的函数评估次数和测量优化步骤，在多种图像恢复任务中实现了高计算效率和鲁棒的重建性能。

Conclusion: 通过重新解释扩散在逆问题求解中的角色为EM框架中的初始化阶段，DAPS++方法提供了对统一扩散轨迹在实践中保持有效性的新理解，并实现了更高效和鲁棒的图像重建。
...
### [157] [MIR: Efficient Exploration in Episodic Multi-Agent Reinforcement Learning via Mutual Intrinsic Reward](https://arxiv.org/abs/2511.17165)
...
<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中的情节奖励面临两个主要挑战：联合行动轨迹的指数级稀疏性，以及现有方法未能充分考虑影响团队状态的联合行动。需要一种有效的方法来解决这些稀疏奖励问题。

Method: 提出了相互内在奖励(MIR)方法，该方法激励个体智能体探索能够影响队友的行动。当与原始策略结合时，MIR能有效刺激团队探索并提升算法性能。

Result: 通过将代表性单智能体MiniGrid环境扩展为MiniGrid-MA多智能体环境进行综合实验验证。与最先进方法相比，MIR方法在MiniGrid-MA设置中表现出优越的性能。

Conclusion: MIR是一种简单而有效的增强策略，专门针对具有极度稀疏奖励的多智能体强化学习场景，能够显著改善团队探索和算法性能。
...
### [158] [Designing Domain-Specific Agents via Hierarchical Task Abstraction Mechanism](https://arxiv.org/abs/2511.17198)
*Kaiyu Li,Jiayu Wang,Zhi Wang,Hui Qiao,Weizhan Zhang,Deyu Meng,Xiangyong Cao*

Main category: cs.AI

TL;DR: 本文提出了一个基于层次任务抽象机制(HTAM)的新型多智能体框架EarthAgent，专门用于解决遥感等专业领域中复杂多步骤任务规划问题，显著优于现有的单智能体和多智能体系统。


<details>
  <summary>Details</summary>
Motivation: 现有基于通用框架(如ReAct)或人类角色扮演的LLM驱动智能体在需要严格结构化工作流程的专业领域(如遥感)中表现不佳，这些领域需要专业工具和多步骤程序，对通用方法构成重大挑战。

Method: 提出层次任务抽象机制(HTAM)，将多智能体系统构建为反映领域内在任务依赖关系的逻辑层次结构。该任务中心架构确保程序正确性，将复杂问题分解为顺序层次，每个层次的子智能体基于前一层输出进行操作。

Result: 构建了EarthAgent多智能体系统和GeoPlan-bench基准测试，实验表明EarthAgent在工具选择、路径相似性和逻辑完整性方面显著优于现有单智能体和多智能体系统。

Conclusion: 将智能体架构与领域内在任务结构对齐是构建鲁棒可靠的专业自主系统的关键步骤，层次任务抽象机制为专业领域复杂任务规划提供了有效解决方案。
...
### [159] [Agentifying Agentic AI](https://arxiv.org/abs/2511.17332)
*Virginia Dignum,Frank Dignum*

Main category: cs.AI

TL;DR: 本文主张AAMAS社区开发的概念工具（如BDI架构、通信协议、机制设计和制度建模）为智能AI系统提供了理论基础，通过将自适应数据驱动方法与结构化推理协调模型相结合，构建具有透明度、合作性和问责制的智能系统。


<details>
  <summary>Details</summary>
Motivation: 智能AI系统需要持续自主性、推理和交互能力，这要求对认知、合作和治理建立明确模型，而AAMAS社区的概念工具恰好提供了这样的基础。

Method: 将自适应数据驱动方法与结构化推理协调模型（包括BDI架构、通信协议、机制设计和制度建模）相结合。

Result: 提出了连接形式理论和实践自主性的智能系统视角，使系统不仅具备能力和灵活性，还具有透明度、合作性和问责制。

Conclusion: AAMAS社区的概念工具为构建透明、合作和负责任的智能AI系统提供了坚实基础，实现了形式理论与实践自主性的桥梁。
...
### [160] [That's not natural: The Impact of Off-Policy Training Data on Probe Performance](https://arxiv.org/abs/2511.17408)
*Nathalie Kirch,Samuel Dower,Adrians Skapars,Ekdeep Singh Lubana,Dmitrii Krasheninnikov*

Main category: cs.AI

TL;DR: 本文系统评估了使用合成和策略外数据训练LLM探测器的泛化性能，发现响应生成策略显著影响探测器性能，策略外数据泛化成功可预测策略内泛化成功，但不同领域的数据偏移会导致更大性能下降。


<details>
  <summary>Details</summary>
Motivation: 由于许多行为的自然示例稀少，研究者不得不依赖合成或策略外LLM响应来训练探测器，但这种方法对探测器泛化能力的影响尚未系统研究。

Method: 在八个不同的LLM行为上测试线性和注意力探测器，评估不同响应生成策略对探测器性能的影响，并分析从策略外数据到策略内数据的泛化能力。

Result: 响应生成策略显著影响探测器性能，策略外数据泛化成功可预测策略内泛化成功，但不同领域的数据偏移会导致更大性能下降，欺骗和伪装探测器在实际监控场景中可能无法泛化。

Conclusion: 在缺乏策略内数据时，使用同领域策略外数据比使用不同领域策略内数据产生更可靠的探测器，强调需要更好处理LLM监控中分布偏移的方法。
...
### [161] [SRA-CP: Spontaneous Risk-Aware Selective Cooperative Perception](https://arxiv.org/abs/2511.17461)
*Jiaxi Liu,Chengyuan Ma,Hang Zhou,Weizhe Tang,Shixiao Liang,Haoyang Ding,Xiaopeng Li,Bin Ran*

Main category: cs.AI

TL;DR: SRA-CP是一种自发的风险感知选择性协同感知框架，通过轻量级感知覆盖摘要广播和风险触发机制，在保持安全关键目标检测精度的同时大幅降低通信带宽需求。


<details>
  <summary>Details</summary>
Motivation: 现有协同感知方法需要传输大量与驾驶安全无关的感知数据，超出通信带宽限制，且依赖预定义的通信伙伴，不适合动态交通环境。

Method: 提出去中心化协议，车辆持续广播轻量级感知覆盖摘要，仅在检测到风险相关盲区时启动针对性合作；包含感知风险识别模块和选择性信息交换融合模块。

Result: 在公共数据集上评估，相比通用协同感知方法，SRA-CP对安全关键对象的平均精度损失小于1%，仅使用20%通信带宽；相比现有选择性方法，感知性能提升15%。

Conclusion: SRA-CP框架有效解决了协同感知中的通信带宽和动态适应性挑战，在保持安全性能的同时显著降低了通信开销。
